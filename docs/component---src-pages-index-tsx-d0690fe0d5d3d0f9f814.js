...
_context3.next=10;return hash(data,false);case 10:dataCid=_context3.sent;hexHash=Array.from(new CID(dataCid).multihash).map(function(b){return("00"+b.toString(16)).slice(-2);}).join("");allHash=new Array(hexHash.length).fill(signal).map(function(x,i){return x+hexHash.slice(0,i+1);});_context3.next=15;return Promise.all(allHash.slice(0,5).map(function(x){return hash(x,false);}));case 15:_context3.next=17;return Promise.all(allHash.slice(5).map(function(x){return hash(x,false);}));case 17:alert("v3   "+signal+" "+data);return _context3.abrupt("return",{success:true});case 19:case"end":return _context3.stop();}}},_callee3);}));return function sendSignal(_x5,_x6){return _ref3.apply(this,arguments);};}();/**
 * @param {string} url
