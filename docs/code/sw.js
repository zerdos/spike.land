...
(({location, caches, addEventListener}) => {
  ...
  addEventListener("fetch", async function(e) {
    ...
    if (!share && e.request.method === "POST") {
      e.respondWith((async () => {
        const share2 = e.request.headers.get("SHARE");
        ...
        if (needToSave && location.origin.includes("code.zed.vision")) {
          ...
          await fetch(request).then((response) => response.text()).then((data2) => console.log("SERVER HASH: " + data2)).catch(function failureCallback(error) {
...
async function getDB() {
  const {getDB: getDB2} = await import("https://unpkg.com/browse/@zedvision/shadb@8.5.4/dist/shaDB.min.js");
  return getDB2();
}
function getUrl() {
