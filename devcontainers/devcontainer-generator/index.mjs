...
var DevcontainerGenerator = class {
  ...
  async generate() {
    ...
    if (this._nodeVersion) {
      this._dockerfile += dockerTemplates["node"].replace("{NODE_VERSION}", node[this._nodeVersion])
        .replace("{YARN_VERSION}", yarn);
      this._readme += readmeTemplates["node"].replace("{NODE_VERSION}", node[this._nodeVersion])
        .replace("{YARN_VERSION}", yarn);
    }
    if (this._dotnet) {
      if (this._dotnet === "6") {
        this._dockerfile += dockerTemplates["dotnet6"].replace("{DOTNET_SDK_VERSION}", dotnet6)
          .replace(
            "{arm_dotnet_sha512}",
        ...
        this._dockerfile += dockerTemplates["dotnet3"].replace("{DOTNET_SDK_VERSION}", dotnet3)
          .replace(
            "{amd_dotnet_sha512}",
            sha.dotnet_sha512[dotnet3],
          );
      } else {
        this._dockerfile += dockerTemplates["dotnet5"].replace("{DOTNET_SDK_VERSION}", dotnet5)
          .replace(
            "{dotnet_sha512}",
