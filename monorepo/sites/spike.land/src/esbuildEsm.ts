  ...
  "../../node_modules/esbuild-wasm/lib/browser.js"(exports, module) {
    ((module2) => {
      ...
      function createChannel(streamIn) {
        ...
        let handleIncomingPacket = (bytes) => {
          if (isFirstPacket) {
            ...
            if (binaryVersion !== "0.14.38") {
              throw new Error(
                `Cannot start service: Host version "${"0.14.38"}" does not match binary version ${
        ...
        let handlePlugins = (initialOptions, plugins, buildKey, stash, refs) =>
          __async(this, null, function* () {
                      ...
                      try {
                        ...
                        if (result != null) {
                          ...
                          checkForInvalidFlags(
                            result,
                            keys,
                            `from onLoad() callback in plugin ${
                              JSON.stringify(name)
                            }`,
                          );
                          response.id = id;
            ...
            if (onEndCallbacks.length > 0) {
              runOnEndCallbacks = (result, logPluginError, done) => {
                (() =>
                  __async(this, null, function* () {
                    for (
                      const { name, callback: callback2, note }
                        of onEndCallbacks
                    ) {
                      try {
            ...
            let onmessage = ((postMessage) => {
              ...
              onmessage2 = ({ data: wasm2 }) => {
                ...
                if (wasm2 instanceof WebAssembly.Module) {
                  WebAssembly.instantiate(wasm2, go.importObject).then((
                    instance,
                  ) => go.run(instance));
                } else {
                  WebAssembly.instantiate(wasm2, go.importObject).then((
...
export { init, transform2 as transform };
