        ...
        return await handleErrors(request, async () => {
            ...
            switch (path[0]) {
                ...
                case "hydrated": {
                    ...
                    const css = mST().css;
                    const html = HYDRATED.replace(`<script type="importmap-shim" src="https://unpkg.com/@spike.land/code@{VERSION}/js/importmap.json"></script>`, `<script type="importmap-shim">${JSON.stringify(imap)}</script>`)
                        .replace(`<div id="root"></div>`, `<div id="root"><style>${css}</style><div id="zbody">${htmlContent}</div></div>`).replaceAll(vReg, version);
                ...
                case "public": {
                    const htmlContent = mST().html;
                    const css = mST().css;
                    const html = HTML.replace(`<script type="importmap-shim" src="https://unpkg.com/@spike.land/code@{VERSION}/js/importmap.json"></script>`, `<script type="importmap-shim">${JSON.stringify(imap)}</script>`).replace(`<div id="root"></div>`, `<div id="root"><style>${css}</style><div id="zbody">${htmlContent}</div></div>`).replaceAll(vReg, version);
                    return new Response(html, {
                        status: 200,
                        headers: {
                            "Access-Control-Allow-Origin": "*",
    ...
    async handleSession(webSocket, ip) {
        ...
        webSocket.addEventListener("message", async (msg) => {
            ...
            try {
                ...
                this.state.mySession.addEvent({ ...data, uuid: session.uuid });
