    ...
    async function tryToResp(request2, env2, retry) {
      try {
        ...
        const origResp = await Promise.any([
          ...
          fetch(
            `https://raw.githubusercontent.com/spike-land/monorepo/v${version2}/monorepo/packages/code/${targetPath}`,
          ).then((req) => {
            if (!req.ok) {
              throw req.status;
            }
  ...
  async handleSession(webSocket, ip) {
    ...
    webSocket.addEventListener("message", async (msg) => {
      ...
      try {
        ...
        if (
          !(data.type &&
            (data.type === "new-ice-candidate" || data.type === "offer" ||
              data.type === "answer")) && !limiter.checkLimit()
        ) {
