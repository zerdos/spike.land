...
var initSession = async (room, initData) => {
  initSess = initSess || (await import("./session-7NWFG3AT.mjs")).default;
...
async function startMonacoWithSession(session) {
  ...
  const { startMonaco } = await import("./editor-UTCUAKWF.mjs");
...
async function runner(c, changes = null, session, counter) {
  ...
  esbuildEsmTransform = esbuildEsmTransform || (await import("./esbuildEsm-YLA7P22H.mjs")).transform;
  ...
  try {
    ...
    if (transpiled.length > 0) {
      ...
      try {
        getHtmlAndCss = getHtmlAndCss || (await import("./renderToString-X5GC7FK6.mjs")).getHtmlAndCss;
        if (counter < session.i) {
          return;
        }
