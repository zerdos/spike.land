...
import { Fragment as Fragment2 } from "https://spike.land/dist/react.mjs";
var webRtcArray = [];
var hostname = window.location.hostname || "spike.land";
var path = location.pathname.split("/");
var [, prefix, selector, room] = path;
console.log({ prefix, selector, room });
var roomName = prefix === "api" && selector === "room" ? room.slice(-12) || "code-main" : prefix === "live" && !!selector ? selector.slice(-12) : "code-main";
...
var { startSession } = await import("./session-3VOSR3IM.mjs");
...
async function join(App) {
  ...
  if (!w.sess) {
    ...
    const { quickStart } = await import("./quickStart-WVHR44VX.mjs");
  ...
  async function processWsMessage(event, source) {
    ...
    if (data.patch && data.name !== username) {
      ...
      if (wsLastHashCode !== mySession.hashCode()) {
        ...
        const resp2 = await fetch(`https://spike.land/api/room/${room}/session`);
    ...
    async function createPeerConnection(target) {
      ...
      connections[target].onicecandidate = (event2) => {
        if (event2.candidate) {
      ...
      connections[target].ondatachannel = (event2) => {
