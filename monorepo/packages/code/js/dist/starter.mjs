import "./chunks/chunk-XA6V4FWH.mjs";
...
var run = async (injectedRoom) => {
  ...
  const user = (self && self.crypto && self.crypto.randomUUID && self.crypto.randomUUID() || (await import("./chunks/uidV4-SH7FVCAO.mjs")).default()).slice(0, 8);
  if (location.pathname.includes("hydrate")) {
    ...
    const { join: join2 } = await import("./chunks/ws-2F2EMHQN.mjs");
  ...
  const { join } = await import("./chunks/ws-2F2EMHQN.mjs");
  join(room, user);
};
export {
