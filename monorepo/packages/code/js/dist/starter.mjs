...
var start = async (App) => {
  ...
  const p = import("./chunks/renderPreviewWindow-LB5S3JYA.mjs");
  ...
  const { join } = await import("./chunks/ws-B6BVGXRT.mjs");
  join(App);
...
var run = async () => {
  if (globalThis.App)
    return;
  const { roomName } = await import("./chunks/ws-B6BVGXRT.mjs");
