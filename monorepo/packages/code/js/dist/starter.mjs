...
var run = async (injectedRoom) => {
  ...
  if (location.pathname.includes("hydrate")) {
    ...
    const createDelta = (await import("https://unpkg.com/@spike.land/esm@0.6.68/dist/textdiff-create.mjs")).default;
    const { jsx } = await import("https://unpkg.com/@spike.land/esm@0.6.68/dist/emotion-react.mjs");
    ...
    const { join: join2 } = await import("./chunks/ws-CCH6KAWZ.mjs");
  ...
  const { join } = await import("./chunks/ws-CCH6KAWZ.mjs");
  join(room, user);
};
export {
