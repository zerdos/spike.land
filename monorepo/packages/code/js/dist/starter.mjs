...
var start = async (App) => {
  ...
  const p = import("./chunks/renderPreviewWindow-EOEJ4L4M.mjs");
  ...
  const { join } = await import("./chunks/ws-5EI3VUBQ.mjs");
  join(App);
...
var run = async () => {
  if (globalThis.App)
    return;
  const { roomName } = await import("./chunks/ws-5EI3VUBQ.mjs");
