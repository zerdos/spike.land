...
const main = async (IPFS, ORBITDB) => {
  ...
  const startWriter = async (db, interval) => {
    ...
    updateInterval = setInterval(async () => {
      try {
        ...
        writerText.innerHTML = '<span style="color: red">' + e.toString() +
          "</span>";
  ...
  const update = async (db) => {
    ...
    if (db.type === "eventlog") {
      const value = "GrEEtinGs from " + orbitdb.id + " " + creature +
        ": Hello #" + count + " (" + time + ")";
      await db.add(value);
  ...
  const queryAndRender = async (db) => {
    ...
    outputElm.innerHTML = `
      ...
      <div><b>Oplog Size:</b> ${
      Math.max(db._replicationStatus.progress, db._oplog.length)
      ...
      result && Array.isArray(result) && result.length > 0 &&
        db.type !== "docstore" && db.type !== "keyvalue"
