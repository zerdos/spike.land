import getVersions from "./versions.js.js";
const v = getVersions();
async function transpile(code) {
    const { transpileCode } = await import("./transpile.js.js");
...
export async function run(mode = "window", _w) {
    ...
    try {
        const { getCodeToLoad } = await import("./data.js.js");
    ...
    if (mode === "editor") {
        const { renderDraggableEditor } = await import("./DraggableEditor.js.js");
        renderDraggableEditor();
    }
    if (mode === "window") {
        const onShare = async () => {
            const { shareItAsHtml } = await import("./share.js.js");
            const link = await shareItAsHtml({ code: session.transpiled, HTML: session.HTML });
            open(link);
        };
        const { renderDraggableWindow } = await import("./DraggableWindow.js.js");
        ...
        try {
            ...
            if (err.length === 0 && transpiled.length) {
                ...
                if (session.transpiled !== transpiled) {
                    ...
                    const { saveCode } = await import("./data.js.js");
