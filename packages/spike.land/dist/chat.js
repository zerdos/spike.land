...
exports.getImportMapStr = exports.imap = void 0;
...
exports.default = {
    async fetch(request, env, ctx) {
        return (0, handleErrors_1.handleErrors)(request, async () => {
            ...
            if (serveJs && u.pathname.endsWith(".tsx") &&
            ...
            if (!path[0]) {
                ...
                return new Response(`<meta http-equiv="refresh" content="0; URL=${u.origin}/live/coder/" />`, {
                    headers: {
            ...
            const handleFetchApi = async (path) => {
                ...
                return (async (request) => {
                    ...
                    if (path[0].startsWith("npm:") || path[0].startsWith("node_modules/")) {
                        ...
                        const responseToCache = new Response(isText
                    ...
                    if (path[0].startsWith("unpkg:")) {
                        const esmUrl = u.toString().replace(u.origin + "/unpkg:", "https://unpkg.com/");
                        let resp = await fetch(esmUrl, { ...request, url: esmUrl });
                        if (resp !== null && !resp.ok || resp.status === 307) {
                            const redirectUrl = resp.headers.get("location");
                            if (redirectUrl) {
              ...
              ` +
                    ...
                    if (path[0].startsWith("node_modules")) {
                        const esmUrl = u.toString().replace(u.origin + "/node_modules/", "https://unpkg.com/");
                        let resp = await fetch(esmUrl, { ...request, url: esmUrl });
                        if (resp !== null && !resp.ok || resp.status === 307) {
                            const redirectUrl = resp.headers.get("location");
                            if (redirectUrl) {
              ...
              ` +
                    ...
                    switch (path[0]) {
                        ...
                        case "files.json":
                            return new Response(__STATIC_CONTENT_MANIFEST_1.default, {
                                headers: {
                        ...
                        case "importmap.json":
                            return new Response((0, exports.getImportMapStr)(url.origin), {
                                headers: {
                                    "Content-Type": "application/json;charset=UTF-8",
                                    "Cache-Control": "no-cache",
                                },
                        ...
                        case "live":
                            ...
                            return Promise.any([
                                handleApiRequest(["room", ...paths, "public"], request, env),
                                path.length > 2
                        ...
                        default:
                            const kvResp = await (0, kv_asset_handler_1.getAssetFromKV)({
                                ...
                                ASSET_MANIFEST: __STATIC_CONTENT_MANIFEST_1.default,
                            });
...
const getImportMapStr = (orig) => {
    const importmapImport = { ...exports.imap.imports };
    for (const [key, value] of Object.entries(exports.imap.imports)) {
