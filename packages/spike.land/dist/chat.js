...
const api = {
    fetch: async (req, env, ctx) => {
        ...
        time`);
                ...
                return new Response(`<meta http-equiv="refresh" content="0; URL=${u.origin}/live/${start}" />`, {
                    status: 307,
                    headers: {
            ...
            const handleFetchApi = async (path) => {
                ...
                if (path[0].startsWith("npm:") || path[0].startsWith("node_modules/")) {
                    ...
                    const isText = !!response?.headers?.get("Content-Type")?.includes("charset");
              ...
              `
                ...
                switch (path[0]) {
                    ...
                    case "live": {
                        ...
                        return handleApiRequest(["room", ...paths], request, env).catch((e) => new Response("Error," + e?.message, {
                            status: 500,
