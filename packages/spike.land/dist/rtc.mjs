...
async function handleGet(request, env) {
    ...
    return new Response(`<html><body style="font-size: 24px; padding: 18px; font-family: Arial, sans-serif"">Hello from P2PCF<br/><div style=\"line-height: 28px; margin-top: 8px; font-size: 0.8em\">${hasStore
        ? "&#128077; R2 bucket is configured properly, ready to serve."
        : '&#10060; Couldn\'t find a configured R2 bucket.<br/>Make sure you <a href="https://github.com/gfodor/p2pcf/blob/master/INSTALL.md#set-up-the-r2-bucket" target="_blank">created a bucket</a> and <a href="https://github.com/gfodor/p2pcf/blob/master/INSTALL.md#bind-the-worker-to-r2" target="_blank">connected the worker to it</a>.'}</div></body></html>`, {
        headers: {
...
async function lookupEntries(rtcId, store) {
    let maxIndex = -1;
    const maxIndexEntry = await store.get(`rtcs/${rtcId}/max_index`);
    if (maxIndexEntry) {
...
async function handleDelete(request, env, context) {
    ...
    const rtcId = payload.r;
    const store = getStore(env);
    const [entries, maxIndex] = await lookupEntries(rtcId, store);
...
async function handlePost(request, env, context) {
    ...
    const now = new Date().getTime();
    // R2 needs vacuum
    const nextVacuumEntry = store.get(`rtcs/${rtcId}/next_vacuum`);
    const [entries, maxIndex] = await lookupEntries(rtcId, store);
    for (let i = 0; i < entries.length; i++) {
        const value = await entries[i];
        if (value) {
    ...
    if (payload.d && payload.p) {
        ...
        if (shouldSave) {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                if (saved) {
                    ...
                    context.waitUntil(store.put(`rtcs/${rtcId}/entries:${i}`, JSON.stringify(newEntry), putOptions));
                    entries[i] = newEntry;
                    saved = true;
                }
            }
            ...
            if (!saved) {
                ...
                if (!saved) {
                    ...
                    context.waitUntil(store.put(`rtcs/${rtcId}/entries:${entries.length - 1}`, JSON.stringify(newEntry), putOptions));
                }
            }
        }
        for (let i = entries.length - 1; i >= 0; i--) {
            ...
            if (maxIndex < i) {
                context.waitUntil(store.put(`rtc/${rtcId}/max_index`, `${i}`, {
                    customMetadata: { expireAt: now + 8 * 60 * 60 * 1000 },
                }));
                break;
            }
        ...
        context.waitUntil(new Promise((res) => {
            setTimeout(async () => {
                ...
                if (!nextVacuumEntry ||
                    ...
                    let removed = 0;
                    // Vacuum
                    await store.put(`rtcs/${rtcId}/next_vacuum`, `${now + 30 * 1000}`); // One mintue rtc vacuum interval
                    const list = await store.list({
                        include: ["customMetadata"],
                        prefix: `rtcs/${rtcId}/`,
                    });
                    const removePromises = [];
                    for (const { key, customMetadata: { expireAt }, } of list.objects) {
...
async function getResponseIfDisallowed(request, env) {
    ...
    if (Math.random() < 1.0 / RATE_LIMITING_SAMPLING_RATE) {
        await store.put(currentCountKey, (currentCount + Math.floor(RATE_LIMITING_SAMPLING_RATE)).toString());
    }
}
exports.default = {
    async fetch(request, env, context) {
        ...
        if (request.headers.get("x-worker-method") === "DELETE" ||
