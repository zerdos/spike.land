...
class Code {
    ...
    async fetch(request, env, ctx) {
        ...
        if (!this.sessionStarted) {
            (0, session_1.startSession)(this.codeSpace, { name: this.codeSpace, state }, url.origin);
        ...
        return (0, handleErrors_1.handleErrors)(request, async () => {
            ...
            switch (path[0]) {
                ...
                case "lazy":
                    ...
                    return new Response(`import { jsx as jsX } from "@emotion/react";
           ...
           `, {
              ...
              export const importmapReplaced=${JSON.stringify({
                ...
                case "public": {
                    const a = JSON.parse(__STATIC_CONTENT_MANIFEST_1.default);
                    const respText = index_html_1.default.replaceAll("/live/coder/", `/live/${this.codeSpace}/`).replace(`/* #root{} */`, `
        #root{
          height: 100%; 
        }
        ${(0, session_1.mST)().css}
    ...
    async handleSession(webSocket, ip) {
        ...
        let limiter = new rateLimiterClient_1.RateLimiterClient(() => this.env.LIMITERS.get(limiterId), (err) => webSocket.close(1011, err.stack));
        let session = {
            name: "",
            webSocket,
            limiter,
...
function importMapReplace(codeInp) {
    ...
    items.map((lib) => {
        returnStr = returnStr.replaceAll(` from "${lib}"`, `from "${chat_1.imap.imports[lib]}"`);
    });
    return returnStr;
}
