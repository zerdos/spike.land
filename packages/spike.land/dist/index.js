...
function maxCompare(comparator, a2, b) {
  ...
  return comp === 0 && b !== a2 && (b === void 0 || b === null || b !== b) ||
    comp > 0;
}
...
function deepEqual(a2, b) {
  ...
  if (
    !isCollection(b) ||
    a2.size !== void 0 && b.size !== void 0 && a2.size !== b.size ||
    a2.__hash !== void 0 && b.__hash !== void 0 && a2.__hash !== b.__hash ||
    isKeyed(a2) !== isKeyed(b) || isIndexed(a2) !== isIndexed(b) ||
    isOrdered(a2) !== isOrdered(b)
  ) {
    return false;
  }
  ...
  var bSize = b.__iterate(function (v, k) {
    if (
      notAssociative
        ? !a2.has(v)
        : flipped
        ? !is(v, a2.get(k, NOT_SET))
        : !is(a2.get(k, NOT_SET), v)
    ) {
      allEqual = false;
      return false;
    }
...
function initSession(room, u) {
  return Record(
    __spreadProps(__spreadValues({}, u), { room, state: Record(u.state)() }),
  );
}
var session = null;
var hashStore = {};
var CodeSession = class {
  constructor(room, user) {
    ...
    __publicField(this, "applyPatch", async ({
      ...
      if (
        ...
        if (resp.ok) {
          ...
          const { mST: mST2 } = await Promise.resolve().then(() =>
            __toESM2(__require2(`/live/${this.room}/mst.mjs?${Date.now()}`))
          );
    ...
    this.session = initSession(
      room,
      __spreadProps(__spreadValues({}, user), {
