...
// ../code/dist/chunk-chunk-I52D4BQZ.mjs
init_define_process();
  ...
  fetch: async (req, env) => {
    ...
    return handleErrors(request, async () => {
      ...
      const handleFetchApi = async (path2) => {
        ...
        if (path2[0] && path2[0].startsWith("npm:") || path2[0].startsWith("node_modules/")) {
          ...
          if (isText && response.url.indexOf(".d.ts") !== -1) {
            ...
            const baSe = new URL(".", url2).toString();
            ...
            replaced = replaceAll2(replaced, `import("../`, ` import("${parent}`);
            replaced = replaceAll2(replaced, `import("./`, ` import("${baSe}`);
            replaced = replaceAll2(replaced, `import('../../../`, ` import('${ggParent}`);
            replaced = replaceAll2(replaced, `import('../../`, ` import('${gParent}`);
            replaced = replaceAll2(replaced, `import('../`, ` import('${parent}`);
            replaced = replaceAll2(replaced, `import('./`, ` import('${baSe}`);
            replaced = replaceAll2(replaced, ` from '../../`, ` from '${gParent}`);
            replaced = replaceAll2(replaced, ` from "../../`, ` from "${gParent}`);
            replaced = replaceAll2(replaced, ` from '../`, ` from '${parent}`);
            replaced = replaceAll2(replaced, ` from './`, ` from '${baSe}`);
            replaced = replaceAll2(replaced, ` from "../`, ` from "${parent}`);
            bodyStr = replaceAll2(replaced, ` from "./`, ` from "${baSe}`);
          }
        ...
        if (path2[0] && path2[0].startsWith("unpkg:")) {
        ...
        if (path2[0] && path2[0].startsWith("node_modules")) {
          const esmUrl = u.toString().replace(
            u.origin + "/node_modules/",
            "https://unpkg.com/"
          );
        ...
        switch (path2[0]) {
          ...
          default:
            try {
              ...
              const req2 = new Request(`${u.origin}/npm:${u.pathname}`, request);
      ...
      root.render(<ErrorBoundary
        ...
        case "public": {
          const respText = src_default.replace(
            ...
            `<script type="importmap"><\/script>`,
            `<script type="importmap">${JSON.stringify(importmap_default)}<\/script>`
          ).replace(
            `<div id="root"></div>`,
...
function importMapReplace(codeInp, origin) {
  const items = Object.keys(
    importmap_default.imports
  );
  let returnStr = codeInp;
  items.map((lib) => {
    const uri = importmap_default.imports[lib].slice(1);
