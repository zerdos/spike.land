...
describe("Code Durable Object", () => {
  ...
  const mockR2Object = (textData: string) => ({
    ...
    arrayBuffer: vi.fn().mockResolvedValue(
      new TextEncoder().encode(textData).buffer,
    ),
    blob: vi.fn(),
    body: null,
    bodyUsed: false,
  ...
  beforeEach(() => {
    ...
    (RouteHandler as ReturnType<typeof vi.fn>).mockImplementation(() => ({
      handleRoute: vi.fn().mockResolvedValue(
        new Response("OK", { status: 200 }),
      ), // Ensure fetch can complete
    }));
  ...
  describe("initializeSession", () => {
    it("should initialize and save a new session if none exists (e.g., 'x' room)", async () => {
      ...
      (mockState.storage.get as ReturnType<typeof vi.fn>).mockResolvedValue(
        undefined,
      ); // No session_core
    ...
    it("should initialize by fetching a backup session if specified and none exists", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        `https://example.com/live/backupRoom/session.json`,
      );
      expect(mockState.storage.put).toHaveBeenCalledWith(
        "session_core",
    ...
    it("should load an existing session from storage", async () => {
      ...
      expect(mockState.storage.put).toHaveBeenCalledWith("session_core", {
        codeSpace: roomName,
      });
      expect(mockState.storage.put).toHaveBeenCalledWith(
        "session_code",
  ...
  describe("updateAndBroadcastSession", () => {
    ...
    beforeEach(async () => {
      ...
      await codeInstance.fetch(
        new Request(`https://example.com/?room=${roomName}`),
      );
      ...
      (codeInstance as unknown as { session: unknown; initialized: boolean; })
