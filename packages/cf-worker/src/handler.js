...
function log(message, data) {
    if (data === void 0) { data = {}; }
    now = now || Date.now();
    var _a = new Date().toLocaleTimeString("en-US").split(/:| /), hour = _a[0], minute = _a[1];
    return LOGS.put(String(2000000000000 - now++), JSON.stringify({ message: message, time: hour + ":" + minute, data: data }), { expirationTtl: 86400 * 7 });
}
exports.log = log;
function handleCloudRequest(request) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, country, colo, url, searchParams, pathname, psk, key, tokenKey, userIdKey, pass, tokenPassUserId, checkTokenPassUserId, uuid, tokenUuid, checkPass, checkPassToken, key_1, waitForChange, data, uuid, uuidHash, uuid, uuidHash, uuidHash, uuid, maybeRoute, shaDB, result, zkey, sha, uKey, gKey, proofKey, checkGkey, myBuffer, hash, smallerKey, uuid, checkProofKey, maybeRoute, shaDB, result;
        var _this = this;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _a = request.cf || { country: "", colo: "" }, country = _a.country, colo = _a.colo;
                    url = new URL(request.url);
                    searchParams = url.searchParams, pathname = url.pathname;
                    psk = String(request.headers.get("API_KEY") || "");
                    log("request", { searchParams: searchParams, pathname: pathname, country: country, colo: colo });
                    if (!(request.method === "GET" && psk && psk == API_KEY)) return [3 /*break*/, 1];
                    return [2 /*return*/, admin_ts_1.handleAdmin(request, searchParams, pathname, SHAKV)];
                case 1:
                    if (!(request.method === "GET")) return [3 /*break*/, 26];
                    if (pathname === "/robots.txt") {
                        return [2 /*return*/, handleOptions_ts_1.text("User-agent: * Disallow: /")];
                    }
                ...
                case 2:
                    ...
                    if (!tokenKey || !userIdKey || !pass ||
                        (checkTokenPassUserId !== tokenPassUserId)) {
                        return [2 /*return*/, handleOptions_ts_1.json({ error: "auth error" })];
                    }
                ...
                case 4:
                    tokenUuid = _b.sent();
                    if (tokenUuid === null) {
                        return [2 /*return*/, handleOptions_ts_1.json({
                                error: 404,
                ...
                case 6:
                    ...
                    return [4 /*yield*/, USERS.put(tokenUuid, JSON.stringify({
                            uuid: uuid,
                            connected: userIdKey
                        }), { expirationTtl: 60 })];
                case 7:
                ...
                case 8:
                    ...
                    _b.label = 9;
                case 9: return [2 /*return*/, handleOptions_ts_1.json({ error: 401 })];
                case 10:
                    if (!(pathname === "/check")) return [3 /*break*/, 12];
                    key_1 = searchParams.get("key");
                    if (key_1 === null)
                        return [2 /*return*/, new Response("500")];
                    waitForChange = function () { return __awaiter(_this, void 0, void 0, function () {
                        var uuid, data;
                        var _this = this;
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0: return [4 /*yield*/, USERKEYS.get(key_1)];
                                case 1:
                                    uuid = _a.sent();
                                    if (!uuid)
                                ...
                                case 2:
                                    ...
                                    return [2 /*return*/, new Promise(function (resolve) {
                                            var clear = setInterval(function () { return __awaiter(_this, void 0, void 0, function () {
                                                var data;
                                                return __generator(this, function (_a) {
                                                    switch (_a.label) {
                                                        case 0: return [4 /*yield*/, USERS.get(uuid, "json")];
                                                        case 1:
                                                            data = _a.sent();
                                                            if (!data || data.connected) {
                                                                clearInterval(clear);
                ...
                case 11:
                    data = _b.sent();
                    return [2 /*return*/, handleOptions_ts_1.json({ expired: data === null })];
                case 12:
                ...
                case 13:
                    uuidHash = _b.sent();
                    return [4 /*yield*/, USERS.put(uuid, JSON.stringify({ uuid: uuid, uuidHash: uuidHash, registered: Date.now(), country: country, colo: colo }))];
                case 14:
                    _b.sent();
                ...
                case 18:
                    uuidHash = _b.sent();
                    return [4 /*yield*/, USERS.put(uuid, JSON.stringify({ uuid: uuid, registered: Date.now(), country: country, colo: colo }), { expirationTtl: 60 })];
                case 19:
                    _b.sent();
                    return [4 /*yield*/, USERKEYS.put(uuidHash, uuid, { expirationTtl: 60 })];
                case 20:
                    _b.sent();
                    return [2 /*return*/, handleOptions_ts_1.json({ uuid: uuid, key: uuidHash })];
                case 21:
                    if (!(pathname === "/create-project")) return [3 /*break*/, 23];
                    uuidHash = request.headers.get("TOKEN");
                    uuid = dec_ts_1.v4();
                    return [4 /*yield*/, USERS.put(uuid, JSON.stringify({ uuid: uuid, registered: Date.now(), country: country, colo: colo }))];
                case 22:
                    _b.sent();
                    return [2 /*return*/, handleOptions_ts_1.json({ uuid: uuid })];
                case 23:
                ...
                case 24:
                    ...
                    _b.label = 25;
                case 25: return [2 /*return*/, Response.redirect("https://zed.vision/code", 301)];
                case 26:
                    ...
                    if (!sha || !uKey || !gKey || !proofKey) {
                        return [2 /*return*/, handleOptions_ts_1.json({ error: 401, "message": "not matching keys" })];
                    }
                    return [4 /*yield*/, sha256_js_1.sha256(sha + uKey)];
                case 27:
                    checkGkey = _b.sent();
                    if (checkGkey !== gKey) {
                        return [2 /*return*/, handleOptions_ts_1.json({ error: 401, "message": "content and userkeys are not a pain" })];
                    }
                ...
                case 29:
                    ...
                    if (smallerKey !== sha) {
                        return [2 /*return*/, handleOptions_ts_1.json({
                                error: 401,
                                message: "body hash not matching with the sent hash: " + smallerKey + " -- " + zkey
                            })];
                    }
                ...
                case 30:
                    uuid = _b.sent();
                    if (!uuid)
                        return [2 /*return*/, handleOptions_ts_1.json({ error: 500, message: "user not found" })];
                    return [4 /*yield*/, sha256_js_1.sha256(sha + uuid)];
                case 31:
                    checkProofKey = _b.sent();
                    if (checkProofKey !== proofKey) {
                        return [2 /*return*/, handleOptions_ts_1.json({ error: 401, message: "user not verified" })];
                    }
                ...
                case 34:
                    result = _b.sent();
                    _b.label = 35;
                case 35: return [2 /*return*/, handleOptions_ts_1.json({
                        hash: smallerKey
                    })];
                case 36: return [2 /*return*/, new Response("404")];
            }
        });
    });
}
