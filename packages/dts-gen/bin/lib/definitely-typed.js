...
function writeDefinitelyTypedPackage(indexDtsContent, packageName, overwrite) {
  ...
  if (!overwrite) {
    if (fs_1.existsSync(packageDir)) {
      console.log(
        `Directory ${packageDir} already exists and --overwrite was not specified; exiting.`,
      );
      process.exit(2);
    }
  }
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      const reg = JSON.parse(
        yield loadString(`http://registry.npmjs.org/${packageName}`),
      );
      ...
      console.warn(
        `Warning: Could not retrieve version/homepage information: ${e.message}`,
      );
    }
    ...
    try {
      const repoGitConfig = parseGitConfig.sync(
        { path: path_1.join(".git", "config") },
      );
      if (
        repoGitConfig['remote "origin"'] && repoGitConfig['remote "origin"'].url
      ) {
        ...
        return reject(
          new Error(
            `HTTP Error ${res.statusCode}: ${
              http_1.STATUS_CODES[res.statusCode || 500]
            } for ${url}`,
          ),
        );
      }
