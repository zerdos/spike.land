  ...
  if (typeof value === "object") {
    ...
    if (isClasslike(value)) {
      return ValueTypes.Class |
        (hasCloduleProperties(value) ? ValueTypes.Object : ValueTypes.None);
    } else {
      return ValueTypes.Function |
        (hasFunduleProperties(value) ? ValueTypes.Object : ValueTypes.None);
    }
  } else {
...
export function generateIdentifierDeclarationFile(
  name: string,
  value: any,
): string {
            ...
            switch (p.kind) {
              case "const":
                primaryDecl.members.push(
                  create.property(p.name, p.type, dom.DeclarationFlags.Static),
                );
                break;
              case "function":
                primaryDecl.members.push(
  ...
  if (isNativeFunction(obj)) {
    ...
    if (fn.parameters) {
      params = fn.parameters.map((p) =>
        create.parameter(`${p.name.getText()}`, inferParameterType(fn, p))
      );
      if (usedArguments) {
        params.push(
          create.parameter(
...
function inferParameterType(
  _fn: ts.FunctionExpression,
  _param: ts.ParameterDeclaration,
): dom.Type {
