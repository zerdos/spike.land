...
try {
  ...
  if (args.module) {
    ...
    name = path.basename(filename, path.extname(filename)).replace(
      /[^A-Za-z0-9]/g,
      "_",
    );
    (module as any).paths.unshift(process.cwd() + "/node_modules");
  ...
  if (args.dt) {
    ...
    let filename = typeof args.file === "boolean" || args.file === undefined
      ? name + ".d.ts"
      : args.file;
...
function allTemplateNames(): string {
  return fs.readdirSync(templatesDirectory).map((t) =>
    t.slice(0, t.length - ".d.ts".length)
  ).join(", ");
}
