...
function generateModuleDeclarationFile(nameHint, root) {
    ...
    if (decls.length === 1 && decls[0].kind === "namespace") {
        ...
        return members.map(function (m) { return dom.emit(m); }).join("");
    }
    else {
        // Going to have to write an export=
        var result = decls.map(function (d) { return dom.emit(d); });
...
function getKeysOfObject(obj) {
    ...
    keys = keys.filter(function (s) { return isVisitableName(s); });
...
function getTopLevelDeclarations(name, obj) {
    ...
    function getResult() {
        if (typeof obj === "function") {
            ...
            for (var _i = 0, keys_1 = keys; _i < keys_1.length; _i++) {
                ...
                getTopLevelDeclarations(k, obj[k]).forEach(function (p) {
                    if (primaryDecl_1.kind === "class") {
                        ...
                        switch (p.kind) {
                            case "const":
                                primaryDecl_1.members.push(dts_dom_1.create.property(p.name, p.type, dom.DeclarationFlags.Static));
                                break;
                            case "function":
                                primaryDecl_1.members.push(dts_dom_1.create.method(p.name, p.parameters, p.returnType, dom.DeclarationFlags.Static));
                                break;
                            default:
                                ns_1.members.push(p);
                                break;
            ...
            if (typeof simpleType === "string" || simpleType.kind === "name" ||
                simpleType.kind === "array") {
                var result = dom.create["const"](name, simpleType);
                if (simpleType === "string") {
                    var preview = "\"" + simpleType.substr(0, 100) + (simpleType.length > 100 ? "..." : "") + "\"";
...
function getClassPrototypeMembers(ctor) {
    ...
    var members = names
        .filter(function (n) { return !isNameToSkip(n); })
        .map(function (name) {
        return getPrototypeMember(name, Object.getOwnPropertyDescriptor(ctor.prototype, name).value);
    })
...
function getParameterListAndReturnType(obj, fn) {
    ...
    if (isNativeFunction(obj)) {
        ...
        var params = [
            dts_dom_1.create.parameter("args", dom.type.array(dom.type.any), dom.ParameterFlags.Rest),
        ];
        if (fn.parameters) {
            params = fn.parameters.map(function (p) {
                return dts_dom_1.create.parameter("" + p.name.getText(), inferParameterType(fn, p));
            });
            if (usedArguments) {
    ...
    function visit(node) {
        switch (node.kind) {
            ...
            case ts.SyntaxKind.ReturnStatement:
                var ret = node;
                if (funcStack.length === 0 && ret.expression &&
