...
function createMotionComponent({ preloadedFeatures, createVisualElement, projectionNodeConstructor, useRender, useVisualState: useVisualState2, Component }) {
  ...
  function MotionComponent(props, externalRef) {
    const configAndProps = __spreadProps(__spreadValues(__spreadValues({}, q(MotionConfigContext)), props), {
    ...
    loadFeatures(renderedProps, isStrict, preloadedFeatures, projectionId, ProjectionNodeConstructor, initialLayoutGroupConfig) {
      ...
      for (let i = 0; i < numFeatures; i++) {
        ...
        if (isEnabled(renderedProps) && Component) {
          features.push(h(Component, __spreadProps(__spreadValues({
            key: name
          }, renderedProps), {
...
function MeasureLayout(props) {
  ...
  return src_default.createElement(MeasureLayoutWithContext, __spreadProps(__spreadValues({}, props), { layoutGroup, switchLayoutGroup: q(SwitchLayoutGroupContext), isPresent, safeToRemove }));
}
...
function useForceUpdate() {
  ...
  const [forcedRenderCount, setForcedRenderCount] = p(0);
  const forceRender = T(() => {
...
var LayoutGroup = ({ children, id: id3, inheritId, inherit = true }) => {
  ...
  const memoizedContext = F(() => __spreadProps(__spreadValues({}, context.current), { forceRender }), [key]);
...
function LazyMotion({ children, features, strict = false }) {
  const [, setIsLoaded] = p(!isLazyBundle(features));
...
function ReorderItem(_a, externalRef) {
  ...
  return h(Component, __spreadProps(__spreadValues({ drag: axis }, props), { dragSnapToOrigin: true, style: __spreadProps(__spreadValues({}, style2), { x: point.x, y: point.y, zIndex }), layout, onDrag: (event, gesturePoint) => {
