...
function mST(p) {
  ...
  const { i, transpiled, code, html, css: css2 } = p ? JSON.parse(
    applyPatch(
      string_(
        sessAsJs
      ),
  ...
  return { i, transpiled, code, html, css: css2 };
}
function string_(s) {
  const { i, transpiled, code, html, css: css2 } = s;
  return JSON.stringify({ i, transpiled, code, html, css: css2 });
}
...
(async () => {
  const { ReactDOMClient: ReactDOMClient2 } = await import("./chunk-react-jsx-runtime-PHSOVGZE.mjs");
  const { hydrateRoot, createRoot } = ReactDOMClient2;
  ...
  if (location.pathname.endsWith("/hydrated")) {
    ...
    bc.onmessage = (event) => {
      if (event.data.codeSpace === codeSpace) {
        ...
        ).then((mod2) => i === current && root.render(mod2.default())).then(
    ...
    importShim(
      `${location.origin}/live/${codeSpace}/index.js/${i}`
    ).then((mod2) => hydrateRoot(document.querySelectorAll("#root>div>div")[0], mod2.default()));
  } else {
    const dry = false;
    start(dry);
  }
