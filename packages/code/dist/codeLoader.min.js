...
`,u=window.document;let k=!0;const{motion:I}=window.Motion;let m="",p=0,B=0,j="",S="",g="";export async function run(){C(I),await b("https://unpkg.com/@babel/standalone@7.12.7/babel.min.js"),b("https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.4.4/qrcode.min.js"),(async()=>{const o=a();g=o;const t=await T({language:"typescript",code:o,onChange:y});function y(e){if(!t)return;if(m=e,!p)l(m);else{const n=e,s=setInterval(()=>{(e!==m||!p)&&clearInterval(s),p||l(m)},100)}}async function d(){if(!t||!t.monaco)return;const e=t.monaco.Uri.parse("file:///main.tsx"),n=await t.monaco.languages.typescript.getTypeScriptWorker(),s=await(await n(e)).getSemanticDiagnostics("file:///main.tsx"),r=await(await n(e)).getCompilerOptionsDiagnostics("file:///main.tsx"),c=await(await n(e)).getSyntacticDiagnostics("file:///main.tsx");return[...s,...r,...c]}async function l(e){const{diff:n}=await import("../dist/diff.min.js");if(p===1)return;try{p=1;const s=await d(),r=u.getElementById("error");if(p=0,e!==m)return;if(s&&s.length){if(m!=e)return;if(j===e)return;const c=n(g,e,0);if(c.length<=3){t.monaco.editor.setTheme("hc-black");return}r.innerHTML=s[0].messageText.toString(),r.style.display="block",j=e;return}g=e,r.style.display="none",t.monaco.editor.setTheme("vs-dark"),B=0,p=0,i(E(e))}catch(s){if(p=0,e!==m)return;t.monaco.editor.setTheme("vs-light"),setTimeout(()=>{t.monaco.editor.setTheme("hc-black")},50),console.error(s)}}})(),i(E(a()));async function i(o){const t=/import/gi,y="///",d=o.replaceAll(/import/gi,"///").replace("export default","DefaultElement = "),l=async()=>{const e=new Function("code",`return function(){  
          let DefaultElement;
        
        ${d}

                return ReactDOM.render(jsx(DefaultElement), document.getElementById("root"));
      }`)();e()};if(!k){const e=async n=>{if(!location.origin.includes("zed.vision"))return;if(n!==g)return;if(S===n)return;S=n;const s={codeTranspiled:o,code:g},r=JSON.stringify(s),c=new Request("https://code.zed.vision",{body:r,method:"POST",headers:{"content-type":"application/json;charset=UTF-8"}}),x=await fetch(c),{hash:w}=await x.json();try{const f=window.localStorage,v=f.getItem("codeBoXHash2");v!==w&&(f.setItem("codeBoXHash2",w),f.setItem(w,g),R("h",w))}catch(f){}};setTimeout(()=>e(m),500)}k=!1,l()}function a(){const o=new URLSearchParams(window.location.search),t=o.get("h")||localStorage.getItem("codeBoXHash2");return t&&window.localStorage.getItem(t)||window.localStorage.getItem("STARTER")||D}}function R(i,a){const o=new URLSearchParams(window.location.search);o.set(i,a),window.history.replaceState({},"",decodeURIComponent(`${window.location.pathname}?${o}`))}function E(i){const{transform:a}=window.Babel;return a(`/** @jsx jsx */
