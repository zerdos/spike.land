const v=(i,a)=>{const n=()=>jsx(React.Fragment,null,jsx(i.div,{css:`
    ...
    `},jsx("button",{css:buttonStyles("red"),onClick:()=>a()},"SHARE")),jsx("div",{css:`  
...
`,u=window.document;let k=!0;const{motion:I}=window.Motion;let m="",p=0,R=0,S="",j="",g="";export async function run(){v(I,()=>console.log("Sharing it")),await b("https://unpkg.com/@babel/standalone@7.12.7/babel.min.js"),b("https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.4.4/qrcode.min.js"),(async()=>{const n=a();g=n;const t=await T({language:"typescript",code:n,onChange:y});function y(e){if(!t)return;if(m=e,!p)l(m);else{const o=e,s=setInterval(()=>{(e!==m||!p)&&clearInterval(s),p||l(m)},100)}}async function c(){if(!t||!t.monaco)return;const e=t.monaco.Uri.parse("file:///main.tsx"),o=await t.monaco.languages.typescript.getTypeScriptWorker(),s=await(await o(e)).getSemanticDiagnostics("file:///main.tsx"),r=await(await o(e)).getCompilerOptionsDiagnostics("file:///main.tsx"),d=await(await o(e)).getSyntacticDiagnostics("file:///main.tsx");return[...s,...r,...d]}async function l(e){const{diff:o}=await import("../dist/diff.min.js");if(p===1)return;try{p=1;const s=await c(),r=u.getElementById("error");if(p=0,e!==m)return;if(s&&s.length){if(m!=e)return;if(S===e)return;const d=o(g,e,0);if(d.length<=3){t.monaco.editor.setTheme("hc-black");return}r.innerHTML=s[0].messageText.toString(),r.style.display="block",S=e;return}g=e,r.style.display="none",t.monaco.editor.setTheme("vs-dark"),R=0,p=0,i(E(e))}catch(s){if(p=0,e!==m)return;t.monaco.editor.setTheme("vs-light"),setTimeout(()=>{t.monaco.editor.setTheme("hc-black")},50),console.error(s)}}})(),i(E(a()));async function i(n){const t=/import/gi,y="///",c=n.replaceAll(/import/gi,"///").replace("export default","DefaultElement = ");console.log(c);const l=async()=>{const e=new Function("code",`return function(){  
          let DefaultElement;
        
        ${c}

                return ReactDOM.render(jsx(DefaultElement), document.getElementById("root"));
      }`)();e()};if(!k){const e=async o=>{if(!location.origin.includes("zed.vision"))return;if(o!==g)return;if(j===o)return;j=o;const s={codeTranspiled:n,code:g},r=JSON.stringify(s),d=new Request("https://code.zed.vision",{body:r,method:"POST",headers:{"content-type":"application/json;charset=UTF-8"}}),x=await fetch(d),{hash:w}=await x.json();try{const f=window.localStorage,C=f.getItem("codeBoXHash2");C!==w&&(f.setItem("codeBoXHash2",w),f.setItem(w,g),B("h",w))}catch(f){}};setTimeout(()=>e(m),500)}k=!1,l()}function a(){const n=new URLSearchParams(window.location.search),t=n.get("h")||localStorage.getItem("codeBoXHash2");return t&&window.localStorage.getItem(t)||window.localStorage.getItem("STARTER")||D}}function B(i,a){const n=new URLSearchParams(window.location.search);n.set(i,a),window.history.replaceState({},"",decodeURIComponent(`${window.location.pathname}?${n}`))}function E(i){const{transform:a}=window.Babel;return a(`/** @jsx jsx */
