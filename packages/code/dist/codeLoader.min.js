const C = async (m) =>
    new Promise((l) => {
      const i = window.JSFrame,
        s = new i({ horizontalAlign: "left", verticalAlign: "top" }),
        r = s.create(
          {
            name: "Win2",
            title: "Z",
            left: (window.innerWidth - window.innerWidth * .7) / 2,
            top: 20,
            ...
            appearanceParam: {
              ...
              titleBar: {
                ...
                fontWeight: "bolder",
                buttons: [
                  { fa: "fas fa-times", name: "closeButton", visible: !0 },
                  {
                ...
                buttonsOnLeft: [
                  {
                    ...
                    visible: !0,
                    childMenuHTML:
                      '<div class="list-group">  <div name="menu1" class="list-group-item list-group-item-action py-2">Menu Item 01</div>  <div name="menu2" class="list-group-item list-group-item-action py-2">Menu Item 02</div>  <div name="menu3" class="list-group-item list-group-item-action py-2">Menu Item 03</div></div>',
                    childMenuWidth: 300,
                  },
                ],
              },
      ...
      r.setControl(
        {
          ...
          maximizeWithoutTitleBar: !0,
          restoreKey: "Escape",
        },
      ),
        r.control.on("hid", (e, d) => {
          e.closeFrame();
        }),
        r.control.on("maximized", (e, d) => {
          s.showToast({ text: 'Press "ESC" to minimize.', align: "center" });
        }),
        r.control.on("demaximized", (e, d) => {}),
        r.on("menu", "click", (e, d, c) => {
          const o = d.target.getAttribute("name");
          o && o.startsWith("menu") && alert(o + " clicked");
        }),
        l();
    }),
  M = async ({ onChange: m, code: l, language: i }) => {
    const s = window.document.getElementById("container");
    if (!s) {
      const o = h.getElementById("container");
      o.id = "container", h.body.appendChild(o);
    }
    const r = i === "typescript" ? "file:///main.tsx" : "file:///main.html";
    let e;
    if (
      ...
      o.id = "ace",
        ...
        await A("https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.12/ace.min.js"),
        i === "typescript"
          ? await A(
        ...
        e = window.ace.edit("ace"),
        e.getSession().setMode("ace/mode/typescript");
      const t = (a) =>
        setTimeout(() => {
          let n = window.ace.edit("ace"), u = n.getTheme();
          u !== "ace/theme/monokai " &&
            (n.setOptions({ fontSize: "14pt" }),
              n.setTheme("ace/theme/monokai"),
              t(2 * a));
        }, a);
      t(100), e.setValue(l), e.blur();
    }
    if (window.monaco === void 0) {
      ...
      t.config(
        ...
        await new Promise((a) =>
          t(["vs/editor/editor.main"], (n) => {
            a(n);
          })
        );
    }
    const d = window.monaco,
      c = {
        monaco: d,
        editor: d.editor.create(
          window.document.getElementById("container"),
          {
            ...
            model: d.editor.getModel(r) ||
              d.editor.createModel(l, i, d.Uri.parse(r)),
            value: l,
            language: i,
            theme: "vs-dark",
          },
    ...
    if (
      c.editor.onDidChangeModelContent(() => m(c.editor.getValue())),
        e && e.session.on("change", function () {
          const o = e.getValue();
          c.editor.setValue(o), m(o);
        }),
        e && h.getElementById("container").replaceWith(h.getElementById("ace")),
        c.monaco.languages.typescript.typescriptDefaults.setDiagnosticsOptions(
          {
      ...
      const o = [
        ...
        t = o.map(({ name: a, url: n }) =>
          (async () =>
            c.monaco.languages.typescript.typescriptDefaults.addExtraLib(
              await (await fetch(n)).text(),
              a.includes("@emotion")
                ? `file:///node_modules/${a}`
                : `file:///node_modules/@types/${a}/index.d.ts`,
            ))()
        );
      return c.monaco.languages.typescript.typescriptDefaults
        .setCompilerOptions(
          {
            target: c.monaco.languages.typescript.ScriptTarget.ESNext,
            ...
            moduleResolution:
              c.monaco.languages.typescript.ModuleResolutionKind.NodeJs,
            module: c.monaco.languages.typescript.ModuleKind.CommonJS,
            noEmit: !0,
            typeRoots: ["node_modules/@types"],
            jsx: c.monaco.languages.typescript.JsxEmit.React,
...
const b = async (m) =>
    h.querySelector(`script[src="${m}"]`) || new Promise(function (l, i) {
...
export async function run() {
  ...
  const m = b("https://unpkg.com/@ampproject/worker-dom@0.27.4/dist/main.js");
  await b("https://unpkg.com/@babel/standalone@7.12.6/babel.min.js"),
    (async () => {
      const r = i();
      f = r;
      const e = await M({ language: "typescript", code: r, onChange: d });
      function d(t) {
        if (!e) return;
        if (y = t, !w) o(y);
        else {
          const a = t,
            n = setInterval(() => {
              (t !== y || !w) && clearInterval(n), w || o(y);
            }, 100);
        }
      }
      async function c() {
        if (!e || !e.monaco) return;
        const t = e.monaco.Uri.parse("file:///main.tsx"),
          a = await e.monaco.languages.typescript.getTypeScriptWorker(),
          n = await (await a(t)).getSemanticDiagnostics("file:///main.tsx"),
          u = await (await a(t)).getCompilerOptionsDiagnostics(
            "file:///main.tsx",
          ),
          p = await (await a(t)).getSyntacticDiagnostics("file:///main.tsx");
        return [...n, ...u, ...p];
      }
      async function o(t) {
        const { diff: a } = await import("../dist/diff.min.js");
        if (w === 1) return;
        try {
          w = 1;
          const n = await c(), u = h.getElementById("error");
          if (w = 0, t !== y) return;
          if (n && n.length) {
            if (y != t) return;
            if (v === t) return;
            const p = a(f, t, 0);
            if (p.length <= 3) {
              e.monaco.editor.setTheme("hc-black");
              return;
            }
            u.innerHTML = n[0].messageText.toString(),
              u.style.display = "block",
              v = t,
              e.monaco.editor.setTheme("vs-light"),
              setTimeout(() => {
                e.monaco.editor.setTheme("hc-black");
              }, T++);
            return;
          ...
          f = t,
            ...
            e.monaco.editor.setTheme("vs-dark"),
            T = 0,
            w = 0,
            l(s(t));
        } catch (n) {
  ...
  async function l(r) {
    const e = /import/gi,
      d = "///",
      c = r.replaceAll(/import/gi, "///").replace(
        "export default",
        "DefaultElement = ",
      ),
      o = async () => {
        ...
        console.log(DefaultElement);
          ...
          a = t(),
          n = Array.from(
            h.querySelector("head > style[data-emotion=css]").sheet.cssRules,
          ).map((g) => g.cssText).filter((g) => a.includes(g.substring(3, 8)))
            .join(`
  `);
        console.log(n);
     ...
     const styled = window["emotionStyled"];
      ...
      `,
          p = z(u),
          x = h.getElementsByTagName("iframe").item(0);
        if (x) {
          const g = h.createElement("iframe");
          g.setAttribute("src", p),
            g.onload = () => console.log("Loooooooaaaded"),
            setTimeout(() => {
              window.requestAnimationFrame(() => {
                x.setAttribute("src", p), g.remove();
              });
            }, 30);
        } else await C(p);
      };
    if (!S) {
      const t = async (n) => {
          ...
          if (j === n) return;
          j = n;
          const u = { codeTranspiled: r, code: f },
            p = JSON.stringify(u),
            x = new Request(
            ...
            { hash: k } = await g.json();
          try {
            const E = window.localStorage, B = E.getItem("codeBoXHash");
            B !== k &&
              (E.setItem("codeBoXHash", k), E.setItem(k, f), I("h", k));
          } catch (E) {
  ...
  function i() {
    const r = new URLSearchParams(window.location.search),
      e = r.get("h") || localStorage.getItem("codeBoXHash");
    return e && window.localStorage.getItem(e) ||
      window.localStorage.getItem("STARTER") || D;
  }
  function s(r) {
...
function I(m, l) {
  ...
  i.set(m, l),
    window.history.replaceState(
      {},
      "",
      decodeURIComponent(`${window.location.pathname}?${i}`),
    );
}
function z(m) {
  const l = new Blob([m], { type: "text/html" }),
