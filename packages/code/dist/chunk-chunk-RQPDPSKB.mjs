...
var AutoUpdateApp = ({ hash, codeSpace: codeSpace2 }) => {
  ...
  useEffect(() => {
    ...
    if (html && css7 && (html !== mst.html || css7 !== mst.css)) {
      patchSync(__spreadProps(__spreadValues({}, mst), { html, css: css7 }));
    }
  }, [hash]);
...
var Editor = ({ code, i, codeSpace: codeSpace2, assets }) => {
  ...
  const [
    ...
    runner: async ({ code: code2, counter: counter2, codeSpace: codeSpace3 }) => {
      const { runner: runner2 } = await import("./chunk-runner-X7B6YPI3.mjs");
  ...
  useEffect(() => {
    ...
    const setMonaco = async () => {
      ...
      const { startMonaco } = await import("./chunk-startMonaco-4L7HWFTN.mjs");
    ...
    const setAce = async () => {
      const { startAce } = await import("./chunk-startAce-GGAADH76.mjs");
    ...
    const handler = setInterval(() => {
      ...
      if (getValue() !== lastCode) {
        ...
        if (code2 === mST().code || code2 === mod.code)
          return;
        changeContent((x) => __spreadProps(__spreadValues({}, x), { myCode: code2, i: i + 1 }));
  ...
  useEffect(() => {
    if (!started)
      return;
    if (i > counter) {
      changeContent((x) => __spreadProps(__spreadValues({}, x), { myCode: code, counter: i }));
      return;
    }
    const cb = async () => {
      const code2 = getValue();
  ...
  onSessionUpdate(() => {
    ...
    setTimeout(() => {
      ...
      if (mod.CH() !== changeContent) {
        const ch = mod.CH();
        ch((x) => __spreadProps(__spreadValues({}, x), {
          myCode: sess.code,
          counter: sess.i
        }));
      }
