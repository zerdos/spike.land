...
var moveToWorker = /* @__PURE__ */ __name(async (codeSpace) => {
  const { html, css: css2, i, transpiled } = (await import(`${location.origin}/live/${codeSpace}/mST.mjs`)).mST;
  const div = document.createElement("div");
  div.setAttribute("id", `${codeSpace}-${i}`);
  div.innerHTML = `<style>${css2}</style><div id="root-${codeSpace}" data-i="${i}" style="height: 100%">
  ${html}</div>`;
