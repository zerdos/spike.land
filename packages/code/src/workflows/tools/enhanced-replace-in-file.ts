...
export const getEnhancedReplaceInFileTool = (codeSession: ICode) => createTool(
  async (args: {
    ...
    try {
      ...
      for (const { search, replace } of searchReplace) {
        ...
        try {
          ...
          if (search.startsWith('/') && search.lastIndexOf('/') > 0) {
            ...
            modifiedCode = modifiedCode.replace(new RegExp(search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), replace);
          }
        } catch (error) {
          validationErrors.push(`Invalid regex pattern "${search}": ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
      ...
      const shouldReturn = returnModifiedCode ?? (instructions ? shouldReturnFullCode(instructions, finalCode) : false);
