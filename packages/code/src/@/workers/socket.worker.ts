...
export async function setConnections(
  ...
  sessionSynchronizer.subscribe((updatedSession: ICodeSession) => {
    ...
    try {
      ...
      logger.debug(`Processing session update from sender: ${(updatedSession as any).sender} for ${codeSpace}`);
      ...
      logger.debug(`Updated session hash for ${codeSpace}: ${newHashCode} (was: ${connection.hashCode})`);
...
function createWebSocket(codeSpace: string): Socket {
  ...
  const delegate = {
    ...
    socketDidClose(socket: Socket, event: CloseEvent) {
      logger.info(`WebSocket closed for ${codeSpace} with code ${event.code}, reason: "${event.reason}", wasClean: ${event.wasClean}`);
      if (event.code === 1000 || event.code === 1001) {
        logger.debug(`Normal closure for ${codeSpace}, code: ${event.code}`);
      } else {
        logger.warn(`Abnormal closure for ${codeSpace}, code: ${event.code}, will retry: ${event.code !== 1008}`);
      }
    },
    ...
    socketDidReceiveMessage(socket: Socket, message: string) {
      ...
      logger.debug(`Received WebSocket message for ${codeSpace} (${messageSize} bytes): ${messagePreview}`);
  ...
  logger.debug(`Socket policy: timeout=${SOCKET_POLICY.timeout}ms, attempts=${SOCKET_POLICY.attempts}, maxDelay=${SOCKET_POLICY.maxDelay}ms`);
...
async function fetchInitialSession(codeSpace: string): Promise<ICodeSession> {
  ...
  try {
    ...
    if (!response.ok) {
      logger.error(`Failed to fetch initial session for ${codeSpace}: ${response.status} ${response.statusText}`);
      throw new Error(`HTTP error: ${response.status} ${response.statusText}`);
    }
    
    const rawSession = await response.json() as ICodeSession;
    logger.debug(`Raw session received for ${codeSpace}, size: ${JSON.stringify(rawSession).length} bytes`);
  ...
  try {
    ...
    if (data.error === "old hashes not matching") {
      ...
      if (hashCode === data.hash) {
        ...
        logger.warn(`Hash still mismatched after refresh for ${codeSpace}: local=${hashCode}, server=${data.hash || 'unknown'}`);
      }
      
      const patch = generateSessionPatch(sess, connection.oldSession);
      logger.debug(`Sending patch after hash mismatch for ${codeSpace}, patch size: ${JSON.stringify(patch).length} bytes`);
      ws.send(JSON.stringify({ ...patch, name: connection.user }));
      return;
    }

    if (data.hash && data.hash !== connection.hashCode) {
      logger.warn(`Hash mismatch for ${codeSpace}: local=${connection.hashCode}, server=${data.hash}`);
      ...
      if (data.hash === freshHash) {
        ...
        logger.warn(`Server hash doesn't match fresh session for ${codeSpace}: server=${data.hash}, fresh=${freshHash}`);
      }
      return;
    }

    if (data.hashCode && data.oldHash) {
      logger.debug(`Received patch data for ${codeSpace}: oldHash=${data.oldHash}, newHash=${data.hashCode}`);
      ...
      if (connection.hashCode !== data.hashCode && connection.hashCode === data.oldHash) {
        ...
        logger.debug(`Current hash: ${connection.hashCode}, patch oldHash: ${data.oldHash}, patch newHash: ${data.hashCode}`);
      }
    }
  ...
  port.addEventListener("message", async (evt: MessageEvent) => {
    ...
    try {
      ...
      if (data.id === 'connect') {
        ...
        logger.info(`Port ${portId} connected to codeSpace ${sessionCodeSpace}, active ports: ${newCount}`);
        return;
      }
      ...
      if (session) {
        ...
        if (connection) {
          ...
          logger.warn(`No connection found for ${session.codeSpace} when broadcasting from port ${portId}`);
        }
      }
...
logger.debug(`Initial state: ${connections.size} connections, ${sessionSynchronizers.size} synchronizers, ${activePorts.size} active ports`);
