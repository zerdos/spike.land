...
import { Code, Zap, Bot, Share2, Package, Download, Sun, Moon } from "lucide-react";
...
const Feature= ({ icon: Icon, title, description, color }) => {
  ...
  return (
    <motion.div
      ...
      `}>
      <Card
        className={`h-full hover:shadow-lg transition-all duration-300 border ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
...
const LandingPage = () => {
  ...
  return (
    <ThemeContext.Provider value={{ isDarkMode, toggleTheme }}>
      <div
        className={`min-h-screen ${isDarkMode ? "bg-gray-900 text-white" : "bg-gradient-to-b from-blue-50 to-white text-gray-900"}`}>
        ...
        <footer
          className={`py-10 ${isDarkMode ? "bg-gray-800 text-gray-300" : "bg-gray-100 text-gray-700"}`}>
          <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center">
            <p>&copy; 2024 Your Company Name. All rights reserved.</p>
            <nav className="mt-4 md:mt-0">
              <a
                href="/privacy-policy"
                className={`mx-3 hover:text-blue-600 transition-colors duration-300 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                Privacy Policy
              </a>
              <a
