...
export async function deleteAllServiceWorkers() {
    if ('serviceWorker' in navigator) {
      try {
        ...
        const deleteCachePromises = cacheNames.map(cacheName => caches.delete(cacheName));
        ...
        console.log('Service Worker cleanup complete. You may need to reload the page or close all tabs of this site for changes to take full effect.');
      } catch (error) {
