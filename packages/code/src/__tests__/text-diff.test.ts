...
import { applyDiff, createDiff, ICodeSessionDiff } from "@/lib/text-diff";
import { AddOperation, Operation, ReplaceOperation, TestOperation } from "fast-json-patch";
...
describe("text-diff", () => {
  describe("createDiff", () => {
    ...
    it("should create a diff for multiple property changes", () => {
      ...
      expect(
        diff.some(op =>
          op.path === "/code" && (op as ReplaceOperation<any>).value === "const x = 2;"
        ),
      ).toBe(true);
      expect(
        diff.some(op =>
          op.path === "/html" && (op as ReplaceOperation<any>).value === "<span></span>"
        ),
      ).toBe(true);
      expect(
        diff.some(op =>
          op.path === "/css" && (op as ReplaceOperation<any>).value === "body { color: red; }"
        ),
      ).toBe(true);
    });
    ...
    it("should optimize adding a new message", () => {
      ...
      expect(diff[0].path).toBe("/messages/-");
      expect((diff[0] as AddOperation<any>).value).toEqual(
        createMessage("assistant", "Hi there", "2"),
      );
    });
    ...
    it("should handle message content that doesn't start with original content", () => {
      ...
      expect(diff.some(op => op.op === "replace" && op.path.includes("/content"))).toBe(true);
      expect(diff.some(op => (op as ReplaceOperation<any>).value === "Different content")).toBe(
        true,
      );
    });
