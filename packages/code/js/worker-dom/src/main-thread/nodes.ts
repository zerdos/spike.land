 ...
 */
const nodeListEach = (list: NodeList, callback: (value: Node, key: number) => any): void =>
    ...
    for (let iterator = 0; iterator < nodeBufferLength; iterator += TransferrableNodeIndex.End) {
      ...
      if (nodeBuffer[iterator + TransferrableNodeIndex.NodeType] === NodeType.TEXT_NODE) {
        node = document.createTextNode(
          this.stringContext.get(nodeBuffer[iterator + TransferrableNodeIndex.TextContent]),
        );
        ...
        node = nodeBuffer[iterator + TransferrableNodeIndex.Namespace] !== 0
          ? document.createElementNS(
            this.stringContext.get(nodeBuffer[iterator + TransferrableNodeIndex.Namespace]),
            nodeName,
          )
