...
var initSession = async (room, initData) => {
  initSess = initSess || (await import("./session-CRV2W5UX.mjs")).default;
  return initSess(room, initData);
};
var prettier = async (code) => {
  formatter = formatter || (await import("./formatter-2JV6M26K.mjs")).formatter;
...
async function startMonacoWithSession(session) {
  ...
  const startMonaco = (await import("./startMonaco-IYB2NKQI.mjs")).default;
...
async function runner(c, changes = null, session, counter) {
  ...
  formatter = formatter || (await import("./formatter-2JV6M26K.mjs")).formatter;
  esbuildEsmTransform = esbuildEsmTransform || (await import("./esbuildEsm-FO62KB3Y.mjs")).transform;
  ...
  try {
    ...
    if (transpiled.length) {
      if (counter < session.i)
        return;
      try {
        getHtmlAndCss = getHtmlAndCss || (await import("./renderToString-DRAR57YL.mjs")).getHtmlAndCss;
        if (counter < session.i)
          return;
...
async function quickStart(session, room, keepFullScreen, saveCode) {
  ...
  const { renderPreviewWindow } = await import("./renderPreviewWindow-WGV2PRR3.mjs");
