...
import { FileType, UnixFSTime, HashAlg, Time, CIDVersion } from 'ipfs-message-port-protocol/src/data';
import { EncodedCID } from './block';
import { ReadStream } from 'fs';
declare type Mode = string | number;
...
export interface DAG {
    ...
    get(cid: CID, options: GetOptions): Promise<{
        value: DAGNode;
        remainderPath: string;
    }>;
    resolve(pathOrCID: string | CID, options: ResolveOptions): Promise<{
        cid: CID;
        remainderPath: string;
    }>;
...
export declare type WriteContent = string | ArrayBufferView | ArrayBuffer | Blob | AsyncIterable<ArrayBufferView>;
export declare type AddInput = Blob | string | ArrayBufferView | ArrayBuffer | FileInput | ReadStream;
export declare type AddAllInput = Iterable<AddInput> | AsyncIterable<AddInput>;
...
export declare type FileContent = string | ArrayBufferView | ArrayBuffer | Blob | Iterable<ArrayBuffer | ArrayBufferView> | AsyncIterable<ArrayBuffer | ArrayBufferView>;
...
export interface BlockService {
    ...
    rm(cid: CID | CID[], options?: RmBlockOptions): AsyncIterable<{
        cid: CID;
        error?: Error;
    }>;
    stat(cid: CID, options?: StatBlockOptions): Promise<{
        cid: CID;
        size: number;
    }>;
}
