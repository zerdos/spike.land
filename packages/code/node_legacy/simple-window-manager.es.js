...
class Clicked {
  constructor(element, callback, options) {
    if (typeof element === "string") {
      ...
      if (!element) {
        console.warn(
          `Unknown element: document.querySelector(${element}) in clicked()`,
        );
        return;
      }
    }
  ...
  pastThreshold(x, y) {
    return Math.abs(this.lastX - x) > this.options.threshold ||
      Math.abs(this.lastY - y) > this.options.threshold;
  }
  ...
  handleClicks(e) {
    if (this.options.doubleClicked) {
      this.doubleClickedTimeout = setTimeout(
        () => this.doubleClickedCancel(e),
  ...
  handleDown(e, x, y) {
    if (this.doubleClickedTimeout) {
      ...
      if (this.options.longClicked) {
        this.longClickedTimeout = setTimeout(
          () => this.longClicked(e),
...
class Menu {
  ...
  show(menuItem) {
    ...
    if (this.menu && this.menu.showing === menuItem) {
      ...
      if (this.menu.applicationMenu) {
        ...
        this.div.style.left = parent.offsetLeft + parent.offsetWidth -
          config.Overlap + "px";
        this.div.style.top = parent.offsetTop + div.offsetTop - config.Overlap +
          "px";
      }
      ...
      for (let child of this.children) {
        ...
        accelerator = childAccelerator > accelerator
          ? childAccelerator
          : accelerator;
        if (child.submenu) {
  ...
  horizontalSelector(e, direction) {
    if (direction === "right") {
      if (this.selector.submenu) {
        ...
        this.selector.submenu.selector.div.style.backgroundColor =
          config.SelectedBackgroundStyle;
        this.selector = null;
      } else {
  ...
  static setApplicationMenu(menu, parent = document.body) {
    ...
    Menu.application = html({
      parent,
      styles: config.ApplicationContainerStyle,
    });
...
class Window extends eventemitter3 {
  ...
  focus() {
    ...
    if (this.options.titlebar) {
      this.winTitlebar.style.backgroundColor =
        this.options.backgroundTitlebarActive;
    }
  ...
  blur() {
    ...
    if (this.options.titlebar) {
      this.winTitlebar.style.backgroundColor =
        this.options.backgroundTitlebarInactive;
    }
    this.emit("blur", this);
  }
  ...
  _buildTransform() {
    this.win.style.transform =
      `translate(${this.options.x}px,${this.options.y}px)`;
  }
  ...
  _createWin() {
    ...
    this.win = html({
      ...
      styles: {
        ...
        "width": isNaN(this.options.width)
          ? this.options.width
          : this.options.width + "px",
        "height": isNaN(this.options.height)
          ? this.options.height
  ...
  _createTitlebar() {
    if (this.options.titlebar) {
      ...
      if (this.options.movable) {
        this.winTitlebar.addEventListener(
          "mousedown",
          (e) => this._downTitlebar(e),
        );
        this.winTitlebar.addEventListener(
          "touchstart",
          (e) => this._downTitlebar(e),
  ...
  reposition(bounds, keepInside) {
    ...
    let x = this.x;
    let y = this.y;
    x = this._attachedToScreen.horziontal === "right"
      ? bounds.right - this.width
      : x;
    x = this._attachedToScreen.horizontal === "left" ? bounds.left : x;
    y = this._attachedToScreen.vertical === "bottom"
...
class Snap {
  ...
  screenMove(rect, horizontal, vertical) {
    ...
    if (
      rect.left - this.options.snap <= width &&
      rect.right + this.options.snap >= 0
    ) {
      if (Math.abs(rect.top - top) <= this.options.snap) {
        horizontal.push({
          distance: Math.abs(rect.top - top),
          left: 0,
          width,
          top: top,
          side: "top",
        ...
        horizontal.push({
          distance: Math.abs(rect.bottom - height),
          left: 0,
          width,
          top: height,
          side: "bottom",
    ...
    if (
      rect.top - this.options.snap <= height &&
      rect.bottom + this.options.snap >= 0
    ) {
      if (Math.abs(rect.left - 0) <= this.options.snap) {
        vertical.push({
          distance: Math.abs(rect.left - 0),
          top: 0,
          height,
          left: 0,
          side: "left",
        ...
        vertical.push({
          distance: Math.abs(rect.right - width),
          top: 0,
          height,
          left: width,
          side: "right",
  ...
  windowsMove(original, rect, horizontal, vertical) {
    for (let win of this.wm.windows) {
      if (!win.options.noSnap && win !== original) {
        ...
        if (
          rect.left - this.options.snap <= rect2.right &&
          rect.right + this.options.snap >= rect2.left
        ) {
          if (Math.abs(rect.top - rect2.bottom) <= this.options.snap) {
            horizontal.push({
              distance: Math.abs(rect.top - rect2.bottom),
              left: rect2.left,
              width: rect2.width,
              top: rect2.bottom,
              side: "top",
            ...
            if (Math.abs(rect.left - rect2.left) <= this.options.snap) {
              vertical.push({
                distance: Math.abs(rect.left - rect2.left),
                top: rect2.top,
                height: rect2.height,
                left: rect2.left,
                side: "left",
              ...
              Math.abs(rect.right - rect2.right) <= this.options.snap
            ) {
              vertical.push({
                distance: Math.abs(rect.right - rect2.right),
                top: rect2.top,
                height: rect2.height,
                left: rect2.right,
                side: "right",
            ...
            horizontal.push({
              distance: Math.abs(rect.bottom - rect2.top),
              left: rect2.left,
              width: rect2.width,
              top: rect2.top,
              side: "bottom",
        ...
        if (
          rect.top - this.options.snap <= rect2.bottom &&
          rect.bottom + this.options.snap >= rect2.top
        ) {
          if (Math.abs(rect.left - rect2.right) <= this.options.snap) {
            vertical.push({
              distance: Math.abs(rect.left - rect2.right),
              top: rect2.top,
              height: rect2.height,
              left: rect2.right,
              side: "left",
            ...
            if (Math.abs(rect.top - rect2.top) <= this.options.snap) {
              horizontal.push({
                distance: Math.abs(rect.top - rect2.top),
                left: rect2.left,
                width: rect2.width,
                top: rect2.top,
                side: "top",
              ...
              Math.abs(rect.bottom - rect2.bottom) <= this.options.snap
            ) {
              horizontal.push({
                distance: Math.abs(rect.bottom - rect2.bottom),
                left: rect2.left,
                width: rect2.width,
                top: rect2.bottom,
                side: "bottom",
            ...
            vertical.push({
              distance: Math.abs(rect.right - rect2.left),
              top: rect2.top,
              height: rect2.height,
              left: rect2.left,
              side: "right",
  ...
  moveEnd(win) {
    ...
    if (this.horizontal.style.display === "block") {
      ...
      switch (this.horizontal.side) {
        ...
        case "bottom":
          win.bottom = Math.floor(
            this.horizontal.yPosition - spacing - bounds.top + top,
          );
          break;
      }
      win.attachToScreen(
        "vertical",
        this.horizontal.screen ? this.horizontal.side : "",
      );
    }
    if (this.vertical.style.display === "block") {
      ...
      switch (this.vertical.side) {
        ...
        case "right":
          win.right = Math.floor(
            this.vertical.xPosition - spacing - bounds.left,
          );
          break;
      }
      win.attachToScreen(
        "horziontal",
        this.vertical.screen ? this.vertical.side : "",
      );
    }
...
class WindowManager {
  ...
  sendToFront(win) {
    ...
    console.assert(
      index !== -1,
      "sendToFront should find window in this.windows",
    );
  ...
  _focus(win) {
    ...
    console.assert(
      index !== -1,
      "WindowManager._focus should find window in this.windows",
    );
    if (index !== this.windows.length - 1) {
      this.windows.splice(index, 1);
      this.windows.push(win);
