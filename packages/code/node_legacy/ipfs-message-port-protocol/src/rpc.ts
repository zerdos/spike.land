...
type Return<T> = T extends Promise<infer U> ? Promise<U & TransferOptions>
 ...
 */
export type Method<T extends Record<string, unknown>> = ServiceQuery<
  T
>["method"];
 ...
 */
export type Namespace<T extends Record<string, unknown>> = ServiceQuery<
  T
>["namespace"];
...
type R<O> = O extends Promise<infer T> ? Promise<WithTransferOptions<T>>
  : Promise<WithTransferOptions<O>>;

type WithTransferOptions<O> = O extends Record<string, unknown>
  ? O & TransferOptions
  : O;
