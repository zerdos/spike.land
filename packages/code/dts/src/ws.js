...
class Code {
    ...
    constructor() {
        ...
        this.users = new AVLTree((a, b) => (a === b ? 0 : a < b ? 1 : -1), true);
        this.init();
    }
    async init() {
        ...
        await mutex.runExclusive(async () => {
            ...
            const startSess = await fetch(`${location.origin}/live/${codeSpace}/session`).then((resp) => resp.json());
            if (head === 0) {
                ...
                const startSessLocal = (await ldb(codeSpace).getItem(String(head)));
    ...
    syncKV(oldSession, newSess, message) {
        syncStorage(ldb(codeSpace).setItem, ldb(codeSpace).getItem, oldSession, newSess, message);
    }
    async syncWS(newSession) {
...
export const syncWS = async (sess) => await cSess.syncWS(sess);
