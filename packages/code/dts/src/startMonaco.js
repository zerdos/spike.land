...
const refreshAta = async (code, originToUse) => {
    try {
...
async function fetchAndCreateExtraModels(code, originToUse) {
...
const monacoContribution = async (code) => {
...
const mod = {};
export const startMonaco = async ({ code, container, codeSpace, onChange, }) => {
    if (!mod[codeSpace]) {
...
async function startMonacoPristine({ code, container, codeSpace, onChange, }) {
    ...
    const tsCheck = async () => {
        ...
        if (needNewAta)
            await refreshAta(model.getValue(), originToUse);
