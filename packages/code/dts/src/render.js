...
export const render = async (_rootEl, codeSpace, mApp = null, signal = null, data = null) => {
    ...
    root.render(_jsx(CacheProvider, { value: cache, children: _jsx(App, { width: window.innerWidth, height: window.innerHeight, top: 0, left: 0 }) }));
...
async function getApp(App, codeSpace) {
    if (!App) {
        try {
            ...
            m.App = () => (_jsxs("div", { children: [_jsx("h1", { children: "Error" }), _jsx("pre", { children: JSON.stringify({ err }) })] }));
        }
    }
    else {
        m.App = App;
...
function mineFromCaches(_cache, html) {
    ...
    try {
        const styledJSXStyles = Array.from(document.querySelectorAll("style[data-styled-jsx]")).map((style) => style.textContent);
        const emotionStyles = Array.from(new Set(Array.from(document.querySelectorAll(`style[data-emotion="${key}"]`))
