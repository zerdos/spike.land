...
`;
const offLoadToServer = async (code, origin) => {
...
export const transpile = async (code, origin, wasmModule) => {
    try {
        if (wasmModule) {
        ...
        const transformedCode = await transform(`${decorateCodeStr}
      ${code.replace(`export default `, `const module = {renderApp}; 
        module.default = `)}
...
export const build = async ({ codeSpace, origin, format = "esm", wasmModule, }) => {
    if (wasmModule) {
        await mod.initialize(wasmModule);
    }
    else {
