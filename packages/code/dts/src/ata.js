...
export async function ata({ code, originToUse, prettierJs, tsx, }) {
    const impRes = {};
    await ataRecursive(`/** @jsx jsx */
...
declare module 'react' {
  interface Attributes {
    ...
    const extraLibs = [
        ...(await Promise.all(Object.keys(impRes)
    ...
    async function ataRecursive(code, baseUrl) {
    ...
    async function extractUrlFromResponse(response, ref) {
    ...
    async function tryToExtractUrlFromPackageJson(npmPackage) {
        try {
            const packageJson = await (await fetch(`${originToUse}/${npmPackage}/package.json`)).json();
    ...
    async function handleNewBase(newBase, ref, baseUrl) {
        if (!impRes[newBase]) {
            ...
            const fileName = new URL(ref.includes("d.ts") || ref.includes(".mjs")
                ? ref
                : `${ref}/index.d.ts`, ref.startsWith(".") ? baseUrl : originToUse).toString();
            if (!impRes[fileName]) {
                impRes[fileName] = {
                    url: fileName,
                    content: `
