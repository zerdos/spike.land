...
// ../../../.yarn/berry/cache/esbuild-wasm-npm-0.24.0-df844cf64e-10c0.zip/node_modules/esbuild-wasm/lib/browser.js
  ...
  "../../../.yarn/berry/cache/esbuild-wasm-npm-0.24.0-df844cf64e-10c0.zip/node_modules/esbuild-wasm/lib/browser.js"(
    ...
    ((module2) => {
        ...
        if (from && typeof from === "object" || typeof from === "function") {
          for (let key of __getOwnPropNames2(from)) {
            if (!__hasOwnProp2.call(to, key) && key !== except) {
              __defProp2(to, key, {
                get: () => from[key],
                enumerable: !(desc = __getOwnPropDesc2(from, key)) ||
                  desc.enumerable,
              });
            }
          }
      ...
      var __toCommonJS = (mod2) =>
        __copyProps2(__defProp2({}, "__esModule", { value: true }), mod2);
...
`,
      ...
      function pushCommonFlags(flags, options, keys) {
        ...
        if (dropLabels) {
          flags.push(
            `--drop-labels=${
              Array.from(dropLabels).map((what) =>
                validateStringValue(what, "dropLabels")
              ).join(",")
            }`,
          );
        }
        ...
        if (define) {
          for (let key in define) {
            ...
            flags.push(
              `--define:${key}=${
                validateStringValue(define[key], "define", key)
              }`,
            );
          }
        }
      ...
      function flagsForBuildOptions(
        ...
        let entryPoints = getFlag(
          options,
          keys,
          "entryPoints",
          mustBeEntryPoints,
        );
        let absWorkingDir = getFlag(
          options,
          keys,
          "absWorkingDir",
          mustBeString,
        ...
        let write =
          (_a = getFlag(options, keys, "write", mustBeBoolean)) != null
            ? _a
            : writeDefault;
        ...
        if (banner) {
          for (let type in banner) {
            ...
            flags.push(
              `--banner:${type}=${
                validateStringValue(banner[type], "banner", type)
              }`,
            );
          }
        }
        if (footer) {
          for (let type in footer) {
            ...
            flags.push(
              `--footer:${type}=${
                validateStringValue(footer[type], "footer", type)
              }`,
            );
          }
        }
        ...
        if (loader) {
          for (let ext in loader) {
            ...
            flags.push(
              `--loader:${ext}=${
                validateStringValue(loader[ext], "loader", ext)
              }`,
            );
          }
        }
        ...
        let handleIncomingPacket = (bytes) => {
          if (isFirstPacket) {
            ...
            if (binaryVersion !== "0.24.0") {
              throw new Error(
                `Cannot start service: Host version "${"0.24.0"}" does not match binary version ${
                  quote(binaryVersion)
                }`,
              );
            }
      ...
      var handlePlugins = (
        ...
        __async(void 0, null, function* () {
          ...
          requestCallbacks["on-start"] = (id, request) =>
            __async(void 0, null, function* () {
              ...
              yield Promise.all(onStartCallbacks.map((_0) =>
                __async(void 0, [_0], function* ({ name, callback, note }) {
                  try {
          ...
          let runOnEndCallbacks = (result, done) =>
            done([], []);
      ...
      function parseStackLinesV8(streamIn, lines, ident) {
        let at = "    at ";
        if (
          streamIn.readFileSync && !lines[0].startsWith(at) &&
            ...
            while (true) {
              ...
              if (match) {
                ...
                return {
                  ...
                  length:
                    encodeUTF8(lineText.slice(column, column + length)).length,
                  lineText: lineText + "\n" + lines.slice(1).join("\n"),
                  suggestion: "",
                };
              }
...
${file}:${line}:${column}: ERROR: ${pluginText}${e.text}`;
        ...
        for (
          const [key, value] of [["errors", errors], ["warnings", warnings]]
        ) {
      ...
      var startRunningService = (wasmURL, wasmModule, useWorker) =>
        __async(void 0, null, function* () {
          let worker;
          let rejectAllWith;
          const rejectAllPromise = new Promise((resolve) =>
            rejectAllWith = resolve
          );
          if (useWorker) {
            let blob = new Blob([
            ...
            let onmessage = ((postMessage) => {
              ...
              for (let o = self; o; o = Object.getPrototypeOf(o)) {
                for (let k of Object.getOwnPropertyNames(o)) {
                  if (!(k in globalThis2)) {
                    Object.defineProperty(globalThis2, k, {
                      get: () => self[k],
                    });
                  }
                }
                    ...
                    this.importObject = {
                      ...
                      gojs: {
                        ...
                        "runtime.nanotime1": (sp) => {
                          ...
                          setInt64(
                            sp + 8,
                            (timeOrigin + performance.now()) * 1e6,
                          );
                        },
                        // func walltime() (sec int64, nsec int32)
                        "runtime.walltime": (sp) => {
                  ...
                  run(instance) {
                    return __async2(this, null, function* () {
                      ...
                      const strPtr = (str) => {
                        const ptr = offset;
                        const bytes = encoder.encode(str + "\0");
                        new Uint8Array(this.mem.buffer, offset, bytes.length)
                          .set(bytes);
                        offset += bytes.length;
                        if (offset % 8 !== 0) {
                          offset += 8 - offset % 8;
              ...
              onmessage2 = ({ data: wasm }) => {
                ...
                fs.read = (fd, buffer, offset, length, position, callback) => {
                  ...
                  let count = Math.max(
                    0,
                    Math.min(length, first.length - stdinPos),
                  );
                  buffer.set(
                    first.subarray(stdinPos, stdinPos + count),
                    offset,
                  );
                  stdinPos += count;
                  if (stdinPos === first.length) {
              ...
              function tryToInstantiateModule(wasm, go2) {
                return __async2(this, null, function* () {
                  ...
                  if (
                    ...
                    const result2 = yield WebAssembly.instantiateStreaming(
                      res,
                      go2.importObject,
                    );
                    return result2.instance;
                  ...
                  const bytes = yield res.arrayBuffer();
                  const result = yield WebAssembly.instantiate(
                    bytes,
                    go2.importObject,
                  );
                  return result.instance;
          ...
          worker.postMessage(
            wasmModule || new URL(wasmURL, location.href).toString(),
          );
...
// ../code/dist/cf-esbuild.mjs
var import_esbuild_wasm = __toESM(require_browser(), 1);
import wasmUrl from "./239eff5ee94e121c4b81281b7af7f4dae40c483c-esbuild-C36J22UB.wasm";
var oo = {
  "/@/": "/@/",
...
var externalString = "bundle=true&external=" +
  Object.keys(oo).filter((o) =>
    !o.endsWith("/") && o !== "@emotion/react/jsx-runtime"
  ).join(",");
function importMapReplace(code, origin) {
  ...
  const replacer = (match, p1, p2, p3char) => {
    ...
    if (packageName?.startsWith(".") || packageName?.startsWith("http")) {
      if (packageName?.startsWith("http") && !packageName?.startsWith(origin)) {
        ...
        const [pkgName2, exports2] = oldUrl.pathname.slice(1).split(
          "?bundle=true&exports=",
        );
        if (exports2) {
          return p1 +
            `"${origin}/${pkgName2}?${externalString}&exports=${exports2}"` +
            p3;
        }
        return match;
      }
    ...
    if (importMatch) {
      ...
      return p1 +
        `"${origin}/${packageName}?${externalString}&exports=${
          importedItems.join(",")
        }"` + p3;
    }
  ...
  let replaced = str.replace(topLevelImportPattern, replacer).replace(
    topLevelExportPattern,
    replacer,
  ).replace(dynamicImportPattern, replacer).replace(
    dynamicImportTemplatePattern,
    replacer,
  ...
  _drainUnlockWaiters() {
    if (this._queue.length === 0) {
      ...
      for (let weight = this._value; weight > 0; weight--) {
        ...
        const i = waiters.findIndex((waiter) =>
          waiter.priority <= queuedPriority
        );
...
var mod = {
  ...
  initialize: async (wasmModule) => {
    ...
    await (0, import_esbuild_wasm.initialize)({ wasmModule, worker: false });
...
var initializeModule = async (wasmModule, origin) => {
  ...
  if (wasmModule) {
    ...
    await (0, import_esbuild_wasm.initialize)({
...
var transpile = async ({ code, originToUse, wasmModule }) => {
  return mutex.runExclusive(async () => {
    try {
      await initializeModule(wasmModule, originToUse);
      try {
        const transformedCode = await (0, import_esbuild_wasm.transform)(code, {
          loader: "tsx",
          format: "esm",
          treeShaking: true,
          platform: "browser",
...
export { cf_esbuild_default as default };
//# sourceMappingURL=cf-esbuild.js.map
