...
function printTable(table, pattern, patternLen, word, wordLen) {
  ...
  for (let i = 0; i <= patternLen; i++) {
    ...
    ret += table[i].slice(0, wordLen + 1).map((n) =>
      pad(n.toString(), 3)
    ).join("|") + "\n";
  }
  return ret;
...
export function fuzzyScore(
  ...
  if (
    patternStart >= patternLen || wordStart >= wordLen ||
  ...
  for (
    ...
    const nextMaxWordMatchPos = (patternPos + 1 < patternLen
    ...
    for (
      ...
      const leftScore = canComeLeft
        ? _table[row][column - 1] + (_diag[row][column - 1] > 0
          ? -5
          : 0)
        : 0; // penalty for a gap start
      const canComeLeftLeft = wordPos > minWordMatchPos + 1 &&
        _diag[row][column - 1] > 0;
      const leftLeftScore = canComeLeftLeft
        ? _table[row][column - 2] + (_diag[row][column - 2] > 0 ? -5 : 0)
        : 0; // penalty for a gap start
      if (
        canComeLeftLeft && (!canComeLeft || leftLeftScore >= leftScore) &&
