...
export class Gesture extends Disposable {
  ...
  static addTarget(element) {
    ...
    return {
      dispose: () => {
        Gesture.INSTANCE.targets = Gesture.INSTANCE.targets.filter((t) =>
          t !== element
        );
      },
    };
  ...
  dispatchEvent(event) {
    if (event.type === EventType.Tap) {
      ...
      let setTapCount = 0;
      if (
        currentTime - this._lastSetTapCountTime > Gesture.CLEAR_TAP_COUNT_TIME
      ) {
        setTapCount = 1;
      } else {
        setTapCount = 2;
      ...
      event.type === EventType.Change || event.type === EventType.Contextmenu
    ) {
