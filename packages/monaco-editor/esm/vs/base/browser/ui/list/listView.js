...
export class ListView {
  ...
  set horizontalScrolling(value) {
    ...
    if (this._horizontalScrolling) {
      ...
      this.rowsContainer.style.width = `${
        Math.max(this.scrollWidth || 0, this.renderWidth)
      }px`;
    } else {
  ...
  render(
    ...
    if (this.horizontalScrolling && scrollWidth !== undefined) {
      this.rowsContainer.style.width = `${
        Math.max(scrollWidth, this.renderWidth)
      }px`;
    }
  ...
  get onMouseClick() {
    return Event.map(
      domEvent(this.domNode, "click"),
      (e) => this.toMouseEvent(e),
    );
  }
  get onMouseDblClick() {
    return Event.map(
      domEvent(this.domNode, "dblclick"),
      (e) => this.toMouseEvent(e),
    );
  }
  get onMouseMiddleClick() {
    return Event.filter(
      Event.map(
        domEvent(this.domNode, "auxclick"),
        (e) => this.toMouseEvent(e),
      ),
  ...
  get onMouseDown() {
    return Event.map(
      domEvent(this.domNode, "mousedown"),
      (e) => this.toMouseEvent(e),
    );
  }
  get onContextMenu() {
    return Event.map(
      domEvent(this.domNode, "contextmenu"),
      (e) => this.toMouseEvent(e),
    );
  }
  get onTouchStart() {
    return Event.map(
      domEvent(this.domNode, "touchstart"),
      (e) => this.toTouchEvent(e),
    );
  }
  get onTap() {
    return Event.map(
      domEvent(this.rowsContainer, TouchEventType.Tap),
  ...
  onDragOver(event) {
    ...
    event.browserEvent.dataTransfer.dropEffect =
      (typeof result !== "boolean" && result.effect === 0 /* Copy */)
        ? "copy"
        : "move";
    let feedback;
    ...
    feedback = distinct(feedback).filter((i) => i >= -1 && i < this.length)
      .sort((a, b) => a - b);
  ...
  getItemIndexFromEventTarget(target) {
    ...
    let element = target;
    while (
      element instanceof HTMLElement && element !== this.rowsContainer &&
      ...
      if (!didChange) {
        ...
        for (const range of renderRanges) {
          for (let i = range.start; i < range.end; i++) {
            ...
            const beforeRow = afterIndex < this.items.length
        ...
        if (typeof anchorElementIndex === "number") {
          ...
          const newScrollTop = this.elementTop(anchorElementIndex) -
  ...
  probeDynamicHeight(index) {
    const item = this.items[index];
    if (
      !item.hasDynamicHeight || item.lastDynamicHeightWidth === this.renderWidth
    ) {
      return 0;
    }
    if (
