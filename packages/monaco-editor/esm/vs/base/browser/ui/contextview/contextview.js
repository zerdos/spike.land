...
export function layout(viewportSize, viewSize, anchor) {
  const layoutAfterAnchorBoundary = anchor.mode === LayoutAnchorMode.ALIGN
    ? anchor.offset
    : anchor.offset + anchor.size;
  const layoutBeforeAnchorBoundary = anchor.mode === LayoutAnchorMode.ALIGN
    ? anchor.offset + anchor.size
    : anchor.offset;
...
export class ContextView extends Disposable {
  constructor(container, domPosition) {
    ...
    this._register(
      toDisposable(() => this.setContainer(null, 1 /* ABSOLUTE */)),
    );
  }
  ...
  doLayout() {
    ...
    if (anchorAxisAlignment === 0 /* VERTICAL */) {
      const verticalAnchor = {
        ...
        size: around.height,
        position: anchorPosition === 0 /* BELOW */
          ? 0 /* Before */
          : 1, /* After */
      };
      const horizontalAnchor = {
        offset: around.left,
        size: around.width,
        position: anchorAlignment === 0 /* LEFT */
          ? 0 /* Before */
          : 1, /* After */
        mode: LayoutAnchorMode.ALIGN,
      };
      top = layout(window.innerHeight, viewSizeHeight, verticalAnchor) +
        window.pageYOffset;
      // if view intersects vertically with anchor,  we must avoid the anchor
      if (
        Range.intersects({ start: top, end: top + viewSizeHeight }, {
      ...
      if (
        Range.intersects({ start: left, end: left + viewSizeWidth }, {
