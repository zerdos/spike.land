...
export class IndexTreeModel {
  constructor(user, list, rootElement, options = {}) {
    ...
    this.collapseByDefault = typeof options.collapseByDefault === "undefined"
      ? false
  ...
  spliceSmart(
    ...
    if (recurseLevels === void 0) {
      recurseLevels = (_a = options.diffDepth) !== null && _a !== void 0
        ? _a
        : 0;
    }
    ...
    for (
      const change of diff.changes.sort((a, b) =>
        b.originalStart - a.originalStart
      )
    ) {
      recurseSplice(
        lastStartO,
  ...
  spliceSimple(
    ...
    const { parentNode, listIndex, revealed, visible } = this
    ...
    for (const child of nodesToInsertIterator) {
      ...
      if (child.visible) {
        child.visibleChildIndex = visibleChildStartIndex +
  ...
  _setCollapseState(location, update) {
    ...
    if (
      node !== this.root && this.autoExpandSingleChildren && result &&
      !isCollapsibleStateUpdate(update) && node.collapsible &&
      !node.collapsed && !update.recursive
    ) {
  ...
  createTreeNode(
    ...
    const childRevealed = revealed && visibility !== 0 /* Hidden */ &&
      !node.collapsed;
    const childNodes = Iterable.map(
      childElements,
      (el) =>
        this.createTreeNode(
          el,
    ...
    node.visible = visibility === 2 /* Recurse */
      ? visibleChildrenCount > 0
      : (visibility === 1 /* Visible */);
    if (!node.visible) {
  ...
  updateNodeAfterFilterChange(node) {
    ...
    this._updateNodeAfterFilterChange(
      node,
      node.visible ? 1 /* Visible */ : 0, /* Hidden */
      result,
    );
    this._updateAncestorsRenderNodeCount(
      node.parent,
