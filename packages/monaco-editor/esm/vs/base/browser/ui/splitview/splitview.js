...
export class SplitView extends Disposable {
  constructor(container, options = {}) {
    ...
    this.el.classList.add(
      this.orientation === 0 /* VERTICAL */ ? "vertical" : "horizontal",
    );
    ...
    this.scrollableElement = this._register(
      new SmoothScrollableElement(this.viewContainer, {
        vertical: this.orientation === 0 /* VERTICAL */
          ? 1 /* Auto */
          : 2, /* Hidden */
        horizontal: this.orientation === 1 /* HORIZONTAL */
          ? 1 /* Auto */
          : 2, /* Hidden */
      }, this.scrollable),
    );
  ...
  layout(size, layoutContext) {
    ...
    if (!this.proportions) {
      ...
      const lowPriorityIndexes = indexes.filter((i) =>
        this.viewItems[i].priority === 1 /* Low */
      );
      const highPriorityIndexes = indexes.filter((i) =>
        this.viewItems[i].priority === 2 /* High */
      );
      this.resize(
  ...
  onSashStart({ sash, start, alt }) {
    ...
    const disposable = combinedDisposable(
      domEvent(document.body, "keydown")((e) =>
        resetSashDragState(this.sashDragState.current, e.altKey)
      ),
      domEvent(document.body, "keyup")(() =>
  ...
  resizeView(index, size) {
    ...
    const lowPriorityIndexes = [
      ...indexes.filter((i) => this.viewItems[i].priority === 1 /* Low */),
      index,
    ];
    const highPriorityIndexes = indexes.filter((i) =>
      this.viewItems[i].priority === 2 /* High */
    ...
    for (let index = 0; index < this.sashItems.length; index++) {
      ...
      if (min && max) {
        ...
        const snappedAfter = typeof snapAfterIndex === "number" &&
          !this.viewItems[snapAfterIndex].visible;
        if (
          snappedBefore && collapsesUp[index] &&
          (position > 0 || this.startSnappingEnabled)
        ) {
          sash.state = 1 /* Minimum */;
        } else if (
          snappedAfter && collapsesDown[index] &&
