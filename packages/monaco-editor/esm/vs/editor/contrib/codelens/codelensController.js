...
let CodeLensContribution = class CodeLensContribution {
  constructor(_editor, _commandService, _notificationService, _codeLensCache) {
    ...
    this._lenses = [];
    this._getCodeLensModelDelays = new LanguageFeatureRequestDelays(
      CodeLensProviderRegistry,
      250,
      2500,
    );
    this._oldCodeLensModels = new DisposableStore();
    this._resolveCodeLensesDelays = new LanguageFeatureRequestDelays(
      CodeLensProviderRegistry,
      250,
      2500,
    );
    ...
    this._disposables.add(this._editor.onDidChangeConfiguration((e) => {
      if (
        e.hasChanged(38 /* fontInfo */) ||
        e.hasChanged(13 /* codeLensFontSize */) ||
        e.hasChanged(12 /* codeLensFontFamily */)
      ) {
    ...
    this._disposables.add(
      CodeLensProviderRegistry.onDidChange(this._onModelChange, this),
    );
    this._onModelChange();
  ...
  _updateLensStyle() {
    ...
    let newStyle = `
		.monaco-editor .codelens-decoration.${this._styleClassName} { line-height: ${codeLensHeight}px; font-size: ${fontSize}px; padding-right: ${
      Math.round(fontSize * 0.5)
    }px; font-feature-settings: ${editorFontInfo.fontFeatureSettings} }
		.monaco-editor .codelens-decoration.${this._styleClassName} span.codicon { line-height: ${codeLensHeight}px; font-size: ${fontSize}px; }
		`;
    if (fontFamily) {
      newStyle +=
        `.monaco-editor .codelens-decoration.${this._styleClassName} { font-family: var(${fontFamilyVar})}`;
    }
    this._styleElement.textContent = newStyle;
    (_a = this._editor.getDomNode()) === null || _a === void 0
      ? void 0
      : _a.style.setProperty(
        fontFamilyVar,
        fontFamily !== null && fontFamily !== void 0 ? fontFamily : "inherit",
      );
    //
  ...
  _onModelChange() {
    ...
    const scheduler = new RunOnceScheduler(() => {
      ...
      this._getCodeLensModelPromise.then((result) => {
        ...
        const newDelay = this._getCodeLensModelDelays.update(
          model,
          Date.now() - t1,
        );
      ...
      this._editor.changeDecorations((decorationsAccessor) => {
        this._editor.changeViewZones((viewZonesAccessor) => {
          ...
          this._lenses.forEach((lens) => {
            if (
              !lens.isValid() || lastLensLineNumber === lens.getLineNumber()
            ) {
    ...
    this._localToDispose.add(this._editor.onMouseDown((e) => {
      ...
      if (
        (target === null || target === void 0 ? void 0 : target.tagName) ===
          "SPAN"
      ) {
  ...
  _renderCodeLensSymbols(symbols) {
    ...
    this._editor.changeDecorations((decorationsAccessor) => {
      this._editor.changeViewZones((viewZoneAccessor) => {
        ...
        let groupsIndex = 0;
        while (
          groupsIndex < groups.length && codeLensIndex < this._lenses.length
        ) {
  ...
  _resolveCodeLensesInViewport() {
    ...
    this._resolveCodeLensesPromise.then(() => {
      // update moving average
      const newDelay = this._resolveCodeLensesDelays.update(
        model,
        Date.now() - t1,
      );
