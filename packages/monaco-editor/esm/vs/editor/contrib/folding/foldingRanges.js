...
export class FoldingRegions {
  ...
  ensureParentIndices() {
    if (!this._parentsComputed) {
      ...
      for (let i = 0, len = this._startIndexes.length; i < len; i++) {
        ...
        if (
          startLineNumber > MAX_LINE_NUMBER || endLineNumber > MAX_LINE_NUMBER
        ) {
          throw new Error(
        ...
        let parentIndex = parentIndexes.length > 0
  ...
  getParentIndex(index) {
    ...
    let parent = ((this._startIndexes[index] & MASK_INDENT) >>> 24) +
  ...
  toString() {
    ...
    for (let i = 0; i < this.length; i++) {
      res[i] = `[${this.isCollapsed(i) ? "+" : "-"}] ${
        this.getStartLineNumber(i)
