  ...
  static findId(candidate, container) {
    ...
    if (typeof candidate === "string") {
      ...
      if (container.children.get(candidateId) !== undefined) {
        candidateId =
          `${container.id}/${candidate.name}_${candidate.range.startLineNumber}_${candidate.range.startColumn}`;
      }
    }
    let id = candidateId;
...
OutlineModel._requestDurations = new LanguageFeatureRequestDelays(
...
OutlineModel._keys = new class {
  ...
  for(textModel, version) {
    return `${textModel.id}/${version ? textModel.getVersionId() : ""}/${
      this._hash(DocumentSymbolProviderRegistry.all(textModel))
    }`;
  }
  _hash(providers) {
    let result = "";
