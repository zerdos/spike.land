...
let GotoDefinitionAtPositionEditorContribution =
  class GotoDefinitionAtPositionEditorContribution {
    ...
    startFindDefinition(position) {
      ...
      return this.previousPromise.then((results) => {
        ...
        if (results.length > 1) {
          ...
          this.textModelResolverService.createModelReference(result.uri).then(
            (ref) => {
              ...
              const modeId = this.modeService.getModeIdByFilepathOrFirstLine(
                textEditorModel.uri,
              );
              this.addDecoration(
                wordRange,
    ...
    getPreviewValue(textEditorModel, startLineNumber, result) {
      ...
      const numberOfLinesInRange = rangeToUse.endLineNumber -
        rangeToUse.startLineNumber;
      if (
    ...
    stripIndentationFromPreviewRange(
      ...
      const previewValue = textEditorModel.getValueInRange(previewRange)
    ...
    getPreviewRangeBasedOnBrackets(textEditorModel, startLineNumber) {
      ...
      let currentBracket = textEditorModel.findNextBracket(
      ...
      while (currentBracket !== null) {
        if (brackets.length === 0) {
          ...
          if (
            ...
            lastBracket.isOpen && !currentBracket.isOpen
          ) {
            brackets.pop();
          } else {
            brackets.push(currentBracket);
    ...
    gotoDefinition(position, openToSide) {
      ...
      return this.editor.invokeWithinContext((accessor) => {
        const canPeek = !openToSide &&
          this.editor.getOption(72 /* definitionLinkOpensInPeek */) &&
