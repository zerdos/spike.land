...
export class CompletionItem {
  constructor(position, completion, container, provider) {
    ...
    this.filterTextLow = completion.filterText &&
      completion.filterText.toLowerCase();
    // normalize ranges
...
export function provideSuggestionItems(
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const onCompletionList = (provider, container, sw) => {
      ...
      for (let suggestion of container.suggestions) {
        if (!options.kindFilter.has(suggestion.kind)) {
          ...
          if (!suggestion.sortText) {
            suggestion.sortText = typeof suggestion.label === "string"
              ? suggestion.label
              : suggestion.label.name;
          }
          if (
            !needsClipboard && suggestion.insertTextRules &&
            suggestion.insertTextRules & 4 /* InsertAsSnippet */
          ) {
      ...
      durations.push({
        providerName:
          (_a = provider._debugDisplayName) !== null && _a !== void 0
            ? _a
            : "unkown_provider",
        elapsedProvider: (_b = container.duration) !== null && _b !== void 0
          ? _b
          : -1,
    ...
    const snippetCompletions = (() =>
      __awaiter(this, void 0, void 0, function* () {
        if (
          !_snippetSuggestSupport || options.kindFilter.has(27 /* Snippet */)
        ) {
          return;
        }
        if (
    ...
    for (
      let providerGroup of modes.CompletionProviderRegistry.orderedGroups(model)
    ) {
      // for each support in the group ask for suggestions
      let lenBefore = result.length;
      yield Promise.all(providerGroup.map((provider) =>
        __awaiter(this, void 0, void 0, function* () {
          if (
            options.providerFilter.size > 0 &&
          ...
          try {
            const sw = new StopWatch(true);
            const list = yield provider.provideCompletionItems(
              model,
              position,
              context,
              token,
