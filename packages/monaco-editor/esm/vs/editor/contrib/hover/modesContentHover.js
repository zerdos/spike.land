...
} from "./markerHoverParticipant.js";
import {
  MarkdownHover,
  MarkdownHoverParticipant,
} from "./markdownHoverParticipant.js";
class ColorHover {
...
class ModesContentComputer {
  ...
  computeAsync(token) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const markdownHovers = yield this._markdownHoverParticipant.computeAsync(
        this._range,
        token,
      );
      return markdownHovers.map((h) =>
        new HoverPartInfo(this._markdownHoverParticipant, h)
      );
    });
  }
  computeSync() {
    ...
    const lineDecorations = this._editor.getLineDecorations(lineNumber).filter(
      (d) => {
        ...
        const endColumn = (d.range.endLineNumber === lineNumber)
          ...
          : maxColumn;
        if (
          startColumn > hoverRange.startColumn ||
          hoverRange.endColumn > endColumn
        ) {
          return false;
        }
        return true;
    ...
    for (const d of lineDecorations) {
      ...
      if (colorData) {
        ...
        result.push(
          new HoverPartInfo(
            null,
            new ColorHover(Range.lift(range), color, colorData.provider),
          ),
        );
        break;
      }
    }
    const markdownHovers = this._markdownHoverParticipant.computeSync(
      this._range,
      lineDecorations,
    );
    result = result.concat(
      markdownHovers.map((h) =>
        new HoverPartInfo(this._markdownHoverParticipant, h)
      ),
    );
    const markerHovers = this._markerHoverParticipant.computeSync(
      this._range,
      lineDecorations,
    );
    result = result.concat(
      markerHovers.map((h) =>
        new HoverPartInfo(this._markerHoverParticipant, h)
      ),
    );
  ...
  getResultWithLoadingMessage() {
    if (this._range) {
      const loadingMessage = new HoverPartInfo(
        this._markdownHoverParticipant,
        this._markdownHoverParticipant.createLoadingMessage(this._range),
      );
...
export class ModesContentHoverWidget extends Widget {
  constructor(editor, _hoverVisibleKey, instantiationService, _themeService) {
    ...
    this._markerHoverParticipant = instantiationService.createInstance(
      MarkerHoverParticipant,
      editor,
      this,
    );
    this._markdownHoverParticipant = instantiationService.createInstance(
      MarkdownHoverParticipant,
      editor,
      this,
    );
    ...
    this._register(TokenizationRegistry.onDidChange(() => {
      if (this._isVisible && this._lastRange && this._messages.length > 0) {
        this._messages = this._messages.map((msg) => {
          ...
          if (
            msg.data instanceof ColorHover &&
            !!((_a = this._lastRange) === null || _a === void 0
              ? void 0
              : _a.intersectRanges(msg.data.range)) &&
            ...
            return new HoverPartInfo(
              msg.owner,
              new ColorHover(msg.data.range, newColor, msg.data.provider),
            );
          } else {
            return msg;
          }
  ...
  layout() {
    ...
    this._hover.contentsDomNode.style.maxWidth = `${
  ...
  _renderMessages(renderRange, messages) {
    ...
    let highlightRange = messages[0].data.range
      ? Range.lift(messages[0].data.range)
      : null;
    ...
    messages.forEach((_msg) => {
      ...
      if (msg instanceof ColorHover) {
        ...
        const widget = new ColorPickerWidget(
          fragment,
          model,
          this._editor.getOption(122 /* pixelRatio */),
          this._themeService,
        );
        getColorPresentations(
          ...
          this._renderDisposable = combinedDisposable(
            colorListener,
            colorChangeListener,
            widget,
            disposables,
    ...
    if (markdownParts.length > 0) {
      disposables.add(
        this._markdownHoverParticipant.renderHoverParts(
          markdownParts,
          fragment,
        ),
      );
    ...
    if (markerMessages.length) {
      disposables.add(
        this._markerHoverParticipant.renderHoverParts(markerMessages, fragment),
      );
    }
