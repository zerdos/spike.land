...
export class OneSnippet {
  ...
  move(fwd) {
    ...
    if (this._placeholderGroupsIdx >= 0) {
      let operations = [];
      for (
        const placeholder of this._placeholderGroups[this._placeholderGroupsIdx]
      ) {
        ...
        if (placeholder.transform) {
          ...
          const transformedValueLines = placeholder.transform.resolve(
            currentValue,
          ).split(/\r\n|\r|\n/);
          // fix indentation for transformed lines
          for (let i = 1; i < transformedValueLines.length; i++) {
            transformedValueLines[i] = this._editor.getModel()
    ...
    const newSelections = this._editor.getModel().changeDecorations(
      (accessor) => {
        ...
        const selections = [];
        for (
          const placeholder of this
            ._placeholderGroups[this._placeholderGroupsIdx]
        ) {
          const id = this._placeholderDecorations.get(placeholder);
          const range = this._editor.getModel().getDecorationRange(id);
          selections.push(
          ...
          for (
            const enclosingPlaceholder of this._snippet.enclosingPlaceholders(
              placeholder,
            )
          ) {
    ...
    return !couldSkipThisPlaceholder
      ? newSelections !== null && newSelections !== void 0 ? newSelections : []
      : this.move(fwd);
  }
...
export class SnippetSession {
    ...
    snippet.walk((marker) => {
      ...
      if (adjustIndentation) {
        ...
        if (offset === 0) {
          ...
          if (
            prevChar === 10 /* LineFeed */ ||
            prevChar === 13 /* CarriageReturn */
          ) {
  ...
  static createEditsAndSnippets(
    ...
    const modelBasedVariableResolver = editor.invokeWithinContext((accessor) =>
      ...
      snippet.resolveVariables(
        new CompositeSnippetVariableResolver([
          ...
          new ClipboardBasedVariableResolver(
            ...
            editor.getOption(65 /* multiCursorPaste */) === "spread",
          ),
  ...
  _logInfo() {
    return `template="${this._template}", merged_templates="${
