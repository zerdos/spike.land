...
export class SnippetParser {
  ...
  _until(type) {
    ...
    const value = this._scanner.value.substring(start.pos, this._token.pos)
      .replace(/\\(\$|}|\\)/g, "$1");
    this._token = this._scanner.next();
    return value;
  }
  ...
  _parseTabstopOrVariableName(parent) {
    ...
    const match = this._accept(0 /* Dollar */) &&
      (value = this._accept(9, /* VariableName */ true) ||
        this._accept(8, /* Int */ true));
    if (!match) {
  ...
  _parseChoiceElement(parent) {
    ...
    while (true) {
      if (
        this._token.type === 2 /* Comma */ || this._token.type === 7 /* Pipe */
      ) {
        break;
      }
      let value;
  ...
  _parseTransform(parent) {
    ...
    while (true) {
      ...
      let escaped;
      if (escaped = this._accept(5, /* Backslash */ true)) {
        escaped = this._accept(5, /* Backslash */ true) ||
          this._accept(6, /* Forwardslash */ true) || escaped;
        transform.appendChild(new Text(escaped));
        continue;
      }
      if (
        this._parseFormatString(transform) || this._parseAnything(transform)
      ) {
        continue;
      }
      return false;
