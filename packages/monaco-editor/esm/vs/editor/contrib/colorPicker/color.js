...
export function getColors(model, token) {
  ...
  const promises = providers.map((provider) =>
    Promise.resolve(provider.provideDocumentColors(model, token)).then(
      (result) => {
...
CommandsRegistry.registerCommand(
  ...
  function (accessor, ...args) {
    ...
    const promises = providers.map((provider) =>
      Promise.resolve(
        provider.provideDocumentColors(model, CancellationToken.None),
      ).then((result) => {
        if (Array.isArray(result)) {
...
CommandsRegistry.registerCommand(
  ...
  function (accessor, ...args) {
    ...
    if (
      !(uri instanceof URI) || !Array.isArray(color) || color.length !== 4 ||
