...
let ColorDetector = class ColorDetector extends Disposable {
  constructor(_editor, _codeEditorService, _configurationService) {
    ...
    this._register(
      _editor.onDidChangeModelLanguage(() => this.onModelChanged()),
    );
    this._register(
      ColorProviderRegistry.onDidChange(() => this.onModelChanged()),
    );
  ...
  isEnabled() {
    ...
    const deprecatedConfig = this._configurationService.getValue(
      languageId.language,
    );
    if (deprecatedConfig) {
      const colorDecorators = deprecatedConfig["colorDecorators"]; // deprecatedConfig.valueOf('.colorDecorators.enable');
      if (
        colorDecorators && colorDecorators["enable"] !== undefined &&
  ...
  updateDecorations(colorDatas) {
    ...
    this._decorationsIds = this._editor.deltaDecorations(
      this._decorationsIds,
      decorations,
    );
    this._colorDatas = new Map();
  ...
  updateColorDecorators(colorData) {
    ...
    for (
      let i = 0;
      i < colorData.length && decorations.length < MAX_DECORATORS;
      i++
    ) {
      ...
      let subKey = hash(`rgba(${rgba.r},${rgba.g},${rgba.b},${rgba.a})`)
        .toString(16);
    ...
    this._colorDecoratorIds = this._editor.deltaDecorations(
      this._colorDecoratorIds,
      decorations,
    );
  }
