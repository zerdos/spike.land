  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const promises = flatten(
      providers.map((provider) =>
        ranges.map((range) =>
          Promise.resolve(provider.provideInlineHints(model, range, token))
            .then((result) => {
              if (result) {
...
let InlineHintsController = class InlineHintsController {
  constructor(_editor, _codeEditorService, _themeService) {
    ...
    this._getInlineHintsDelays = new LanguageFeatureRequestDelays(
      InlineHintsProviderRegistry,
      250,
      2500,
    );
    ...
    this._disposables.add(
      InlineHintsProviderRegistry.onDidChange(() => this._update()),
    );
    this._disposables.add(
      _themeService.onDidColorThemeChange(() => this._update()),
    );
  ...
  _update() {
    ...
    const scheduler = new RunOnceScheduler(
      () =>
        __awaiter(this, void 0, void 0, function* () {
          ...
          const visibleRanges = this._editor
            .getVisibleRangesPlusViewportAboveBelow();
          const result = yield getInlineHints(model, visibleRanges, cts.token);
          // update moving average
          const newDelay = this._getInlineHintsDelays.update(
            model,
            Date.now() - t1,
          );
    ...
    this._sessionDisposables.add(
      this._editor.onDidChangeModelContent(() => scheduler.schedule()),
    );
    this._disposables.add(
      this._editor.onDidScrollChange(() => scheduler.schedule()),
    );
    scheduler.schedule();
    ...
    for (const provider of InlineHintsProviderRegistry.all(model)) {
      if (typeof provider.onDidChangeInlineHints === "function") {
        providerListener.add(
          provider.onDidChangeInlineHints(() => scheduler.schedule()),
        );
      }
    }
  }
  _updateHintsDecorators(hintsData) {
    ...
    const backgroundColor = this._themeService.getColorTheme().getColor(
      editorInlineHintBackground,
    );
    const fontColor = this._themeService.getColorTheme().getColor(
      editorInlineHintForeground,
    );
    ...
    for (const { list: hints } of hintsData) {
      for (
        let j = 0;
        j < hints.length && newDecorationsData.length < MAX_DECORATORS;
        j++
      ) {
        const {
          text,
    ...
    this._decorationsTypeIds.forEach(
...
CommandsRegistry.registerCommand(
  "_executeInlineHintProvider",
  (accessor, ...args) =>
    __awaiter(void 0, void 0, void 0, function* () {
      const [uri, range] = args;
      assertType(URI.isUri(uri));
      assertType(Range.isIRange(range));
      const ref = yield accessor.get(ITextModelService).createModelReference(
        uri,
      );
      try {
        const data = yield getInlineHints(ref.object.textEditorModel, [
          Range.lift(range),
        ], CancellationToken.None);
        return flatten(data.map((item) => item.list)).sort((a, b) =>
