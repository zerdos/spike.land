...
export class LineCommentCommand {
  ...
  static _analyzeLines(
    ...
    for (let i = 0, lineCount = lines.length; i < lineCount; i++) {
      ...
      if (shouldRemoveComments && insertSpace) {
        ...
        const commentStrEndOffset = lineContentStartOffset +
  ...
  _attemptRemoveBlockComment(model, s, startToken, endToken) {
    ...
    let startTokenAllowedBeforeColumn = endToken.length +
      Math.max(
  ...
  _executeBlockComment(model, builder, s) {
    ...
    if (
      !config || !config.blockCommentStartToken || !config.blockCommentEndToken
    ) {
