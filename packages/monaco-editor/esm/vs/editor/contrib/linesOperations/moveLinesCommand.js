...
export class MoveLinesCommand {
  ...
  getEditOperations(model, builder) {
    ...
    if (
      this._isMovingDown && this._selection.endLineNumber === modelLineCount
    ) {
    ...
    let indentConverter = this.buildIndentConverter(
      tabSize,
      indentSize,
      insertSpaces,
    );
    ...
    if (
      ...
      let otherLineNumber = (this._isMovingDown
        ? lineNumber + 1
        : lineNumber - 1);
      ...
      if (this._isMovingDown) {
        ...
        if (this.shouldAutoIndent(model, s)) {
          ...
          if (movingLineMatchResult !== null) {
            let oldIndentation = strings.getLeadingWhitespace(
              model.getLineContent(movingLineNumber),
            );
            let newSpaceCnt = movingLineMatchResult +
              indentUtils.getSpaceCnt(oldIndentation, tabSize);
            let newIndentation = indentUtils.generateIndent(
              newSpaceCnt,
              tabSize,
              insertSpaces,
            );
            ...
            let indentOfMovingLine = LanguageConfigurationRegistry
            ...
            if (indentOfMovingLine !== null) {
              ...
              let newSpaceCnt = indentUtils.getSpaceCnt(
                indentOfMovingLine,
                tabSize,
              );
              let oldSpaceCnt = indentUtils.getSpaceCnt(
                oldIndentation,
                tabSize,
              );
          ...
          if (ret !== null) {
            ...
            let newIndentatOfMovingBlock = LanguageConfigurationRegistry
              .getGoodIndentForLine(
                this._autoIndent,
                virtualModel,
                model.getLanguageIdAtPosition(movingLineNumber, 1),
        ...
        if (this.shouldAutoIndent(model, s)) {
          ...
          if (ret !== null) {
            ...
            let indentOfFirstLine = LanguageConfigurationRegistry
              .getGoodIndentForLine(
                this._autoIndent,
                virtualModel,
            ...
            if (indentOfFirstLine !== null) {
              ...
              let oldIndent = strings.getLeadingWhitespace(
                model.getLineContent(s.startLineNumber),
              );
              let newSpaceCnt = indentUtils.getSpaceCnt(
                indentOfFirstLine,
  ...
  parseEnterResult(model, indentConverter, tabSize, line, enter) {
    if (enter) {
      ...
      if (enter.indentAction === IndentAction.None) {
        ...
        enterPrefix = indentConverter.unshiftIndent(enter.indentation) +
          enter.appendText;
      }
      let movingLineText = model.getLineContent(line);
      if (
        this.trimLeft(movingLineText).indexOf(this.trimLeft(enterPrefix)) >= 0
      ) {
        let oldIndentation = strings.getLeadingWhitespace(
          model.getLineContent(line),
        );
        let newIndentation = strings.getLeadingWhitespace(enterPrefix);
        let indentMetadataOfMovelingLine = LanguageConfigurationRegistry
          .getIndentMetadata(model, line);
        if (
          indentMetadataOfMovelingLine !== null &&
          indentMetadataOfMovelingLine & 2 /* DECREASE_MASK */
        ) {
  ...
  matchEnterRule(
    ...
    while (validPrecedingLine >= 1) {
      ...
      let lineContent;
      if (
        validPrecedingLine === oneLineAbove && previousLineText !== undefined
      ) {
  ...
  shouldAutoIndent(model, selection) {
    ...
    let languageAtSelectionStart = model.getLanguageIdAtPosition(
      selection.startLineNumber,
      1,
    );
    let languageAtSelectionEnd = model.getLanguageIdAtPosition(
    ...
    if (
      LanguageConfigurationRegistry.getIndentRulesSupport(
  ...
  getIndentEditsOfMovingBlock(
    ...
    for (let i = s.startLineNumber; i <= s.endLineNumber; i++) {
      ...
      if (newIndent !== originalIndent) {
        ...
        if (
          i === s.endLineNumber && s.endColumn <= originalIndent.length + 1 &&
          newIndent === ""
        ) {
