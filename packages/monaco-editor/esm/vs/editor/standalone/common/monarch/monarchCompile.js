      ...
      if (typeof (lexer[attr]) === "string") {
        ...
        if (lexer[attr] === undefined) {
          throw monarchCommon.createError(
            lexer,
            "language definition does not contain attribute '" + attr +
              "', used at: " + str,
          );
        } else {
          throw monarchCommon.createError(
            lexer,
            "attribute reference '" + attr + "' must be a string, used at: " +
              str,
          );
        }
      }
      return charBeforeAtSign +
        (monarchCommon.empty(sub) ? "" : "(?:" + sub + ")");
    });
    n++;
...
function createGuard(lexer, ruleName, tkey, val) {
  ...
  if ((op === "~" || op === "!~") && /^(\w|\|)*$/.test(pat)) {
    ...
    if (
      ...
      throw monarchCommon.createError(
        lexer,
        "the @ match target '" + pat +
          "' must be an array of strings, in rule: " + ruleName,
      );
    }
              ...
              if (
                ...
                throw monarchCommon.createError(
                  ...
                  "the next state '" + action.next +
                    "' is not defined in rule: " + ruleName,
                );
              }
            }
          }
...
export function compile(languageId, json) {
  ...
  for (let el of json.brackets) {
    ...
    if (
      typeof desc.open === "string" && typeof desc.token === "string" &&
