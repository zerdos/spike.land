...
import {
  ...
  TokenTheme,
} from "../../common/modes/supports/tokenization.js";
...
export class StandaloneThemeServiceImpl extends Disposable {
  ...
  _updateThemeOrColorMap() {
    ...
    themingRegistry.getThemingParticipants().forEach((p) =>
      p(this._theme, ruleCollector, this._environment)
    );
    const colorMap = this._colorMapOverride ||
      this._theme.tokenTheme.getColorMap();
  ...
  _updateCSS() {
    this._allCSS = `${this._codiconCSS}\n${this._themeCSS}`;
    this._styleElements.forEach((styleElement) =>
      styleElement.textContent = this._allCSS
    );
  }
  getFileIconTheme() {
