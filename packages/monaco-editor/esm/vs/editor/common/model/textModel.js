...
import { countEOL, TokensStore, TokensStore2 } from "./tokensStore.js";
...
export class TextModel extends Disposable {
  ...
  constructor(
    ...
    if (
      typeof associatedResource === "undefined" || associatedResource === null
    ) {
  ...
  onDidChangeRawContentFast(listener) {
    return this._eventEmitter.fastEvent((e) =>
  ...
  updateOptions(_newOpts) {
    ...
    let indentSize = (typeof _newOpts.indentSize !== "undefined")
      ? _newOpts.indentSize
  ...
  static normalizeIndentation(str, indentSize, insertSpaces) {
    ...
    return TextModel._normalizeIndentationFromWhitespace(
  ...
  findMatches(
    ...
    searchRanges = searchRanges.sort((d1, d2) =>
      d1.startLineNumber - d2.startLineNumber || d1.startColumn - d2.startColumn
    );
        ...
        for (
          ...
          for (let j = 0, lenJ = incomingEdits.length; j < lenJ; j++) {
            ...
            if (
              ...
              editRange.isEmpty() && editText && editText.length > 0 &&
              editText.charAt(0) === "\n"
            ) {
    ...
    if (contentChanges.length !== 0) {
      ...
      for (let i = 0, len = contentChanges.length; i < len; i++) {
        ...
        for (let j = editingLinesCnt; j >= 0; j--) {
          ...
          const currentEditLineNumber = newLineCount - lineCount -
            changeLineCountDelta + editLineNumber;
        ...
        if (editingLinesCnt < insertingLinesCnt) {
          ...
          const fromLineNumber = newLineCount - lineCount - cnt +
            spliceLineNumber + 1;
          let newLines = [];
          for (let i = 0; i < cnt; i++) {
            let lineNumber = fromLineNumber + i;
            newLines[lineNumber - fromLineNumber] = this.getLineContent(
              lineNumber,
            );
          }
          rawContentChanges.push(
  ...
  getWordAtPosition(_position) {
    ...
    if (
      rightBiasedWord && rightBiasedWord.startColumn <= _position.column &&
    ...
    if (tokenIndex > 0 && rbStartOffset === position.column - 1) {
      ...
      // Make sure the result touches the original passed in position
      if (
        leftBiasedWord && leftBiasedWord.startColumn <= _position.column &&
  ...
  _matchBracket(position) {
    ...
    const currentModeBrackets = LanguageConfigurationRegistry
      .getBracketsSupport(lineTokens.getLanguageId(tokenIndex));
    ...
    if (
      ...
      if (
        ...
        // check that we didn't hit a bracket too far away from position
        if (
          foundBracket && foundBracket.startColumn <= position.column &&
          position.column <= foundBracket.endColumn
        ) {
          const foundBracketText = lineText.substring(
            foundBracket.startColumn - 1,
  ...
  _findMatchingBracketUp(bracket, position, continueSearchPredicate) {
    ...
    const searchPrevMatchingBracketInRange = (
      ...
      while (true) {
        if (
          continueSearchPredicate && (++totalCallCount) % 100 === 0 &&
          !continueSearchPredicate()
        ) {
        ...
        const hitText = lineText.substring(r.startColumn - 1, r.endColumn - 1)
          .toLowerCase();
    ...
    for (let lineNumber = position.lineNumber; lineNumber >= 1; lineNumber--) {
      ...
      for (; tokenIndex >= 0; tokenIndex--) {
        ...
        if (languageId !== tokenLanguageId) {
          // language id change!
          if (
            modeBrackets && prevSearchInToken &&
  ...
  getActiveIndentGuide(lineNumber, minLineNumber, maxLineNumber) {
    ...
    for (let distance = 0; goUp || goDown; distance++) {
      ...
      if (distance === 1) {
        if (
          downLineNumber <= lineCount && downLineIndentLevel >= 0 &&
        ...
        if (
          upLineNumber >= 1 && upLineIndentLevel >= 0 &&
  ...
  _getIndentLevelForWhitespaceLine(
    ...
    if (aboveContentLineIndent === -1 || belowContentLineIndent === -1) {
      ...
      return (1 +
        Math.floor(aboveContentLineIndent / this._options.indentSize));
    } else if (aboveContentLineIndent === belowContentLineIndent) {
      ...
      if (offSide) {
        ...
        return (1 +
          Math.floor(belowContentLineIndent / this._options.indentSize));
      }
    }
  }
}
...
export class DidChangeDecorationsEmitter extends Disposable {
  ...
  checkAffectedAndFire(options) {
    if (!this._affectsMinimap) {
      this._affectsMinimap = options.minimap && options.minimap.position
        ? true
        : false;
    }
