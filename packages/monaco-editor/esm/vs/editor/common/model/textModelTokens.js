...
import { countEOL, MultilineTokensBuilder } from "./tokensStore.js";
...
export class TextModelTokenization extends Disposable {
  ...
  isCheapToTokenize(lineNumber) {
    ...
    if (
      this._textModel.getLineLength(lineNumber) <
        2048 /* CHEAP_TOKENIZATION_LENGTH_LIMIT */
    ) {
      return true;
    }
    return false;
  ...
  _hasLinesToTokenize() {
    ...
    return (this._tokenizationStateStore.invalidLineStartIndex <
  ...
  _tokenizeViewport(builder, startLineNumber, endLineNumber) {
    ...
    let nonWhitespaceColumn = this._textModel.getLineFirstNonWhitespaceColumn(
      startLineNumber,
    );
    let fakeLines = [];
    let initialState = null;
    for (let i = startLineNumber - 1; nonWhitespaceColumn > 0 && i >= 1; i--) {
      let newNonWhitespaceIndex = this._textModel
        .getLineFirstNonWhitespaceColumn(i);
