...
function spacesDiff(a, aLength, b, bLength, result) {
  ...
  if (tabsDiff === 0) {
    ...
    if (
      spacesDiff > 0 && 0 <= bSpacesCnt - 1 && bSpacesCnt - 1 < a.length &&
      bSpacesCnt < b.length
    ) {
      if (
        b.charCodeAt(bSpacesCnt) !== 32 /* Space */ &&
        a.charCodeAt(bSpacesCnt - 1) === 32 /* Space */
      ) {
...
export function guessIndentation(source, defaultTabSize, defaultInsertSpaces) {
  ...
  if (insertSpaces) {
    ...
    if (
      tabSize === 4 && spacesDiffCount[4] > 0 && spacesDiffCount[2] > 0 &&
