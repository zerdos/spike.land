...
function getCollapseOnReplaceEdit(node) {
  return ((node.metadata & 64 /* CollapseOnReplaceEditMask */) >>>
    6 /* CollapseOnReplaceEditOffset */) === 1;
}
...
export function nodeAcceptEdit(node, start, end, textLength, forceMoveMarkers) {
  ...
  const startStickToPreviousCharacter =
    (nodeStickiness === 0 || /* AlwaysGrowsWhenTypingAtEdges */
      nodeStickiness === 2 /* GrowsOnlyWhenTypingBefore */);
  const endStickToPreviousCharacter =
    (nodeStickiness === 1 || /* NeverGrowsWhenTypingAtEdges */
      nodeStickiness === 2 /* GrowsOnlyWhenTypingBefore */);
  ...
  }
  {
    const moveSemantics = forceMoveMarkers
      ? 1 /* ForceMove */
      : (deletingCnt > 0 ? 2 /* ForceStay */ : 0 /* MarkerDefined */);
    if (
      !startDone &&
...
function noOverlapReplace(T, start, end, textLength) {
  ...
  while (node !== SENTINEL) {
    ...
    if (nodeStart > end) {
      ...
      if (
        node.delta < -1073741824 /* MIN_SAFE_DELTA */ ||
        node.delta > 1073741824 /* MAX_SAFE_DELTA */
      ) {
...
function rbTreeDelete(T, z) {
  ...
  if (z.left === SENTINEL) {
    ...
    x.delta += z.delta;
    if (
      x.delta < -1073741824 /* MIN_SAFE_DELTA */ ||
      x.delta > 1073741824 /* MAX_SAFE_DELTA */
    ) {
      T.requestNormalizeDelta = true;
    }
    x.start += z.delta;
    ...
    y.delta = z.delta;
    if (
      y.delta < -1073741824 /* MIN_SAFE_DELTA */ ||
      y.delta > 1073741824 /* MAX_SAFE_DELTA */
    ) {
      T.requestNormalizeDelta = true;
    }
  }
  ...
  while (x !== T.root && getNodeColor(x) === 0 /* Black */) {
    if (x === x.parent.left) {
      ...
      if (
        getNodeColor(w.left) === 0 /* Black */ &&
        getNodeColor(w.right) === 0 /* Black */
      ) {
