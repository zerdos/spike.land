...
export class CharacterMapping {
  ...
  static getPartIndex(partData) {
    return (partData & 4294901760 /* PART_INDEX_MASK */) >>>
      16 /* PART_INDEX_OFFSET */;
  }
  static getCharIndex(partData) {
    return (partData & 65535 /* CHAR_INDEX_MASK */) >>>
      0 /* CHAR_INDEX_OFFSET */;
  }
    ...
    if (input.lineDecorations.length > 0) {
      ...
      if (containsForeignElements !== 0 /* None */) {
        const beforeSpan = (beforeClassNames.length > 0
          ? `<span class="${beforeClassNames.join(" ")}"></span>`
          : ``);
        const afterSpan = (afterClassNames.length > 0
          ? `<span class="${afterClassNames.join(" ")}"></span>`
          : ``);
...
function resolveRenderLineInput(input) {
  ...
  if (
    ...
    (input.renderWhitespace === 2 /* Selection */ &&
    ...
    for (let i = 0, len = tokens.length; i < len; i++) {
      ...
      if (lastTokenEndIndex + 50 /* LongToken */ < tokenEndIndex) {
        ...
        for (let j = lastTokenEndIndex; j < tokenEndIndex; j++) {
          ...
          if (
            lastSpaceOffset !== -1 && j - currTokenStart >= 50 /* LongToken */
          ) {
...
function _applyRenderWhitespace(input, lineContent, len, tokens) {
  ...
  for (let charIndex = fauxIndentLength; charIndex < len; charIndex++) {
    ...
    let isInWhitespace;
    if (
      charIndex < firstNonWhitespaceIndex || charIndex > lastNonWhitespaceIndex
    ) {
      ...
      if (onlyBoundary) {
        ...
        if (wasInWhitespace) {
          ...
          const nextChCode = (charIndex + 1 < len
            ...
            : 0 /* Null */);
          isInWhitespace =
            (nextChCode === 32 /* Space */ || nextChCode === 9 /* Tab */);
        }
      } else {
        isInWhitespace = true;
      }
...
function _renderLine(input, sb) {
  ...
  for (
    ...
    const partRendersWhitespace =
      (renderWhitespace !== 0 /* None */ && part.isWhitespace());
    ...
    const partIsEmptyAndHasPseudoAfter =
      (charIndex === partEndIndex && part.metadata === 4 /* PSEUDO_AFTER */);
    charOffsetInPart = 0;
    ...
    if (partRendersWhitespace) {
      let partContentCnt = 0;
      {
        ...
        for (; _charIndex < partEndIndex; _charIndex++) {
          ...
          const charWidth = (charCode === 9 /* Tab */
