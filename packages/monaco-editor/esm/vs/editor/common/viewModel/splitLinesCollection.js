...
export class SplitLinesCollection {
  ...
  _constructLines(resetHiddenAreas, previousLineBreaks) {
    ...
    let values = [];
    let hiddenAreas = this.hiddenAreasIds.map((areaId) =>
      this.model.getDecorationRange(areaId)
  ...
  _reduceRanges(_ranges) {
    ...
    let ranges = _ranges.map((r) => this.model.validateRange(r)).sort(
  ...
  setHiddenAreas(_ranges) {
    ...
    let oldRanges = this.hiddenAreasIds.map((areaId) =>
      this.model.getDecorationRange(areaId)
    ).sort(Range.compareRangesUsingStarts);
  ...
  setWrappingSettings(
    ...
    if (
      equalFontInfo && equalWrappingStrategy && equalWrappingColumn &&
      equalWrappingIndent
    ) {
      return false;
    }
  ...
  onModelLinesInserted(versionId, fromLineNumber, _toLineNumber, lineBreaks) {
    ...
    this.lines = this.lines.slice(0, fromLineNumber - 1).concat(insertLines)
...
export class SplitLine {
  ...
  getViewLineMaxColumn(model, modelLineNumber, outputLineIndex) {
    ...
    return this.getViewLineContent(model, modelLineNumber, outputLineIndex)
      .length + 1;
  }
  getViewLineData(model, modelLineNumber, outputLineIndex) {
    ...
    if (outputLineIndex > 0) {
      lineContent = spaces(this._lineBreakData.wrappedTextIndentLength) +
        lineContent;
    }
    let minColumn =
      (outputLineIndex > 0
        ? this._lineBreakData.wrappedTextIndentLength + 1
        : 1);
