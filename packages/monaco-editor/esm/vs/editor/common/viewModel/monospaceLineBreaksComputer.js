...
export class MonospaceLineBreaksComputerFactory {
  ...
  static create(options) {
    return new MonospaceLineBreaksComputerFactory(
      options.get(113 /* wordWrapBreakBeforeCharacters */),
      options.get(112 /* wordWrapBreakAfterCharacters */),
    );
  }
...
function createLineBreaksFromPreviousLineBreaks(
  ...
  while (prevIndex < prevLen) {
    ...
    let prevBreakOffsetVisibleColumn = prevIndex < 0
      ? 0
    ...
    if (breakOffset <= lastBreakingOffset) {
      ...
      if (strings.isHighSurrogate(charCode)) {
        ...
        breakOffsetVisibleColumn = lastBreakingOffsetVisibleColumn +
          computeCharWidth(
            charCode,
...
function canBreak(prevCharCode, prevCharCodeClass, charCode, charCodeClass) {
  return (charCode !== 32 && /* Space */
    ((prevCharCodeClass === 2 /* BREAK_AFTER */) ||
      (prevCharCodeClass === 3 /* BREAK_IDEOGRAPHIC */ &&
        charCodeClass !== 2 /* BREAK_AFTER */) ||
      (charCodeClass === 1 /* BREAK_BEFORE */) ||
      (charCodeClass === 3 /* BREAK_IDEOGRAPHIC */ &&
        prevCharCodeClass !== 1 /* BREAK_BEFORE */)));
}
  ...
  if (wrappingIndent !== 0 /* None */) {
    ...
    if (firstNonWhitespaceIndex !== -1) {
      ...
      for (let i = 0; i < firstNonWhitespaceIndex; i++) {
        const charWidth = (lineText.charCodeAt(i) === 9 /* Tab */
          ? tabCharacterWidth(wrappedTextIndentLength, tabSize)
          : 1);
        wrappedTextIndentLength += charWidth;
      }
      // Increase indent of continuation lines, if desired
      const numberOfAdditionalTabs = (wrappingIndent === 3 /* DeepIndent */
        ? 2
        : wrappingIndent === 2 /* Indent */
        ? 1
        : 0);
      ...
      if (
        wrappedTextIndentLength + columnsForFullWidthChar > firstLineBreakColumn
      ) {
