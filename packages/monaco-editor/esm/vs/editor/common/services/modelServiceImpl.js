...
import {
  ...
  toMultilineTokens2,
} from "./semanticTokensProviderStyling.js";
import {
  getDocumentSemanticTokens,
  isSemanticTokens,
  isSemanticTokensEdits,
} from "./getSemanticTokens.js";
...
let ModelServiceImpl = class ModelServiceImpl extends Disposable {
  ...
  static _readModelOptions(config, isForSimpleWidget) {
    ...
    if (
      config.editor && typeof config.editor.indentSize !== "undefined" &&
    ...
    if (
      config.editor && typeof config.editor.trimAutoWhitespace !== "undefined"
    ) {
    ...
    if (
      ...
      largeFileOptimizations = (config.editor.largeFileOptimizations === "false"
        ? false
  ...
  static _setModelOptionsForModel(model, newOptions, currentOptions) {
    if (
      currentOptions && currentOptions.defaultEOL !== newOptions.defaultEOL &&
      model.getLineCount() === 1
    ) {
      model.setEOL(
        newOptions.defaultEOL === 1 /* LF */ ? 0 /* LF */ : 1, /* CRLF */
      );
    }
    if (
      currentOptions &&
  ...
  _onWillDispose(model) {
    ...
    if (!maintainUndoRedoStack) {
      if (!sharesUndoRedoStack) {
        const initialUndoRedoSnapshot = modelData.model
...
export class ModelSemanticColoring extends Disposable {
  constructor(model, themeService, stylingProvider) {
    ...
    const bindDocumentChangeListeners = () => {
      ...
      this._documentProvidersChangeListeners = [];
      for (
        const provider of DocumentSemanticTokensProviderRegistry.all(model)
      ) {
