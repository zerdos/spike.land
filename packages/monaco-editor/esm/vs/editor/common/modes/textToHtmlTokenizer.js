...
const fallback = {
  ...
  tokenize2: (buffer, hasEOL, state, deltaOffset) =>
    nullTokenize2(0, /* Null */ buffer, state, deltaOffset),
};
...
export function tokenizeLineToHTML(
  ...
  for (
    ...
    result += `<span style="${
      viewLineTokens.getInlineStyle(tokenIndex, colorMap)
    }">${partContent}</span>`;
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    for (let j = 0, lenJ = viewLineTokens.getCount(); j < lenJ; j++) {
      ...
      result += `<span class="${type}">${
        strings.escape(line.substring(startOffset, endIndex))
      }</span>`;
