...
} from "../model/wordHelper.js";
import { AutoClosingPairs, IndentAction } from "./languageConfiguration.js";
...
export class RichEditSupport {
  constructor(languageIdentifier, rawConf) {
    ...
    this._onEnterSupport =
      (this._conf.brackets || this._conf.indentationRules ||
...
class LanguageConfigurationEntries {
  ...
  _resolve() {
    ...
    for (const entry of this._entries) {
      ...
      result.indentationRules = conf.indentationRules ||
        result.indentationRules;
      result.onEnterRules = conf.onEnterRules || result.onEnterRules;
      result.autoClosingPairs = conf.autoClosingPairs ||
        result.autoClosingPairs;
      result.surroundingPairs = conf.surroundingPairs ||
      ...
      result.__electricCharacterSupport = conf.__electricCharacterSupport ||
...
export class LanguageConfigurationRegistryImpl {
  ...
  getPrecedingValidLine(model, lineNumber, indentRulesSupport) {
    ...
    if (lineNumber > 1) {
      ...
      for (
        ...
        if (
          indentRulesSupport.shouldIgnore(text) || /^\s+$/.test(text) ||
          text === ""
        ) {
    ...
    if (
      ...
      if (
        !(previousLineIndentMetadata &
          (1 /* INCREASE_MASK */ | 2 /* DECREASE_MASK */)) &&
  ...
  getIndentForEnter(autoIndent, model, range, indentConverter) {
    ...
    if (
      ...
      beforeEnterText = lineTokens.getLineContent().substring(
        0,
        range.startColumn - 1,
      );
    }
    ...
    if (range.isEmpty()) {
      ...
      afterEnterText = endScopedLineTokens.getLineContent().substr(
        range.endColumn - 1 - scopedLineTokens.firstCharOffset,
      );
    }
    const indentRulesSupport = this.getIndentRulesSupport(
      scopedLineTokens.languageId,
  ...
  getIndentActionForType(autoIndent, model, range, ch, indentConverter) {
    ...
    if (range.isEmpty()) {
      ...
      afterTypeText = endScopedLineTokens.getLineContent().substr(
        range.endColumn - 1 - scopedLineTokens.firstCharOffset,
      );
    }
