...
export class CursorMoveCommands {
  ...
  static line(viewModel, cursor, inSelectionMode, _position, _viewPosition) {
    ...
    if (position.lineNumber < enteringLineNumber) {
      ...
      const endPositionOfSelectionStart = cursor.modelState.selectionStart
        .getEndPosition();
      return CursorState.fromModelState(
      ...
      case 12 /* ViewPortCenter */: {
        ...
        const modelLineNumber = Math.round(
          (visibleModelRange.startLineNumber +
            visibleModelRange.endLineNumber) / 2,
        );
        const modelColumn = viewModel.model.getLineFirstNonWhitespaceColumn(
          modelLineNumber,
        );
  ...
  static _moveLeft(viewModel, cursors, inSelectionMode, noOfColumns) {
    ...
    for (let i = 0, len = cursors.length; i < len; i++) {
      const cursor = cursors[i];
      const skipWrappingPointStop = hasMultipleCursors ||
        !cursor.viewState.hasSelection();
      ...
      if (
        ...
        cursor.viewState.position.column ===
          viewModel.getLineMinColumn(cursor.viewState.position.lineNumber) &&
        newViewState.position.lineNumber !==
          cursor.viewState.position.lineNumber
      ) {
        // moved over to the previous view line
        const newViewModelPosition = viewModel.coordinatesConverter
          .convertViewPositionToModelPosition(newViewState.position);
        if (
          newViewModelPosition.lineNumber ===
          ...
          newViewState = MoveOperations.moveLeft(
            viewModel.cursorConfig,
            viewModel,
            newViewState,
            inSelectionMode,
  ...
  static _moveRight(viewModel, cursors, inSelectionMode, noOfColumns) {
    ...
    for (let i = 0, len = cursors.length; i < len; i++) {
      ...
      if (
        ...
        noOfColumns === 1 &&
        cursor.viewState.position.column ===
          viewModel.getLineMaxColumn(cursor.viewState.position.lineNumber) &&
        newViewState.position.lineNumber !==
          cursor.viewState.position.lineNumber
      ) {
        ...
        if (
          ...
          // stayed on the same model line => pass wrapping point where 2 view positions map to a single model position
          newViewState = MoveOperations.moveRight(
            viewModel.cursorConfig,
            viewModel,
            newViewState,
            inSelectionMode,
    ...
    for (let i = 0, len = cursors.length; i < len; i++) {
      ...
      const viewColumn = Math.round(
        (viewModel.getLineMaxColumn(viewLineNumber) +
          viewModel.getLineMinColumn(viewLineNumber)) / 2,
      );
      result[i] = this._moveToViewPosition(
        viewModel,
        cursor,
...
(function (CursorMove) {
  const isCursorMoveArgs = function (arg) {
    ...
    if (
      !types.isUndefined(cursorMoveArg.by) && !types.isString(cursorMoveArg.by)
    ) {
      return false;
    }
    if (
