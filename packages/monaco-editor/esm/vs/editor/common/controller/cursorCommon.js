...
export class CursorConfiguration {
  ...
  static _getLanguageDefinedShouldAutoClose(languageIdentifier) {
    try {
      const autoCloseBeforeSet = LanguageConfigurationRegistry
        .getAutoCloseBeforeSet(languageIdentifier.id);
...
export class CursorColumns {
  static visibleColumnFromColumn(lineContent, column, tabSize) {
    ...
    const endOffset = column - 1 < lineContentLength
      ? column - 1
      : lineContentLength;
    let result = 0;
    let i = 0;
    while (i < endOffset) {
      ...
      if (codePoint === 9 /* Tab */) {
        ...
        while (i < endOffset) {
          ...
          const nextGraphemeBreakType = strings.getGraphemeBreakType(
            nextCodePoint,
          );
          if (
          ...
          i += (nextCodePoint >= 65536 /* UNICODE_SUPPLEMENTARY_PLANE_BEGIN */
            ? 2
            : 1);
