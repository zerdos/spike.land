...
export class WordOperations {
  ...
  static _moveWordPartLeft(model, position) {
    ...
    for (let column = position.column - 1; column > 1; column--) {
      ...
      if (
        strings.isLowerAsciiLetter(left) && strings.isUpperAsciiLetter(right)
      ) {
        // camelCaseVariables
        return new Position(lineNumber, column);
      }
      if (
        strings.isUpperAsciiLetter(left) && strings.isUpperAsciiLetter(right)
      ) {
    ...
    if (wordNavigationType === 2 /* WordEnd */) {
      if (nextWordOnLine && nextWordOnLine.wordType === 2 /* Separator */) {
        if (
          ...
          nextWordOnLine.nextCharClass === 0 /* Regular */
        ) {
          // Skip over a word made up of one single separator and followed by a regular character
  ...
  static _deleteInsideWordWhitespace(model, position) {
    ...
    while (
      leftIndex > 0 && this._charAtIsWhitespace(lineContent, leftIndex - 1)
    ) {
      leftIndex--;
    }
  ...
  static getWordAtPosition(model, _wordSeparators, position) {
    ...
    if (
      prevWord && prevWord.wordType === 1 /* Regular */ &&
    ...
    if (
      nextWord && nextWord.wordType === 1 /* Regular */ &&
