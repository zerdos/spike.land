...
export class MouseHandler extends ViewEventHandler {
      ...
      const doMouseWheelZoom =
        (platform.isMacintosh
          ? (browserEvent.metaKey && !browserEvent.ctrlKey &&
            !browserEvent.shiftKey && !browserEvent.altKey)
  ...
  getTargetAtClientPoint(clientX, clientY) {
    ...
    if (
      pos.y < editorPos.y || pos.y > editorPos.y + editorPos.height ||
      pos.x < editorPos.x || pos.x > editorPos.x + editorPos.width
    ) {
      return null;
    }
...
class MouseDownOperation extends Disposable {
  ...
  start(targetType, e) {
    ...
    this._mouseState.setStartedOnLineNumbers(
      targetType === 3, /* GUTTER_LINE_NUMBERS */
    );
  ...
  _getPositionOutsideEditor(e) {
    ...
    if (e.posy > editorContent.y + editorContent.height) {
      const verticalOffset = viewLayout.getCurrentScrollTop() +
        (e.posy - editorContent.y);
      const viewZoneData = HitTestContext.getZoneAtCoord(
        this._context,
        verticalOffset,
      );
