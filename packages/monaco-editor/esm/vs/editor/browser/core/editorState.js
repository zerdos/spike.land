...
export class EditorState {
  ...
  _equals(other) {
    ...
    if (
      this.scrollLeft !== state.scrollLeft || this.scrollTop !== state.scrollTop
    ) {
      return false;
    }
    if (
      !this.position && state.position || this.position && !state.position ||
  ...
  static capture(editor) {
    ...
    if (editor.getScrollTop() !== 0) {
      ...
      if (visibleRanges.length > 0) {
        ...
        visiblePositionScrollDelta = editor.getScrollTop() -
