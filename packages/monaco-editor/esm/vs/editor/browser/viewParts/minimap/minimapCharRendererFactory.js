...
export class MinimapCharRendererFactory {
  ...
  static create(scale, fontFamily) {
    ...
    if (
      this.lastCreated && scale === this.lastCreated.scale &&
  ...
  static createSampleData(fontFamily) {
    ...
    canvas.style.width = 96 /* CHAR_COUNT */ * 10 /* SAMPLED_CHAR_WIDTH */ +
      "px";
    ...
    return ctx.getImageData(
      0,
      0,
      96 /* CHAR_COUNT */ * 10, /* SAMPLED_CHAR_WIDTH */
      16, /* SAMPLED_CHAR_HEIGHT */
    );
  }
  /**
  ...
  static createFromSampleData(source, scale) {
    const expectedLength = 16 /* SAMPLED_CHAR_HEIGHT */ *
      10 /* SAMPLED_CHAR_WIDTH */ * 4 /* RGBA_CHANNELS_CNT */ *
      96 /* CHAR_COUNT */;
      ...
      for (let x = 0; x < width; x++) {
        ...
        for (let sy = sourceY1; sy < sourceY2; sy++) {
          const sourceRow = sourceOffset +
            Math.floor(sy) * 3840 /* RGBA_SAMPLED_ROW_WIDTH */;
          const yBalance = 1 - (sy - Math.floor(sy));
          for (let sx = sourceX1; sx < sourceX2; sx++) {
            const xBalance = 1 - (sx - Math.floor(sx));
            const sourceIndex = sourceRow +
              Math.floor(sx) * 4 /* RGBA_CHANNELS_CNT */;
            const weight = xBalance * yBalance;
            samples += weight;
            value += ((source[sourceIndex] * source[sourceIndex + 3]) / 255) *
              weight;
          }
        }
  ...
  static _downsample(data, scale) {
    const pixelsPerCharacter = 2 /* BASE_CHAR_HEIGHT */ * scale *
      1 /* BASE_CHAR_WIDTH */ * scale;
