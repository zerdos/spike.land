...
export class SelectionsOverlay extends DynamicViewOverlay {
  ...
  _enrichVisibleRangesWithStyle(viewport, linesVisibleRanges, previousFrame) {
    ...
    if (
      previousFrame && previousFrame.length > 0 && linesVisibleRanges.length > 0
    ) {
  ...
  _getVisibleRangesWithStyle(selection, ctx, previousFrame) {
    ...
    if (
      !isIEWithZoomingIssuesNearRoundedBorders && !visibleRangesHaveGaps &&
  ...
  prepareRender(ctx) {
    ...
    this._renderResult = output.map(([internalCorners, restOfSelection]) =>
