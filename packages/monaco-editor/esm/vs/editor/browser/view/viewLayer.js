...
export class RenderedLinesCollection {
  ...
  onLinesDeleted(deleteFromLineNumber, deleteToLineNumber) {
    ...
    for (
      ...
      const lineIndex = lineNumber - this._rendLineNumberStart;
      if (
        deleteFromLineNumber <= lineNumber && lineNumber <= deleteToLineNumber
      ) {
  ...
  onLinesChanged(changeFromLineNumber, changeToLineNumber) {
    ...
    for (
      ...
      if (
        ...
        // Notify the line
        this._lines[changedLineNumber - this._rendLineNumberStart]
          .onContentChanged();
  ...
  onTokensChanged(ranges) {
    ...
    for (let i = 0, len = ranges.length; i < len; i++) {
      const rng = ranges[i];
      if (
        rng.toLineNumber < startLineNumber || rng.fromLineNumber > endLineNumber
      ) {
