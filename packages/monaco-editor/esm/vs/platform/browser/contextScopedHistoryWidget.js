...
function getContextScopedWidget(contextKeyService, contextKey) {
  return contextKeyService.getContext(document.activeElement).getValue(
    contextKey,
  );
}
export function createAndBindHistoryNavigationWidgetScopedContextKeyService(
  ...
  const historyNavigationEnablement = new RawContextKey(
  ...
  constructor(
    ...
    this._register(
      createAndBindHistoryNavigationWidgetScopedContextKeyService(
        contextKeyService,
        { target: this.inputBox.element, historyNavigator: this.inputBox },
      ).scopedContextKeyService,
    );
  }
};
...
let ContextScopedReplaceInput = class ContextScopedReplaceInput
  extends ReplaceInput {
  constructor(
    container,
    contextViewProvider,
    options,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
  ...
  handler: (accessor, arg2) => {
    const widget = getContextScopedWidget(
