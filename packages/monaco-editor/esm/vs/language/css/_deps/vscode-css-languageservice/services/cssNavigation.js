...
var CSSNavigation = /** @class */ (function () {
  ...
  CSSNavigation.prototype.findDocumentHighlights = function (
    ...
    var node = nodes.getNodeAtOffset(stylesheet, offset);
    if (
      !node || node.type === nodes.NodeType.Stylesheet ||
      node.type === nodes.NodeType.Declarations
    ) {
      return result;
    }
    if (
      node.type === nodes.NodeType.Identifier && node.parent &&
    ...
    stylesheet.accept(function (candidate) {
      if (symbol) {
        ...
        node && node.type === candidate.type && candidate.matches(name)
      ) {
  ...
  CSSNavigation.prototype.findUnresolvedLinks = function (
    document,
    stylesheet,
  ) {
    var _this = this;
    ...
    stylesheet.accept(function (candidate) {
      ...
      /**
             ...
             */
      if (
        candidate.parent && _this.isRawStringDocumentLinkNode(candidate.parent)
      ) {
  ...
  CSSNavigation.prototype.getColorPresentations = function (
    ...
    if (color.alpha === 1) {
      ...
      label = "rgba(" + red256 + ", " + green256 + ", " + blue256 + ", " +
        color.alpha + ")";
    }
    ...
    if (hsl.a === 1) {
      label = "hsl(" + hsl.h + ", " + Math.round(hsl.s * 100) + "%, " +
        Math.round(hsl.l * 100) + "%)";
    } else {
      label = "hsla(" + hsl.h + ", " + Math.round(hsl.s * 100) + "%, " +
  ...
  CSSNavigation.prototype.resolveRelativeReference = function (
    ref,
    documentUri,
    documentContext,
  ) {
    return __awaiter(this, void 0, void 0, function () {
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            if (
              !(ref[0] === "~" && ref[1] !== "/" && this.fileSystemProvider)
            ) {
              return [3, /*break*/ 3];
            }
            ref = ref.substring(1);
          ...
          case 2:
            return [
              2, /*return*/
              documentContext.resolveReference(ref, documentUri),
            ];
          case 3:
            return [
              2, /*return*/
  ...
  CSSNavigation.prototype.resolvePathToModule = function (
    _moduleName,
    documentFolderUri,
    rootFolderUri,
  ) {
        ...
        switch (_a.label) {
          ...
          case 1:
            if (_a.sent()) {
              ...
              rootFolderUri && documentFolderUri.startsWith(rootFolderUri) &&
