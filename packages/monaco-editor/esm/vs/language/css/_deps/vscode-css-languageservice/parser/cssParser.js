...
var Parser = /** @class */ (function () {
  ...
  Parser.prototype.resync = function (resyncTokens, resyncStopTokens) {
    while (true) {
      if (resyncTokens && resyncTokens.indexOf(this.token.type) !== -1) {
        ...
        resyncStopTokens && resyncStopTokens.indexOf(this.token.type) !== -1
      ) {
        return true;
      } else {
  ...
  Parser.prototype.markError = function (
    node,
    error,
    resyncTokens,
    resyncStopTokens,
    ...
    do {
      ...
      do {
        ...
        if (statement) {
          ...
          inRecovery = false;
          if (
            !this.peek(TokenType.EOF) && this._needsSemicolonAfter(statement) &&
        ...
        while (
          this.accept(TokenType.SemiColon) || this.accept(TokenType.CDO) ||
  ...
  Parser.prototype._parseDeclarations = function (parseDeclaration) {
    ...
    while (node.addChild(decl)) {
      ...
      if (
        this._needsSemicolonAfter(decl) && !this.accept(TokenType.SemiColon)
      ) {
      ...
      if (
        decl && this.prevToken && this.prevToken.type === TokenType.SemiColon
      ) {
  ...
  Parser.prototype._tryParseCustomPropertyDeclaration = function (stopTokens) {
    ...
    if (this.peek(TokenType.CurlyL)) {
      ...
      var declarations = this._parseDeclarations(
  ...
  Parser.prototype._parseKeyframeSelector = function () {
    ...
    if (
      !node.addChild(this._parseIdent()) && !this.accept(TokenType.Percentage)
    ) {
      return null;
    }
    while (this.accept(TokenType.Comma)) {
  ...
  Parser.prototype._parseCombinator = function () {
    if (this.peekDelim(">")) {
      ...
      var mark = this.mark();
      if (
        !this.hasWhitespace() && this.acceptIdent("deep") &&
  ...
  Parser.prototype._parseSimpleSelectorBody = function () {
    return this._parsePseudo() || this._parseHash() || this._parseClass() ||
      this._parseAttrib();
  };
    ...
    if (node) {
      if (!this.hasWhitespace() && this.accept(TokenType.ParenthesisL)) {
        var tryAsSelector = function () {
          ...
          while (
            _this.accept(TokenType.Comma) &&
            selectors.addChild(_this._parseSelector(false))
          ) {
            // loop
          }
  ...
  Parser.prototype._parseURLArgument = function () {
    var node = this.create(nodes.Node);
    if (
      !this.accept(TokenType.String) && !this.accept(TokenType.BadString) &&
