          ...
          if (expression && operator) {
            switch (unescape(operator.getText())) {
              ...
              case "$=":
                // suffix
                value = "\u2026" +
                  quotes.remove(unescape(expression.getText()));
                break;
              case "~=":
...
var SelectorPrinting = /** @class */ (function () {
  ...
  SelectorPrinting.prototype.selectorToSpecificityMarkedString = function (
    node,
  ) {
    var _this = this;
      ...
      if (
        selector.getChildren().some(function (c) {
          return c.hasChildren() &&
            c.getChild(0).type === nodes.NodeType.SelectorCombinator;
        })
      ) {
    ...
    for (var _i = 0, _a = selector.getChildren(); _i < _a.length; _i++) {
      ...
      if (selectorChild instanceof nodes.SimpleSelector) {
        if (this.prev instanceof nodes.SimpleSelector) {
          ...
          this.prev && (this.prev.matches("+") || this.prev.matches("~")) &&
          this.element.parent
        ) {
