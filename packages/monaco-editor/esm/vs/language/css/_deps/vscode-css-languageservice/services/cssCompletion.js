...
var CSSCompletion = /** @class */ (function () {
  ...
  CSSCompletion.prototype.setCompletionParticipants = function (
    registeredCompletionParticipants,
  ) {
    this.completionParticipants = registeredCompletionParticipants || [];
  };
  CSSCompletion.prototype.doComplete2 = function (
    document,
    position,
    styleSheet,
    documentContext,
        ...
        switch (_a.label) {
          case 0:
            if (
              ...
              return [
                2, /*return*/
                this.doComplete(document, position, styleSheet),
              ];
            }
  ...
  CSSCompletion.prototype.doComplete = function (
    document,
    position,
    styleSheet,
  ) {
    ...
    try {
      ...
      for (var i = this.nodePath.length - 1; i >= 0; i--) {
        ...
        if (node instanceof nodes.Property) {
          ...
          var parentRef = node.findAParent(
  ...
  CSSCompletion.prototype.getCompletionsForDeclarationProperty = function (
    declaration,
    result,
  ) {
    return this.getPropertyProposals(declaration, result);
  };
  CSSCompletion.prototype.getPropertyProposals = function (
    declaration,
    result,
  ) {
    var _this = this;
    var triggerPropertyValueCompletion =
      this.isTriggerPropertyValueCompletionEnabled;
    var completePropertyWithSemicolon =
    ...
    properties.forEach(function (entry) {
      ...
      if (declaration && !declaration.semicolonPosition) {
        if (
          ...
          _this.offset >= _this.textDocument.offsetAt(range.end)
        ) {
          insertText += "$0;";
        }
      }
      ...
      var relevance = typeof entry.relevance === "number"
        ? Math.min(Math.max(entry.relevance, 0), 99)
        : 50;
      var sortTextSuffix = (255 - relevance).toString(16);
      var sortTextPrefix = strings.startsWith(entry.name, "-")
  ...
  Object.defineProperty(
    CSSCompletion.prototype,
    "isTriggerPropertyValueCompletionEnabled",
    {
      get: function () {
        if (
          !this.settings ||
  ...
  CSSCompletion.prototype.getCompletionsForDeclarationValue = function (
    node,
    result,
  ) {
    var _this = this;
  ...
  CSSCompletion.prototype.getValueEnumProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCSSWideKeywordProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForInterpolation = function (
    node,
    result,
  ) {
  ...
  CSSCompletion.prototype.getVariableProposals = function (
    existingNode,
    result,
  ) {
    var symbols = this.getSymbolContext().findSymbolsAtOffset(
      this.offset,
      nodes.ReferenceType.Variable,
    );
    for (var _i = 0, symbols_1 = symbols; _i < symbols_1.length; _i++) {
      var symbol = symbols_1[_i];
      var insertText = strings.startsWith(symbol.name, "--")
        ? "var(" + symbol.name + ")"
        : symbol.name;
      var completionItem = {
        label: symbol.name,
      ...
      if (
        ...
        isColorString(completionItem.documentation)
      ) {
      ...
      if (symbol.node.type === nodes.NodeType.FunctionParameter) {
        ...
        if (mixinNode.type === nodes.NodeType.MixinDeclaration) {
          completionItem.detail = localize(
            "completion.argument",
            "argument from '{0}'",
            mixinNode.getName(),
          );
  ...
  CSSCompletion.prototype.getVariableProposalsForCSSVarFunction = function (
    result,
  ) {
    var symbols = this.getSymbolContext().findSymbolsAtOffset(
      this.offset,
  ...
  CSSCompletion.prototype.getUnitProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionRange = function (existingNode) {
    if (
      existingNode && existingNode.offset <= this.offset &&
      this.offset <= existingNode.end
    ) {
      var end = existingNode.end !== -1
  ...
  CSSCompletion.prototype.getColorProposals = function (
    entry,
    existingNode,
    result,
  ) {
    ...
    var _loop_1 = function (p) {
      ...
      result.items.push({
        ...
        textEdit: TextEdit.replace(
          this_1.getCompletionRange(existingNode),
          insertText,
        ),
        insertTextFormat: SnippetFormat,
  ...
  CSSCompletion.prototype.getPositionProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getRepeatStyleProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getLineStyleProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getLineWidthProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getGeometryBoxProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getBoxProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getImageProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getTimingFunctionProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getBasicShapeProposals = function (
    entry,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForRuleSet = function (
    ruleSet,
    result,
  ) {
    var declarations = ruleSet.getDeclarations();
    var isAfter = declarations && declarations.endsWith("}") &&
  ...
  CSSCompletion.prototype.getCompletionsForSelector = function (
    ruleSet,
    isNested,
    result,
  ) {
    ...
    if (ruleSet && ruleSet.isNested()) {
      var selector = ruleSet.getSelectors().findFirstChildBeforeOffset(
        this.offset,
      );
      if (
        selector && ruleSet.getSelectors().getChildren().indexOf(selector) === 0
      ) {
  ...
  CSSCompletion.prototype.getCompletionsForDeclarations = function (
    declarations,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForVariableDeclaration = function (
    declaration,
    result,
  ) {
    if (
      this.offset && isDefined(declaration.colonPosition) &&
      this.offset > declaration.colonPosition
    ) {
  ...
  CSSCompletion.prototype.getCompletionsForExpression = function (
    expression,
    result,
  ) {
    ...
    if (
      node instanceof nodes.NumericValue || node instanceof nodes.Identifier
    ) {
      return this.getCompletionsForDeclarationValue(declaration, result);
    }
    return result;
  };
  CSSCompletion.prototype.getCompletionsForFunctionArgument = function (
    arg,
    func,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForFunctionDeclaration = function (
    decl,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForMixinReference = function (
    ref,
    result,
  ) {
    var _this = this;
    var allMixins = this.getSymbolContext().findSymbolsAtOffset(
      this.offset,
  ...
  CSSCompletion.prototype.getTermProposals = function (
    entry,
    existingNode,
    result,
  ) {
    var allFunctions = this.getSymbolContext().findSymbolsAtOffset(
      this.offset,
      nodes.ReferenceType.Function,
    );
    for (
      var _i = 0, allFunctions_1 = allFunctions;
      _i < allFunctions_1.length;
      _i++
    ) {
  ...
  CSSCompletion.prototype.makeTermProposal = function (
    symbol,
    parameters,
    existingNode,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForSupportsCondition = function (
    supportsCondition,
    result,
  ) {
    ...
    if (child) {
      if (child instanceof nodes.Declaration) {
        if (
          !isDefined(child.colonPosition) || this.offset <= child.colonPosition
        ) {
  ...
  CSSCompletion.prototype.getCompletionsForSupports = function (
    supports,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionsForExtendsReference = function (
    extendsRef,
    existingNode,
    result,
  ) {
  ...
  CSSCompletion.prototype.getCompletionForUriLiteralValue = function (
    uriLiteralNode,
    result,
  ) {
    var uriValue;
    ...
    if (!uriLiteralNode.hasChildren()) {
      ...
      var emptyURIValuePosition = this.textDocument.positionAt(
  ...
  CSSCompletion.prototype.getCompletionForImportPath = function (
    importPathNode,
    result,
  ) {
    var _this = this;
...
function collectValues(styleSheet, declaration) {
  ...
  function visitValue(node) {
    if (
      node instanceof nodes.Identifier || node instanceof nodes.NumericValue ||
...
function isColorString(s) {
  ...
  return (s.toLowerCase() in languageFacts.colors) ||
    /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(s);
}
