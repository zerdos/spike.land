...
var Scanner = /** @class */ (function () {
  ...
  Scanner.prototype._escape = function (result, includeNewLines) {
    ...
    if (ch === _BSL) {
      ...
      while (
        hexNumCount < 6 &&
        (ch >= _0 && ch <= _9 || ch >= _a && ch <= _f || ch >= _A && ch <= _F)
      ) {
        this.stream.advance(1);
        ch = this.stream.peekChar();
        hexNumCount++;
  ...
  Scanner.prototype._stringChar = function (closeQuote, result) {
    ...
    var ch = this.stream.peekChar();
    if (
      ch !== 0 && ch !== closeQuote && ch !== _BSL && ch !== _CAR &&
      ch !== _LFD && ch !== _NWL
    ) {
      this.stream.advance(1);
      result.push(String.fromCharCode(ch));
  ...
  Scanner.prototype._string = function (result) {
    if (this.stream.peekChar() === _SQO || this.stream.peekChar() === _DQO) {
      ...
      while (
        this._stringChar(closeQuote, result) || this._escape(result, true)
      ) {
        // loop
      }
  ...
  Scanner.prototype._unquotedChar = function (result) {
    ...
    var ch = this.stream.peekChar();
    if (
      ch !== 0 && ch !== _BSL && ch !== _SQO && ch !== _DQO && ch !== _LPA &&
      ch !== _RPA && ch !== _WSP && ch !== _TAB && ch !== _NWL && ch !== _LFD &&
      ch !== _CAR
    ) {
      this.stream.advance(1);
      result.push(String.fromCharCode(ch));
