    ...
    switch (token.type) {
      ...
      case TokenType.CurlyR: {
        if (delimiterStack.length !== 0) {
          var prevDelimiter = popPrevStartDelimiterOfType(
            delimiterStack,
            "brace",
          );
          if (!prevDelimiter) {
      ...
      case TokenType.Comment: {
        ...
        var getCurrDelimiter = function (token) {
          var matches = token.text.match(
            /^\s*\/\*\s*(#region|#endregion)\b\s*(.*?)\s*\*\//,
          );
          if (matches) {
            return commentRegionMarkerToDelimiter_1(matches[1]);
          } else if (
            document.languageId === "scss" || document.languageId === "less"
          ) {
            var matches_1 = token.text.match(
              /^\s*\/\/\s*(#region|#endregion)\b\s*(.*?)\s*/,
            );
            if (matches_1) {
