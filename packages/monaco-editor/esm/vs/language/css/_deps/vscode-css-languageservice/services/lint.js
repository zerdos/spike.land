...
var LintVisitor = /** @class */ (function () {
  ...
  LintVisitor.prototype.visitRuleSet = function (node) {
    ...
    var displayElems = this.fetchWithValue(
      propertyTable,
      "display",
      "inline-block",
    );
        ...
        if (value && this.documentText.charAt(value.offset) !== "-") {
          ...
          if (elements_1.length > 1) {
            for (var k = 0; k < elements_1.length; k++) {
              ...
              if (
                value_1 && this.documentText.charAt(value_1.offset) !== "-" &&
                elements_1[k] !== element
              ) {
    ...
    if (!isExportBlock) {
      ...
      for (
        var _d = 0, propertyTable_1 = propertyTable;
        _d < propertyTable_1.length;
        _d++
      ) {
      ...
      if (!containsUnknowns) { // don't perform this test if there are
        for (var suffix in propertiesBySuffix.data) {
          ...
          var needsStandard = this.cssDataManager.isStandardProperty(suffix) &&
  ...
  LintVisitor.prototype.visitNumericValue = function (node) {
    ...
    if (decl) {
      ...
      if (declValue) {
        ...
        if (
          !value.unit ||
          languageFacts.units.length.indexOf(value.unit.toLowerCase()) === -1
        ) {
          return true;
        }
        if (
          parseFloat(value.value) === 0.0 && !!value.unit &&
