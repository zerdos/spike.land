...
import { endsWith, isLetterOrDigit, startsWith } from "../utils/strings.js";
...
var HTMLCompletion = /** @class */ (function () {
    ...
    function collectDataAttributesSuggestions(range, seenAttributes) {
      ...
      function addNodeDataAttributes(node) {
        node.attributeNames.forEach(function (attr) {
          if (
            startsWith(attr, dataAttr) && !dataAttributes[attr] &&
            !seenAttributes[attr]
          ) {
  ...
  HTMLCompletion.prototype.doTagComplete = function (
    ...
    if (char === ">") {
      var node = htmlDocument.findNodeBefore(offset);
      if (
        node && node.tag && !isVoidElement(node.tag) && node.start < offset &&
      ...
      if (node && node.tag) {
        ...
        while (token !== TokenType.EOS && scanner.getTokenEnd() <= offset) {
          if (
            token === TokenType.EndTagOpen && scanner.getTokenEnd() === offset
          ) {
            return node.tag + ">";
          }
          token = scanner.scan();
