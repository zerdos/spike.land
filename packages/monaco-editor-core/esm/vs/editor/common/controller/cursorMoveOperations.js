...
export class MoveOperations {
    ...
    static leftPositionAtomicSoftTabs(model, lineNumber, column, tabSize) {
        ...
        const newPosition = AtomicTabMoveOperations.atomicPosition(lineContent, column - minColumn, tabSize, 0 /* Left */);
        if (newPosition === -1) {
    ...
    static rightPositionAtomicSoftTabs(model, lineNumber, column, tabSize, indentSize) {
        const minColumn = model.getLineMinColumn(lineNumber);
        const lineContent = model.getLineContent(lineNumber);
        const newPosition = AtomicTabMoveOperations.atomicPosition(lineContent, column - minColumn, tabSize, 1 /* Right */);
        if (newPosition === -1) {
