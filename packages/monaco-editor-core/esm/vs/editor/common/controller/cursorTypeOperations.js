...
export class TypeOperations {
  ...
  static _goodIndentForLine(config, model, lineNumber) {
    ...
    const expectedIndentAction = LanguageConfigurationRegistry
      .getInheritIndentForLine(config.autoIndent, model, lineNumber, false);
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (selection.isEmpty()) {
        ...
        if (selection.startLineNumber === selection.endLineNumber) {
          ...
          if (
            selection.startColumn !== 1 || selection.endColumn !== lineMaxColumn
          ) {
  ...
  static _enter(config, model, keepPosition, range) {
    ...
    if (
      ...
      config.autoIndent === 1 /* Keep */
    ) {
      let lineText = model.getLineContent(range.startLineNumber);
      let indentation = strings.getLeadingWhitespace(lineText).substring(
        0,
        range.startColumn - 1,
      );
      ...
      if (ir) {
        ...
        if (keepPosition) {
          ...
          if (oldEndColumn <= firstNonWhitespace + 1) {
            ...
            offset = Math.min(
              oldEndViewColumn + 1 -
                config.normalizeIndentation(ir.afterEnter).length - 1,
              0,
            );
          }
          return new ReplaceCommandWithOffsetCursorState(
  ...
  static _runAutoIndentType(config, model, range, ch) {
    const currentIndentation = LanguageConfigurationRegistry
      .getIndentationAtPosition(
        model,
        range.startLineNumber,
        range.startColumn,
      );
    const actualIndentation = LanguageConfigurationRegistry
      .getIndentActionForType(config.autoIndent, model, range, ch, {
      ...
      if (firstNonWhitespace === 0) {
        ...
        return TypeOperations._typeCommand(
          ...
          config.normalizeIndentation(actualIndentation) +
            model.getLineContent(range.startLineNumber).substring(
  ...
  static _findAutoClosingPairOpen(config, model, positions, ch) {
    const autoClosingPairCandidates = config.autoClosingPairs
      .autoClosingPairsOpenByEnd.get(ch);
  ...
  static _getAutoClosingPairClose(
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (
        autoClosingPair.open.length === 1 && chIsQuote &&
        autoCloseConfig !== "always"
      ) {
        const wordSeparators = getMapForWordSeparators(config.wordSeparators);
        if (
          insertOpenCharacter && position.column > 1 &&
        ...
        if (
          !insertOpenCharacter && position.column > 2 &&
  ...
  static compositionEndWithInterceptors(
    ...
    if (
      ...
      const commands = selections.map((s) =>
        new ReplaceCommand(
          new Range(
  ...
  static typeWithInterceptors(
    ...
    if (
      !isDoingComposition && this._isAutoIndentType(config, model, selections)
    ) {
      let commands = [];
    ...
    let shouldPushStackElementBefore =
      (prevEditOperationType !== 1 /* Typing */);
    if (ch === " ") {
      shouldPushStackElementBefore = true;
    }
    return new EditOperationResult(1, /* Typing */ commands, {
...
export class TypeWithAutoClosingCommand
  ...
  computeCursorState(model, helper) {
    ...
    this.enclosingRange = new Range(
      range.startLineNumber,
      range.endColumn - this._openCharacter.length -
