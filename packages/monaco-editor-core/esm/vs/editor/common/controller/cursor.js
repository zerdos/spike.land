...
export class Cursor extends Disposable {
  ...
  onModelContentChanged(eventsCollector, e) {
    ...
    if (hadFlushEvent) {
      ...
      if (
        this._hasFocus && e.resultingSelection &&
  ...
  getCursorColumnSelectData() {
    ...
    const viewSelectionStart = primaryCursor.viewState.selectionStart
      .getStartPosition();
  ...
  _emitStateChangedIfNecessary(
    ...
    if (
      ...
      newState.cursorState.some((newCursorState, i) =>
  ...
  _findAutoClosingPairs(edits) {
    ...
    for (let i = 0, len = edits.length; i < len; i++) {
      ...
      const closeChar = m[1];
      const autoClosingPairsCandidates = this.context.cursorConfig
        .autoClosingPairs.autoClosingPairsCloseSingleChar.get(closeChar);
      if (
        !autoClosingPairsCandidates || autoClosingPairsCandidates.length !== 1
      ) {
        return null;
      }
      ...
      (undoEdits) => {
        if (autoClosingIndices) {
          for (let i = 0, len = autoClosingIndices.length; i < len; i++) {
            ...
            const openCharIndex = undoEdit.range.startColumn - 1 +
              openCharInnerIndex;
            const closeCharIndex = undoEdit.range.startColumn - 1 +
              closeCharInnerIndex;
    ...
    this._executeEdit(
      () => {
        if (source === "keyboard") {
          ...
          const autoClosedCharacters = AutoClosedAction
            .getAllAutoClosedCharacters(this._autoClosedActions);
