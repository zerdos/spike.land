...
import {
  ...
  SimpleWorkerClient,
} from "../../../base/common/worker/simpleWorker";
...
let EditorWorkerServiceImpl = class EditorWorkerServiceImpl extends Disposable {
  ...
  canComputeDiff(original, modified) {
    return (canSyncModel(this._modelService, original) &&
  ...
  computeMoreMinimalEdits(resource, edits) {
    if (isNonEmptyArray(edits)) {
      ...
      const result = this._workerManager.withWorker().then((client) =>
        client.computeMoreMinimalEdits(resource, edits)
      );
      result.finally(() =>
  ...
  navigateValueSet(resource, range, up) {
    return this._workerManager.withWorker().then((client) =>
      client.navigateValueSet(resource, range, up)
    );
  }
  ...
  computeWordRanges(resource, range) {
    return this._workerManager.withWorker().then((client) =>
      client.computeWordRanges(resource, range)
    );
  }
};
...
class WorkerManager extends Disposable {
  ...
  _checkStopIdleWorker() {
    ...
    let timeSinceLastWorkerUsedTime = (new Date()).getTime() -
      this._lastWorkerUsedTime;
