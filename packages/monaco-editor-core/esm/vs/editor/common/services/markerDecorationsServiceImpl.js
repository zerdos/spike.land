...
import { MinimapPosition, OverviewRulerLane } from "../model";
...
let MarkerDecorationsService = class MarkerDecorationsService
  extends Disposable {
  ...
  _onModelRemoved(model) {
    ...
    if (
      ...
      if (this._markerService) {
        this._markerService.read({ resource: model.uri }).map((marker) =>
          marker.owner
  ...
  _updateDecorations(markerDecorations) {
    ...
    const markers = this._markerService.read(
      { resource: markerDecorations.model.uri, take: 500 },
    );
  ...
  _createDecorationRange(model, rawMarker) {
    ...
    if (ret.isEmpty()) {
      ...
      rawMarker.endColumn === Number.MAX_VALUE && rawMarker.startColumn === 1 &&
