...
import { createFindMatch, isValidMatch, Searcher } from "../textModelSearch";
...
export class PieceTreeBase {
  ...
  getLineLength(lineNumber) {
    ...
    return this.getOffsetAt(lineNumber + 1, 1) -
      this.getOffsetAt(lineNumber, 1) - this._EOLLength;
  }
  findMatchesInNode(
    node,
    searcher,
    ...
    do {
      ...
      if (m) {
        ...
        let retStartColumn = ret.line === startCursor.line
  ...
  findMatchesLineByLine(
    ...
    while (currentNode !== endPosition.node) {
      ...
      if (lineBreakCnt >= 1) {
        ...
        let startColumn = startLineNumber === searchRange.startLineNumber
          ? searchRange.startColumn
          : 1;
  ...
  delete(offset, cnt) {
    ...
    if (startNode === endNode) {
      ...
      if (
        startPosition.nodeStartOffset + startNode.piece.length === offset + cnt
      ) {
  ...
  insertContentToNodeLeft(value, node) {
    ...
    let nodesToDel = [];
    if (
      this.shouldCheckCRLF() && this.endWithCR(value) && this.startWithLF(node)
    ) {
      // move `\n` to new node.
      let piece = node.piece;
      ...
      while (x !== SENTINEL) {
        if (x.left !== SENTINEL && x.lf_left >= lineNumber - 1) {
          ...
          this._searchCache.set({
            node: x,
            nodeStartOffset,
            nodeStartLineNumber: originalLineNumber -
              (lineNumber - 1 - x.lf_left),
          });
          return buffer.substring(
            startOffset + prevAccumulatedValue,
            startOffset + accumulatedValue - endOffset,
  ...
  getAccumulatedValue(node, index) {
    ...
    if (expectedLineStartIndex > piece.end.line) {
      ...
      return lineStarts[expectedLineStartIndex] - lineStarts[piece.start.line] -
        piece.start.column;
    }
  }
  ...
  appendToNode(node, value) {
    ...
    const hitCRLF = this.shouldCheckCRLF() && this.startWithLF(value) &&
      this.endWithCR(node);
    ...
    if (prev.piece.end.column === 0) {
      ...
      newEnd = {
        ...
        column: lineStarts[prev.piece.end.line] -
          lineStarts[prev.piece.end.line - 1] - 1,
      };
    } else {
      // \r\n
