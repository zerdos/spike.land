  ...
  getCharacterCountInRange(range, eol = 0 /* TextDefined */) {
    if (this._mightContainNonBasicASCII) {
      ...
      for (
        ...
        const fromOffset = (lineNumber === fromLineNumber
          ? range.startColumn - 1
          : 0);
      ...
      result += this._getEndOfLine(eol).length *
  ...
  applyEdits(rawOperations, recordTrimAutoWhitespace, computeUndoEdits) {
    ...
    for (let i = 0; i < rawOperations.length; i++) {
      ...
      if (op.text) {
        ...
        if (
          !mightContainUnusualLineTerminators && textMightContainNonBasicASCII
        ) {
          // check if the new inserted text contains unusual line terminators
          mightContainUnusualLineTerminators = strings
            .containsUnusualLineTerminators(op.text);
        }
      }
      let validText = "";
      let eolCount = 0;
        ...
        if (op.isAutoWhitespaceEdit && op.range.isEmpty()) {
          ...
          for (
            ...
            newTrimAutoWhitespaceCandidates.push(
              { lineNumber: lineNumber, oldContent: currentLineContent },
            );
          }
        }
      }
    ...
    if (
      recordTrimAutoWhitespace && newTrimAutoWhitespaceCandidates.length > 0
    ) {
      // sort line numbers auto whitespace removal candidates for next edit descending
      newTrimAutoWhitespaceCandidates.sort((a, b) =>
      ...
      for (
        ...
        if (
          lineContent.length === 0 || lineContent === prevContent ||
