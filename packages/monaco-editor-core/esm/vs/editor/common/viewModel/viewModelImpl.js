...
export class ViewModel extends Disposable {
  constructor(
    ...
    if (
      USE_IDENTITY_LINES_COLLECTION && this.model.isTooLargeForTokenization()
    ) {
      ...
      const wrappingInfo = options.get(124 /* wrappingInfo */);
  ...
  tokenizeViewport() {
    ...
    const startPosition = this.coordinatesConverter
      .convertViewPositionToModelPosition(
        new Position(linesViewportData.startLineNumber, 1),
      );
    const endPosition = this.coordinatesConverter
      .convertViewPositionToModelPosition(
  ...
  _onConfigurationChanged(eventsCollector, e) {
    ...
    if (this._viewportStartLine !== -1) {
      ...
      previousViewportStartModelPosition = this.coordinatesConverter
        .convertViewPositionToModelPosition(previousViewportStartViewPosition);
    }
    ...
    if (restorePreviousViewportStart && previousViewportStartModelPosition) {
      const viewPosition = this.coordinatesConverter
        .convertModelPositionToViewPosition(previousViewportStartModelPosition);
  ...
  _registerModelEvents() {
    this._register(this.model.onDidChangeRawContentFast((e) => {
      ...
      if (
        ...
        if (modelRange) {
          const viewPosition = this.coordinatesConverter
            .convertModelPositionToViewPosition(modelRange.getStartPosition());
          const viewPositionTop = this.viewLayout
            .getVerticalOffsetForLineNumber(viewPosition.lineNumber);
          this.viewLayout.setScrollPosition(
            { scrollTop: viewPositionTop + this._viewportStartLineDelta },
            1, /* Immediate */
          );
  ...
  getVisibleRangesPlusViewportAboveBelow() {
    const layoutInfo = this._configuration.options.get(123 /* layoutInfo */);
  ...
  saveState() {
    ...
    const firstPosition = this.coordinatesConverter
      .convertViewPositionToModelPosition(
        new Position(
          firstViewLineNumber,
  ...
  setViewport(startLineNumber, endLineNumber, centeredLineNumber) {
    ...
    const viewportStartLineTop = this.viewLayout.getVerticalOffsetForLineNumber(
      startLineNumber,
    );
  ...
  deduceModelPositionRelativeToViewPosition(
    ...
    const modelAnchor = this.coordinatesConverter
      .convertViewPositionToModelPosition(viewAnchorPosition);
        ...
        if (modelRange.isEmpty()) {
          ...
          result.push(
            this.model.getValueInRange(
              modelRange,
              forceCRLF ? 2 /* CRLF */ : 0, /* TextDefined */
            ),
          );
        }
  ...
  getRichTextToCopy(modelRanges, emptySelectionClipboard) {
    ...
    const fontFamily = fontInfo.fontFamily === EDITOR_FONT_DEFAULTS.fontFamily
      ? fontInfo.fontFamily
    ...
    return {
      ...
      html: (`<div style="` +
        `color: ${
          colorMap[1/* DefaultForeground */
          ]
        };` +
        `background-color: ${
          colorMap[2/* DefaultBackground */
          ]
        };` +
  ...
  _getHTMLToCopy(modelRange, colorMap) {
    ...
    for (
      ...
      const startOffset = (lineNumber === startLineNumber
        ? startColumn - 1
        : 0);
  ...
  setSelections(source, selections) {
    this._withViewEventsCollector((eventsCollector) =>
      this._cursor.setSelections(eventsCollector, source, selections)
    );
  }
  saveCursorState() {
  ...
  replacePreviousChar(text, replaceCharCnt, source) {
    this._executeCursorEdit((eventsCollector) =>
      this._cursor.replacePreviousChar(
        eventsCollector,
        text,
        replaceCharCnt,
        source,
