...
let MessageController = class MessageController {
  constructor(editor, contextKeyService) {
    ...
    this._editorListener = this._editor.onDidAttemptReadOnlyEdit(() =>
  ...
  showMessage(message, position) {
    ...
    this._messageListeners.add(
      this._editor.onDidBlurEditorText(() => this.closeMessage()),
    );
    this._messageListeners.add(
    ...
    // 3sec
    this._messageListeners.add(
      new TimeoutTimer(() => this.closeMessage(), 3000),
    );
    // close on mouse move
    let bounds;
...
registerThemingParticipant((theme, collector) => {
  ...
  if (border) {
    ...
    collector.addRule(
      `.monaco-editor .monaco-editor-overlaymessage .anchor.below { border-top-color: ${border}; }`,
    );
    collector.addRule(
      `.monaco-editor .monaco-editor-overlaymessage .anchor.top { border-bottom-color: ${border}; }`,
    );
    collector.addRule(
  ...
  if (foreground) {
    collector.addRule(
      `.monaco-editor .monaco-editor-overlaymessage .message { color: ${foreground}; }`,
    );
  }
});
