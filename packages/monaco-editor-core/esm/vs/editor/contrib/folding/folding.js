...
let FoldingController = class FoldingController extends Disposable {
  constructor(editor, contextKeyService) {
    ...
    this._useFoldingProviders =
      options.get(34 /* foldingStrategy */) !== "indentation";
    ...
    this.foldingDecorationProvider.autoHideFoldingControls =
      options.get(94 /* showFoldingControls */) === "mouseover";
    ...
    this._register(this.editor.onDidChangeConfiguration((e) => {
      ...
      if (
        e.hasChanged(94 /* showFoldingControls */) ||
        e.hasChanged(35 /* foldingHighlight */)
      ) {
        const options = this.editor.getOptions();
        this.foldingDecorationProvider.autoHideFoldingControls =
          options.get(94 /* showFoldingControls */) === "mouseover";
      ...
      if (e.hasChanged(34 /* foldingStrategy */)) {
        this._useFoldingProviders =
          this.editor.getOptions().get(34 /* foldingStrategy */) !==
            "indentation";
  ...
  restoreViewState(state) {
    let model = this.editor.getModel();
    if (
      !model || !this._isEnabled || model.isTooLargeForTokenization() ||
      !this.hiddenRangeModel
    ) {
      return;
    }
    if (
      !state || !state.collapsedRegions ||
  ...
  getRangeProvider(editorModel) {
    ...
    if (this._useFoldingProviders && this.foldingModel) {
      ...
      if (
        foldingProviders.length === 0 && this.foldingStateMemento &&
    ...
    if (this.updateScheduler) {
      ...
      this.foldingModelPromise = this.updateScheduler.trigger(() => {
        ...
        return foldingRegionPromise.then((foldingRanges) => {
          if (
            foldingRanges && foldingRegionPromise === this.foldingRegionPromise
  ...
  onHiddenRangesChanges(hiddenRanges) {
    if (
      this.hiddenRangeModel && hiddenRanges.length && !this._restoringViewState
    ) {
      let selections = this.editor.getSelections();
      if (selections) {
  ...
  onEditorMouseDown(e) {
    ...
    switch (e.target.type) {
      ...
      case 7 /* CONTENT_EMPTY */: {
        if (
          this._unfoldOnClickAfterEndOfLine && this.hiddenRangeModel.hasRanges()
        ) {
          const data = e.target.detail;
  ...
  constructor() {
    super({
      ...
      kbOpts: {
        ...
        primary: 2048 /* CtrlCmd */ | 1024 /* Shift */ |
          89, /* US_CLOSE_SQUARE_BRACKET */
        mac: {
          primary: 2048 /* CtrlCmd */ | 512 /* Alt */ |
            89, /* US_CLOSE_SQUARE_BRACKET */
        },
