...
import {
  Sizing,
  SplitView,
} from "../../../../base/browser/ui/splitview/splitview";
  ...
  _onDecorationChanged() {
    ...
    for (let [decorationId, reference] of this._decorations) {
      ...
      if (Range.spansMultipleLines(newRange)) {
        ...
        const lineLength = reference.range.endColumn -
...
let ReferenceWidget = class ReferenceWidget extends peekView.PeekViewWidget {
  ...
  _applyTheme(theme) {
    const borderColor = theme.getColor(peekView.peekViewBorder) ||
      Color.transparent;
    this.style({
  ...
  _fillBody(containerElement) {
    ...
    const treeOptions = {
      ...
      keyboardNavigationLabelProvider: this._instantiationService
        .createInstance(StringRepresentationProvider),
    ...
    this._disposables.add(this._splitView.onDidSashChange(() => {
      if (this._dim.width) {
        this.layoutData.ratio = this._splitView.getViewSize(0) /
          this._dim.width;
      }
    }, undefined));
  ...
  revealReference(reference) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      this._onDidSelectReference.fire(
        { element: reference, kind: "goto", source: "tree" },
      );
    });
  }
...
registerThemingParticipant((theme, collector) => {
  const findMatchHighlightColor = theme.getColor(
  ...
  const referenceHighlightColor = theme.getColor(
  ...
  const referenceHighlightBorder = theme.getColor(
  ...
  const resultsMatchForeground = theme.getColor(
  ...
  const resultsFileForeground = theme.getColor(
  ...
  const resultsSelectedBackground = theme.getColor(
  ...
  const resultsSelectedForeground = theme.getColor(
  ...
  const editorGutterBackground = theme.getColor(
