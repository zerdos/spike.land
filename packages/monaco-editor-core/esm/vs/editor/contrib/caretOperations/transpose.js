...
class TransposeLettersAction extends EditorAction {
  ...
  run(accessor, editor) {
    ...
    for (let selection of selections) {
      ...
      if (
        lineNumber === 1 && (column === 1 || (column === 2 && lastColumn === 2))
      ) {
      ...
      let middlePosition = MoveOperations.leftPosition(
        model,
        endPosition.lineNumber,
        endPosition.column,
      );
      let beginPosition = MoveOperations.leftPosition(
        model,
        middlePosition.lineNumber,
        middlePosition.column,
      );
      let leftChar = model.getValueInRange(
        Range.fromPositions(beginPosition, middlePosition),
      );
      let rightChar = model.getValueInRange(
