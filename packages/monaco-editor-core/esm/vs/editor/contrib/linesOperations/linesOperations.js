...
} from "../../common/commands/replaceCommand";
  ...
  constructor() {
    super(false, {
      ...
      kbOpts: {
        ...
        linux: {
          primary: 2048 /* CtrlCmd */ | 512 /* Alt */ | 1024 /* Shift */ |
            16, /* UpArrow */
        },
...
export class TrimTrailingWhitespaceAction extends EditorAction {
  ...
  run(_accessor, editor, args) {
    ...
    if (args.reason === "auto-save") {
      ...
      cursors = (editor.getSelections() || []).map((s) =>
...
export class DeleteLinesAction extends EditorAction {
  ...
  _getLinesToRemove(editor) {
    ...
    for (let i = 1; i < operations.length; i++) {
      if (
        previousOperation.endLineNumber + 1 >= operations[i].startLineNumber
      ) {
    ...
    rangesToDelete = rangesToDelete.map((selection) => {
      if (selection.isEmpty()) {
        if (selection.startColumn === 1) {
          ...
          let deleteFromColumn = selection.startLineNumber === 1
            ? 1
            : model.getLineContent(deleteFromLine).length + 1;
          return new Range(
            deleteFromLine,
            deleteFromColumn,
    ...
    for (let i = 0, len = reducedSelections.length; i < len; i++) {
      ...
      for (let i = startLineNumber + 1; i <= endLineNumber; i++) {
        ...
        if (firstNonWhitespaceIdx >= 1) {
          ...
          trimmedLinesContent += (insertSpace ? " " : "") +
            lineTextWithoutIndent;
          if (insertSpace) {
      ...
      lineOffset += deleteSelection.endLineNumber -
  ...
  run(_accessor, editor) {
    ...
    for (let i = 0, len = selections.length; i < len; i++) {
      ...
      if (cursor.column >= maxColumn) {
        ...
        let chars = model.getValueInRange(deleteSelection).split("").reverse()
          .join("");
        commands.push(
          new ReplaceCommand(
            new Selection(
