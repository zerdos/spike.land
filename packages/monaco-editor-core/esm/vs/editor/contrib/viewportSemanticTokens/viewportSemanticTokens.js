...
} from "../../common/services/modelServiceImpl";
let ViewportSemanticTokensContribution =
  class ViewportSemanticTokensContribution extends Disposable {
    constructor(editor, _modelService, _themeService, _configurationService) {
      ...
      this._editor = editor;
      this._tokenizeViewport = new RunOnceScheduler(
        () => this._tokenizeViewportNow(),
        100,
      );
    ...
    _tokenizeViewportNow() {
      ...
      const provider = ViewportSemanticTokensContribution
        ._getSemanticColoringProvider(model);
      if (!provider) {
        if (model.hasSomeSemanticTokens()) {
          model.setSemanticTokens(null, false);
        }
      ...
      const styling = this._modelService.getSemanticTokensProviderStyling(
        provider,
      );
      ...
      this._outstandingRequests = this._outstandingRequests.concat(
        visibleRanges.map((range) =>
          this._requestRange(model, range, provider, styling)
        ),
      );
    }
    _requestRange(model, range, provider, styling) {
      ...
      const request = createCancelablePromise((token) =>
        Promise.resolve(
          provider.provideDocumentRangeSemanticTokens(model, range, token),
        )
      );
      request.then((r) => {
        if (
          !r || model.isDisposed() || model.getVersionId() !== requestVersionId
        ) {
          return;
        }
        model.setPartialSemanticTokens(
          range,
          toMultilineTokens2(r, styling, model.getLanguageIdentifier()),
        );
      }).then(
