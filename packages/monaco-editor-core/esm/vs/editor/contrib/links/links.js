  ...
  if (link.url) {
    ...
    if (/^command:/i.test(link.url.toString())) {
      ...
      if (match) {
        ...
        const nativeLabelText = nls.localize(
          "tooltip.explanation",
          "Execute command {0}",
          commandId,
        );
    ...
    const hoverMessage = new MarkdownString("", true).appendMarkdown(
      `[${label}](${link.url.toString()}${nativeLabel}) (${kb})`,
    );
    return hoverMessage;
  } else {
...
let LinkDetector = class LinkDetector {
  ...
  beginCompute() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      this.computePromise = async.createCancelablePromise((token) =>
        getLinks(model, token)
      );
      try {
  ...
  updateDecorations(links) {
    const useMetaKey =
      (this.editor.getOption(64 /* multiCursorModifier */) === "altKey");
    let oldDecorations = [];
  ...
  openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
    ...
    link.resolve(CancellationToken.None).then((uri) => {
      ...
      if (typeof uri === "string" && this.editor.hasModel()) {
        ...
        if (
          modelUri.scheme === Schemas.file && uri.startsWith(`${Schemas.file}:`)
        ) {
