...
import {
  IBulkEditService,
  ResourceEdit,
} from "../../browser/services/bulkEditService";
...
let QuickFixController = class QuickFixController extends Disposable {
  constructor(
    ...
    this._ui = new Lazy(() =>
      this._register(
        new CodeActionUi(editor, QuickFixAction.Id, AutoFixAction.Id, {
  ...
  showCodeActions(trigger, actions, at) {
    return this._ui.getValue().showCodeActionList(
      trigger,
      actions,
      at,
...
export function applyCodeAction(accessor, item, editor) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (item.action.edit) {
      yield bulkEditService.apply(
        ResourceEdit.convert(item.action.edit),
        { editor, label: item.action.title },
      );
    }
