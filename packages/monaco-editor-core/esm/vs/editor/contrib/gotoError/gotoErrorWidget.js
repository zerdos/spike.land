...
class MessageWidget {
  ...
  update(marker) {
    ...
    if (isNonEmptyArray(relatedInformation)) {
      ...
      relatedInformationNode.style.paddingTop = `${
        Math.floor(this._editor.getOption(53 /* lineHeight */) * 0.66)
      }px`;
      this._lines += 1;
  ...
  getAriaLabel(marker) {
    ...
    if (
      model && (marker.startLineNumber <= model.getLineCount()) &&
...
let MarkerNavigationWidget = class MarkerNavigationWidget
  extends PeekViewWidget {
  constructor(
    editor,
    _themeService,
    _openerService,
    ...
    super(
      editor,
      { showArrow: true, showFrame: true, isAccessible: true },
      instantiationService,
    );
  ...
  _fillHead(container) {
    ...
    this._disposables.add(
      this._actionbarWidget.actionRunner.onBeforeRun((e) =>
        this.editor.focus()
      ),
    );
    const actions = [];
  ...
  _getActionBarOptions() {
    return Object.assign(
      Object.assign({}, super._getActionBarOptions()),
  ...
  showAtMarker(marker, markerIdx, markerCount) {
    ...
    let position = editorPosition && range.containsPosition(editorPosition)
      ? editorPosition
...
registerThemingParticipant((theme, collector) => {
  ...
  if (linkFg) {
    ...
    collector.addRule(
      `.monaco-editor .marker-widget a.code-link span:hover { color: ${linkFg}; }`,
    );
  }
});
