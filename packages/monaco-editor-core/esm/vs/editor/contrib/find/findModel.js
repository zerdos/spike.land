...
export class FindModelBoundToEditorModel {
  ...
  _onStateChanged(e) {
    ...
    if (
      e.searchString || e.isReplaceRevealed || e.isRegex || e.wholeWord ||
      e.matchCase || e.searchScope
    ) {
  ...
  research(moveCursor, newFindScope) {
    ...
    if (currentMatchesPosition === 0 && findMatches.length > 0) {
      ...
      currentMatchesPosition = matchAfterSelection > 0
  ...
  _moveToPrevMatch(before, isRecursed = false) {
    ...
    if (this._decorations.getCount() < MATCHES_LIMIT) {
      ...
      if (
        prevMatchRange && prevMatchRange.isEmpty() &&
    ...
    if (
      prevMatch && prevMatch.range.isEmpty() &&
  ...
  _moveToNextMatch(after) {
    ...
    if (this._decorations.getCount() < MATCHES_LIMIT) {
      ...
      if (
        nextMatchRange && nextMatchRange.isEmpty() &&
  ...
  _getNextMatch(after, captureMatches, forceMove, isRecursed = false) {
    ...
    if (
      forceMove && nextMatch && nextMatch.range.isEmpty() &&
