...
} from "../../../platform/browser/contextScopedHistoryWidget";
import { Codicon } from "../../../base/common/codicons";
import {
  registerIcon,
  widgetClose,
} from "../../../platform/theme/common/iconRegistry";
...
const ctrlKeyMod =
  (platform.isMacintosh ? 256 /* WinCtrl */ : 2048 /* CtrlCmd */);
...
function stopPropagationForMultiLineDownwards(event, value, textarea) {
  const isMultiline = !!value.match(/\n/);
  if (
    textarea && isMultiline && textarea.selectionEnd < textarea.value.length
  ) {
    event.stopPropagation();
    return;
  }
...
export class FindWidget extends Widget {
    ...
    this._register(this._codeEditor.onDidChangeConfiguration((e) => {
      ...
      if (e.hasChanged(31 /* find */)) {
        const addExtraSpaceOnTop =
          this._codeEditor.getOption(31 /* find */).addExtraSpaceOnTop;
      ...
      this._codeEditor.onDidFocusEditorWidget(() =>
        __awaiter(this, void 0, void 0, function* () {
          if (this._isVisible) {
            ...
            if (
              globalBufferTerm && globalBufferTerm !== this._state.searchString
            ) {
  ...
  _onStateChanged(e) {
    ...
    if (e.isReplaceRevealed) {
      if (this._state.isReplaceRevealed) {
        if (
          !this._codeEditor.getOption(75 /* readOnly */) &&
  ...
  _getAriaLabel(label, currentMatch, searchString) {
    ...
    if (currentMatch) {
      ...
      if (
        model && (currentMatch.startLineNumber <= model.getLineCount()) &&
  ...
  _reveal() {
    ...
    if (!this._isVisible) {
      ...
      if (
        this._codeEditor.getOption(31 /* find */)
  ...
  _tryUpdateWidgetWidth() {
    ...
    if (this._resized) {
      ...
      if (widgetWidth > FIND_WIDGET_INITIAL_WIDTH) {
        ...
        this._domNode.style.maxWidth = `${editorWidth - 28 - minimapWidth -
          15}px`;
        this._replaceInput.width = dom.getTotalWidth(this._findInput.domNode);
        return;
      }
  ...
  _getHeight() {
    ...
    if (this._isReplaceVisible) {
      ...
      totalheight += 4;
      totalheight += this._replaceInput.inputBox.height +
        2 /** input box border */;
    }
    // margin bottom
    totalheight += 4;
  ...
  _onReplaceInputKeyDown(e) {
    if (e.equals(ctrlKeyMod | 3 /* Enter */)) {
      if (
        platform.isWindows && platform.isNative &&
  ...
  _buildDomNode() {
    ...
    this._prevBtn = this._register(
      new SimpleButton({
        ...
        onTrigger: () => {
          this._codeEditor.getAction(FIND_IDS.PreviousMatchFindAction).run()
    ...
    this._nextBtn = this._register(
      new SimpleButton({
        ...
        onTrigger: () => {
          this._codeEditor.getAction(FIND_IDS.NextMatchFindAction).run().then(
            undefined,
            onUnexpectedError,
          );
        },
    ...
    this._resizeSash = new Sash(
      this._domNode,
      this,
      { orientation: 0, /* VERTICAL */ size: 2 },
    );
    this._resized = false;
