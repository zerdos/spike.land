...
class OccurenceAtPositionRequest {
  constructor(model, selection, wordSeparators) {
    ...
    this.result = createCancelablePromise((token) =>
      this._compute(model, selection, wordSeparators, token)
    );
  }
...
function computeOccurencesAtPosition(model, selection, wordSeparators) {
  ...
  return new TextualOccurenceAtPositionRequest(
    model,
    selection,
    wordSeparators,
  );
...
class WordHighlighter {
  ...
  moveNext() {
    ...
    let index = highlights.findIndex((range) =>
    ...
    try {
      ...
      if (word) {
        ...
        alert(
          `${lineContent}, ${newIndex +
            1} of ${highlights.length} for '${word.word}'`,
        );
      }
    } finally {
      this._ignorePositionChangeEvent = false;
  ...
  _run() {
    ...
    if (workerRequestIsValid) {
      ...
      this.workerRequest = computeOccurencesAtPosition(
        ...
        this.editor.getOption(110 /* wordSeparators */),
      );
...
let WordHighlighterContribution = class WordHighlighterContribution
  extends Disposable {
  constructor(editor, contextKeyService) {
    super();
...
registerThemingParticipant((theme, collector) => {
  ...
  if (selectionHighlight) {
    ...
    collector.addRule(
      `.monaco-editor .selectionHighlight { background-color: ${
        selectionHighlight.transparent(0.5)
      }; }`,
    );
  }
  ...
  const selectionHighlightBorder = theme.getColor(
    editorSelectionHighlightBorder,
  );
  ...
  const wordHighlightStrongBorder = theme.getColor(
