    ...
    super({
      ...
      kbOpts: {
        ...
        linux: {
          ...
          secondary: [
            2048 /* CtrlCmd */ | 1024 /* Shift */ | 18, /* DownArrow */
          ],
        },
        weight: 100, /* EditorContrib */
      },
...
export class MultiCursorSession {
  ...
  static create(editor, findController) {
    ...
    if (
      !editor.hasTextFocus() && findState.isRevealed &&
  ...
  _getNextMatch() {
    ...
    const nextMatch = this._editor.getModel().findNextMatch(
      ...
      false,
      this.matchCase,
      this.wholeWord ? this._editor.getOption(110 /* wordSeparators */) : null,
      false,
    );
    if (!nextMatch) {
      return null;
...
export class MultiCursorSelectionController extends Disposable {
  ...
  selectAll(findController) {
    ...
    if (
      findState.isRevealed && findState.searchString.length > 0 &&
      findState.isRegex
    ) {
      matches = this._editor.getModel().findMatches(
        ...
        findState.matchCase,
        findState.wholeWord
          ? this._editor.getOption(110 /* wordSeparators */)
          : null,
        false,
        1073741824, /* MAX_SAFE_SMALL_INTEGER */
      );
...
export class AddSelectionToNextFindMatchAction
  extends MultiCursorSelectionControllerAction {
  constructor() {
    super({
...
export class MoveSelectionToNextFindMatchAction
  extends MultiCursorSelectionControllerAction {
  constructor() {
    super({
...
export class MoveSelectionToPreviousFindMatchAction
  extends MultiCursorSelectionControllerAction {
  constructor() {
    super({
...
export class SelectHighlightsAction
  extends MultiCursorSelectionControllerAction {
  constructor() {
    super({
...
export class SelectionHighlighter extends Disposable {
  constructor(editor) {
    ...
    this._register(
      CommonFindController.get(editor).getState().onFindReplaceStateChange(
        (e) => {
          this._update();
        },
      ),
  ...
  static _createState(isEnabled, editor) {
    ...
    if (findState.isRevealed) {
      ...
      if (
        ...
        r.wholeWord === findState.wholeWord && !findState.isRegex
      ) {
        return null;
      }
    }
    return new SelectionHighlighterState(
      r.searchText,
      r.matchCase,
      r.wholeWord ? editor.getOption(110 /* wordSeparators */) : null,
  ...
  _setState(state) {
    ...
    const hasFindOccurrences = DocumentHighlightProviderRegistry.has(model) &&
      this.editor.getOption(66 /* occurrencesHighlight */);
    let allMatches = model.findMatches(
    ...
    if (
      currentFindState.isRegex || currentFindState.matchCase ||
      currentFindState.wholeWord
    ) {
...
SelectionHighlighter._SELECTION_HIGHLIGHT_OVERVIEW = ModelDecorationOptions
  .register({
