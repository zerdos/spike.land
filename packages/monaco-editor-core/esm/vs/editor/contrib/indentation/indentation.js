...
export function getReindentEditOperations(
  ...
  if (inheritedIndent !== undefined && inheritedIndent !== null) {
    ...
    adjustedLineContent = globalIndent +
      currentLineText.substring(oldIndentation.length);
    if (
  ...
  for (
    ...
    let adjustedLineContent = idealIndentForNextLine +
      text.substring(oldIndentation.length);
    if (
      indentationRules.decreaseIndentPattern &&
      indentationRules.decreaseIndentPattern.test(adjustedLineContent)
    ) {
...
export class AutoIndentOnPasteCommand {
  ...
  getEditOperations(model, builder) {
    ...
    if (
      Array.isArray(this._edits) && this._edits.length === 1 &&
...
export class AutoIndentOnPaste {
  ...
  update() {
    ...
    // we are disabled
    if (
      this.editor.getOption(8 /* autoIndent */) < 4 /* Full */ ||
      this.editor.getOption(42 /* formatOnPaste */)
    ) {
      return;
    }
    // no model
  ...
  trigger(range) {
    ...
    if (!/\S/.test(firstLineText.substring(0, range.startColumn - 1))) {
      ...
      if (indentOfFirstLine !== null) {
        ...
        if (newSpaceCnt !== oldSpaceCnt) {
          ...
          firstLineText = newIndent +
            firstLineText.substr(oldIndentation.length);
        } else {
          ...
          if (
            indentMetadata === 0 || indentMetadata === 8 /* UNINDENT_MASK */
          ) {
    ...
    if (startLineNumber !== range.endLineNumber) {
      ...
      let indentOfSecondLine = LanguageConfigurationRegistry
        .getGoodIndentForLine(
          autoIndent,
          virtualModel,
          model.getLanguageIdentifier().id,
      ...
      if (indentOfSecondLine !== null) {
        ...
        if (newSpaceCntOfSecondLine !== oldSpaceCntOfSecondLine) {
          let spaceCntOffset = newSpaceCntOfSecondLine -
  ...
  shouldIgnoreLine(model, lineNumber) {
    ...
    if (tokens.getCount() > 0) {
      let firstNonWhitespaceTokenIndex = tokens.findTokenIndexAtOffset(
        nonWhitespaceColumn,
      );
      if (
