...
let LinkedEditingContribution = class LinkedEditingContribution
  extends Disposable {
  constructor(editor, contextKeyService) {
    ...
    this._register(this._editor.onDidChangeConfiguration((e) => {
      if (
        e.hasChanged(56 /* linkedEditing */) ||
        e.hasChanged(76 /* renameOnType */)
      ) {
        this.reinitialize();
      }
    }));
    this._register(
      LinkedEditingRangeProviderRegistry.onDidChange(() => this.reinitialize()),
    );
    this._register(
      this._editor.onDidChangeModelLanguage(() => this.reinitialize()),
    );
    this.reinitialize();
  }
  static get(editor) {
  ...
  reinitialize() {
    ...
    const isEnabled = model !== null &&
      (this._editor.getOption(56 /* linkedEditing */) ||
        this._editor.getOption(76 /* renameOnType */)) &&
    ...
    this._languageWordPattern = LanguageConfigurationRegistry.getWordDefinition(
      model.getLanguageIdentifier().id,
    );
    this._localToDispose.add(model.onDidChangeLanguageConfiguration(() => {
      this._languageWordPattern = LanguageConfigurationRegistry
        .getWordDefinition(model.getLanguageIdentifier().id);
    }));
    ...
    this._localToDispose.add(this._editor.onDidChangeModelContent((e) => {
      if (!this._ignoreChangeEvent) {
        if (this._currentDecorations.length > 0) {
          const referenceRange = model.getDecorationRange(
            this._currentDecorations[0],
          );
          if (
            referenceRange &&
  ...
  _syncRanges(decorations) {
    ...
    if (
      !this._editor.hasModel() || decorations !== this._currentDecorations ||
    ...
    try {
      ...
      const prevEditOperationType = this._editor._getViewModel()
  ...
  clearRanges() {
    ...
    this._currentDecorations = this._editor.deltaDecorations(
      this._currentDecorations,
      [],
    );
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const request = createCancelablePromise((token) =>
        __awaiter(this, void 0, void 0, function* () {
          try {
            const response = yield getLinkedEditingRanges(
              model,
              position,
              token,
            );
            ...
            this._currentWordPattern = (response === null || response === void 0
              ? void 0
  ...
  return first(
    orderedByScore.map((provider) =>
      () =>
        __awaiter(this, void 0, void 0, function* () {
          try {
            return yield provider.provideLinkedEditingRanges(
              model,
              position,
              token,
            );
...
registerThemingParticipant((theme, collector) => {
  const editorLinkedEditingBackgroundColor = theme.getColor(
    editorLinkedEditingBackground,
  );
  if (editorLinkedEditingBackgroundColor) {
    collector.addRule(
      `.monaco-editor .${DECORATION_CLASS_NAME} { background: ${editorLinkedEditingBackgroundColor}; border-left-color: ${editorLinkedEditingBackgroundColor}; }`,
    );
  }
});
registerModelAndPositionCommand(
  ...
  (model, position) =>
    getLinkedEditingRanges(model, position, CancellationToken.None),
);
registerEditorContribution(
