...
} from "../../../platform/actions/browser/menuEntryActionViewItem";
...
registerSingleton(
  ...
  class {
    ...
    addExclusiveWidget(editor, widget) {
      ...
      this._widgets.set(
        editor,
        { widget, listener: widget.onDidClose(remove) },
      );
    }
  },
);
...
let PeekViewWidget = class PeekViewWidget extends ZoneWidget {
  ...
  _applyStyles() {
    ...
    if (this._headElement && options.headerBackgroundColor) {
      this._headElement.style.backgroundColor = options.headerBackgroundColor
        .toString();
    }
    ...
    if (this._secondaryHeading && options.secondaryHeadingColor) {
      this._secondaryHeading.style.color = options.secondaryHeadingColor
        .toString();
    }
  ...
  _doLayout(heightInPixel, widthInPixel) {
    ...
    const headHeight = Math.ceil(
      this.editor.getOption(53 /* lineHeight */) * 1.2,
    );
