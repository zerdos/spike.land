...
class MonarchClassicTokensCollector {
  ...
  emit(startOffset, type) {
    if (
      this._lastTokenType === type && this._lastTokenLanguage === this._language
    ) {
      return;
    }
...
export class MonarchTokenizer {
  ...
  getLoadStatus() {
    ...
    for (let nestedModeId in this._embeddedModes) {
      ...
      const tokenizationSupportPromise = modes.TokenizationRegistry.getPromise(
        nestedModeId,
      );
  ...
  _myTokenize(line, lineState, offsetDelta, tokensCollector) {
    ...
    while (forceEvaluation || pos < lineLength) {
      ...
      while (
        ...
        monarchCommon.isIAction(action) && action.test
      ) {
      ...
      if (typeof action === "string" || Array.isArray(action)) {
        ...
        if (action.switchTo && typeof action.switchTo === "string") {
          ...
          if (!monarchCommon.findRules(this._lexer, nextState)) {
            throw monarchCommon.createError(
              this._lexer,
              "trying to switch to a state '" + nextState +
                "' that is undefined in rule: " + this._safeRuleName(rule),
            );
          } else {
          ...
          if (action.next === "@push") {
            ...
            if (!monarchCommon.findRules(this._lexer, nextState)) {
              throw monarchCommon.createError(
                this._lexer,
                "trying to set a next state '" + nextState +
                  "' that is undefined in rule: " + this._safeRuleName(rule),
              );
            } else {
      ...
      const computeNewStateForEmbeddedMode = (enteringEmbeddedMode) => {
        ...
        const embeddedModeData = this._getNestedEmbeddedModeData(
          enteringEmbeddedMode,
        );
        if (pos < lineLength) {
      ...
      if (Array.isArray(result)) {
        ...
        if (matched.length === 0) {
          if (
            lineLength === 0 || stackLen0 !== stack.depth ||
            state !== stack.state || (!groupMatching
                ? 0
                : groupMatching.groups.length) !== groupLen0
          ) {
            continue;
          } else {
            throw monarchCommon.createError(
        ...
        let tokenType = null;
        if (
          monarchCommon.isString(result) && result.indexOf("@brackets") === 0
        ) {
  ...
  _locateMode(mimetypeOrModeId) {
    if (
      !mimetypeOrModeId || !this._modeService.isRegisteredMode(mimetypeOrModeId)
    ) {
      return null;
    }
