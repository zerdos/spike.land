...
function doCreateModel(value, languageSelection, uri) {
  return StaticServices.modelService.get().createModel(
    value,
    languageSelection,
    uri,
  );
...
export function createModel(value, language, uri) {
  ...
  if (!language) {
    ...
    return doCreateModel(
      value,
      StaticServices.modeService.get().createByFilepathOrFirstLine(
        uri || null,
        firstLine,
      ),
      uri,
...
export function setModelLanguage(model, languageId) {
  StaticServices.modelService.get().setMode(
    model,
    StaticServices.modeService.get().create(languageId),
  );
}
/**
...
function getSafeTokenizationSupport(language) {
  ...
  return {
    ...
    tokenize: (line, state, deltaOffset) =>
