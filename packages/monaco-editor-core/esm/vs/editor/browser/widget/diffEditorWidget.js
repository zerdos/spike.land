...
import {
  RenderLineInput,
  renderViewLine,
} from "../../common/viewLayout/viewLineRenderer";
import {
  InlineDecoration,
  ViewLineRenderingData,
} from "../../common/viewModel/viewModel";
...
let DiffEditorWidget = class DiffEditorWidget extends Disposable {
  constructor(
    ...
    const leftScopedInstantiationService = instantiationService.createChild(
      leftServices,
    );
  ...
  _createLeftHandSideEditor(
    ...
    this._register(editor.onDidScrollChange((e) => {
      ...
      if (
        !e.scrollTopChanged && !e.scrollLeftChanged && !e.scrollHeightChanged
      ) {
        return;
      }
    ...
    const isInDiffLeftEditorKey = contextKeyService.createKey(
      "isInDiffLeftEditor",
      undefined,
    );
    this._register(
    ...
    this._register(editor.onDidContentSizeChange((e) => {
      const width = this._originalEditor.getContentWidth() +
  ...
  _createRightHandSideEditor(
    ...
    const isInDiffRightEditorKey = contextKeyService.createKey(
      "isInDiffRightEditor",
      undefined,
    );
    this._register(
  ...
  hasTextFocus() {
    return this._originalEditor.hasTextFocus() ||
      this._modifiedEditor.hasTextFocus();
  }
  ...
  _layoutOverviewRulers() {
    ...
    const freeSpace = DiffEditorWidget.ENTIRE_DIFF_OVERVIEW_WIDTH -
  ...
  _updateDecorations() {
    ...
    const lineChanges =
      (this._diffComputationResult ? this._diffComputationResult.changes : []);
  ...
  _doLayout() {
    ...
    this._originalEditor.layout(
      { width: splitPoint, height: (height - reviewHeight) },
    );
  ...
  _computeOverviewViewport() {
    ...
    const computedRatio = scrollHeight > 0
  ...
  _getEquivalentLineForOriginalLineNumber(lineNumber) {
    ...
    const originalEquivalentLineNumber = lineChange.originalStartLineNumber +
      (lineChange.originalEndLineNumber > 0 ? -1 : 0);
    const modifiedEquivalentLineNumber = lineChange.modifiedStartLineNumber +
    ...
    if (delta <= lineChangeOriginalLength) {
      return modifiedEquivalentLineNumber +
        Math.min(delta, lineChangeModifiedLength);
    }
    return modifiedEquivalentLineNumber + lineChangeModifiedLength -
      lineChangeOriginalLength + delta;
  }
  _getEquivalentLineForModifiedLineNumber(lineNumber) {
    ...
    if (delta <= lineChangeModifiedLength) {
      return originalEquivalentLineNumber +
        Math.min(delta, lineChangeOriginalLength);
    }
    return originalEquivalentLineNumber + lineChangeOriginalLength -
      lineChangeModifiedLength + delta;
  }
...
class DiffEditorWidgetStyle extends Disposable {
  ...
  applyColors(theme) {
    const newInsertColor = (theme.getColor(diffInserted) || defaultInsertColor)
      .transparent(2);
    const newRemoveColor = (theme.getColor(diffRemoved) || defaultRemoveColor)
      .transparent(2);
...
class ViewZonesComputer {
  ...
  getViewZones() {
    ...
    const originalHasWrapping =
      (this._originalEditor.getOption(124 /* wrappingInfo */).wrappingColumn !==
        -1);
    const modifiedHasWrapping =
      (this._modifiedEditor.getOption(124 /* wrappingInfo */).wrappingColumn !==
        -1);
    ...
    for (let i = 0, length = this._lineChanges.length; i <= length; i++) {
      ...
      if (lineChange !== null) {
        originalEquivalentLineNumber = lineChange.originalStartLineNumber +
          (lineChange.originalEndLineNumber > 0 ? -1 : 0);
        modifiedEquivalentLineNumber = lineChange.modifiedStartLineNumber +
          (lineChange.modifiedEndLineNumber > 0 ? -1 : 0);
      ...
      if (hasWrapping) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          const originalViewLineCount = originalCoordinatesConverter
            .getModelLineViewLineCount(originalLineNumber);
          const modifiedViewLineCount = modifiedCoordinatesConverter
            .getModelLineViewLineCount(modifiedLineNumber);
        ...
        if (lineChange) {
          lastOriginalLineNumber = (lineChange.originalEndLineNumber > 0
            ? lineChange.originalEndLineNumber
            : lineChange.originalStartLineNumber) + 1;
          lastModifiedLineNumber = (lineChange.modifiedEndLineNumber > 0
            ? lineChange.modifiedEndLineNumber
            : lineChange.modifiedStartLineNumber) + 1;
        }
      }
      ...
      while (
        ...
        if (
          ...
          viewZoneLineNumber = originalEquivalentLineNumber -
            modifiedEquivalentLineNumber +
            modifiedForeignVZ.current.afterLineNumber;
        } else {
      ...
      while (
        ...
        if (
          ...
          viewZoneLineNumber = modifiedEquivalentLineNumber -
            originalEquivalentLineNumber +
            originalForeignVZ.current.afterLineNumber;
        } else {
      ...
      while (
        ...
        const originalDelta = original.afterLineNumber -
          originalEquivalentLineNumber;
        const modifiedDelta = modified.afterLineNumber -
          modifiedEquivalentLineNumber;
...
const DECORATIONS = {
  ...
  lineInsertWithSign: ModelDecorationOptions.register({
    ...
    linesDecorationsClassName: "insert-sign " +
      ThemeIcon.asClassName(diffInsertIcon),
    marginClassName: "line-insert",
    isWholeLine: true,
  }),
  ...
  lineDeleteWithSign: ModelDecorationOptions.register({
    ...
    linesDecorationsClassName: "delete-sign " +
      ThemeIcon.asClassName(diffRemoveIcon),
    marginClassName: "line-delete",
    isWholeLine: true,
  }),
...
class DiffEditorWidgetSideBySide extends DiffEditorWidgetStyle {
  constructor(dataSource, enableSplitViewResizing) {
    ...
    this._sash = this._register(
      new Sash(
        this._dataSource.getContainerDomNode(),
        this,
  ...
  layout(sashRatio = this._sashRatio) {
    ...
    if (contentWidth > DiffEditorWidgetSideBySide.MINIMUM_EDITOR_WIDTH * 2) {
      ...
      if (
        ...
        sashPosition = contentWidth -
          DiffEditorWidgetSideBySide.MINIMUM_EDITOR_WIDTH;
      }
    } else {
...
class DiffEditorWidgetInline extends DiffEditorWidgetStyle {
  constructor(dataSource, enableSplitViewResizing) {
    super(dataSource);
    this._decorationsLeft =
      dataSource.getOriginalEditor().getLayoutInfo().decorationsLeft;
    this._register(
...
class InlineViewZonesComputer extends ViewZonesComputer {
  constructor(
    ...
    this._lineBreaksComputer = this._modifiedEditor._getViewModel()
  ...
  _produceModifiedFromDiff(
    ...
    domNode.className =
      `view-lines line-delete ${MOUSE_CURSOR_TEXT_CSS_CLASS_NAME}`;
    const marginDomNode = document.createElement("div");
  ...
  _finalize(result) {
    ...
    const disableMonospaceOptimizations = modifiedEditorOptions.get(
      26, /* disableMonospaceOptimizations */
    );
    const typicalHalfwidthCharacterWidth =
      fontInfo.typicalHalfwidthCharacterWidth;
    const scrollBeyondLastColumn = modifiedEditorOptions.get(
      88, /* scrollBeyondLastColumn */
    );
    const mightContainNonBasicASCII = this._originalModel
    ...
    for (let i = 0; i < this._pendingLineChange.length; i++) {
      ...
      viewZone.minWidthInPx =
        (maxCharsPerLine * typicalHalfwidthCharacterWidth);
      if (viewLineCounts) {
        // make sure all lines have an entry in `viewLineCounts`
        const cnt = lineChange.originalEndLineNumber -
          lineChange.originalStartLineNumber;
  ...
  _renderOriginalLine(
    ...
    if (this._renderIndicators) {
      ...
      marginElement.className = `delete-sign ${
        ThemeIcon.asClassName(diffRemoveIcon)
      }`;
      marginElement.setAttribute(
        "style",
        `position:absolute;top:${renderedLineCount *
          lineHeight}px;width:${lineDecorationsWidth}px;height:${lineHeight}px;right:0;`,
      );
