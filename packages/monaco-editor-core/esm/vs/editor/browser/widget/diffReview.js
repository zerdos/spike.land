...
export class DiffReview extends Disposable {
  ...
  static _mergeAdjacent(lineChanges, originalLineCount, modifiedLineCount) {
    ...
    for (let i = 0, len = lineChanges.length; i < len; i++) {
      ...
      // Emit before anchors
      {
        const originalEqualAbove = (originalEnd === 0
          ? originalStart
          : originalStart - 1);
        const modifiedEqualAbove = (modifiedEnd === 0
          ? modifiedStart
          : modifiedStart - 1);
  ...
  _render() {
    ...
    Configuration.applyFontInfoSlow(
      container,
      modifiedOptions.get(38 /* fontInfo */),
    );
    ...
    cell.appendChild(
      document.createTextNode(
        `${diffIndex +
          1}/${this._diffs.length}: @@ -${minOriginalLine},${originalChangedLinesCnt} +${minModifiedLine},${modifiedChangedLinesCnt} @@`,
      ),
    );
  ...
  static _renderSection(
    ...
    const originalLayoutInfo = originalOptions.get(123 /* layoutInfo */);
    const originalLineNumbersWidth = originalLayoutInfo.glyphMarginWidth +
      originalLayoutInfo.lineNumbersWidth;
    const modifiedLayoutInfo = modifiedOptions.get(123 /* layoutInfo */);
    const modifiedLineNumbersWidth = 10 + modifiedLayoutInfo.glyphMarginWidth +
      modifiedLayoutInfo.lineNumbersWidth;
    for (let i = 0; i <= cnt; i++) {
      const originalLine = (originalLineStart === 0
        ? 0
        : originalLineStart + i);
      const modifiedLine = (modifiedLineStart === 0
        ? 0
        : modifiedLineStart + i);
      ...
      originalLineNumber.className = "diff-review-line-number" +
        lineNumbersExtraClassName;
      if (originalLine !== 0) {
      ...
      modifiedLineNumber.className = "diff-review-line-number" +
        lineNumbersExtraClassName;
      if (modifiedLine !== 0) {
  ...
  static _renderLine(model, options, tabSize, lineNumber) {
    ...
    const r = renderViewLine(
      new RenderLineInput(
        ...
        options.get(100 /* stopRenderingLineAfter */),
        options.get(83 /* renderWhitespace */),
        options.get(77 /* renderControlCharacters */),
        options.get(39 /* fontLigatures */) !== EditorFontLigatures.OFF,
        null,
      ),
    );
    return r.html;
