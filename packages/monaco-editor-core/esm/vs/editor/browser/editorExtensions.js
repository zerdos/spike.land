...
import { assertType, withNullAsUndefined } from "../../base/common/types";
...
export class EditorAction extends EditorCommand {
  ...
  reportTelemetry(accessor, editor) {
    accessor.get(ITelemetryService).publicLog2(
      "editorActionInvoked",
...
export function registerModelAndPositionCommand(id, handler) {
  CommandsRegistry.registerCommand(id, function (accessor, ...args) {
    ...
    return accessor.get(ITextModelService).createModelReference(resource).then(
      (reference) => {
...
registerCommand(
  new ProxyCommand(
    UndoCommand,
    { id: "default:undo", precondition: undefined },
  ),
);
...
registerCommand(
  new ProxyCommand(
    RedoCommand,
    { id: "default:redo", precondition: undefined },
  ),
);
