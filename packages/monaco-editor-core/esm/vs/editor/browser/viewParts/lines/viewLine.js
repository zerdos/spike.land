...
export class ViewLine {
  ...
  renderLine(lineNumber, deltaTop, viewportData, sb) {
    ...
    if (
      monospaceAssumptionsAreValid && canUseFastRenderedViewLine &&
      lineData.isBasicASCII && options.useMonospaceOptimizations &&
  ...
  getVisibleRangesForRange(startColumn, endColumn, context) {
    ...
    if (
      stopRenderingLineAfter !== -1 && startColumn > stopRenderingLineAfter + 1
    ) {
      startColumn = stopRenderingLineAfter + 1;
    }
    if (
      stopRenderingLineAfter !== -1 && endColumn > stopRenderingLineAfter + 1
    ) {
      endColumn = stopRenderingLineAfter + 1;
    }
...
class RenderedViewLine {
  ...
  _actualReadPixelOffset(domNode, column, context) {
    ...
    if (
      column === this._characterMapping.length && this._isWhitespaceOnly &&
...
class WebKitRenderedViewLine extends RenderedViewLine {
  _readVisibleRangesForRange(domNode, startColumn, endColumn, context) {
    ...
    if (
      !output || output.length === 0 || startColumn === endColumn ||
