...
} from "../../../../base/browser/globalMouseMoveMonitor";
...
class MinimapOptions {
  constructor(configuration, theme, tokensColorTracker) {
    ...
    const pixelRatio = options.get(121 /* pixelRatio */);
...
class MinimapLayout {
  ...
  static create(
    ...
    if (options.minimapHeightIsEditorHeight) {
      ...
      const computedSliderRatio = (maxMinimapSliderTop) /
    ...
    if (viewportContainsWhitespaceGaps && viewportEndLineNumber !== lineCount) {
      ...
      const viewportLineCount = viewportEndLineNumber -
...
class MinimapSamplingState {
  ...
  static compute(options, viewLineCount, oldSamplingState) {
    ...
    const { minimapLineCount } = EditorLayoutInfoComputer
    ...
    for (let i = 0; i < minimapLineCount; i++) {
      ...
      while (
        oldIndex < oldLength && oldMinimapLines[oldIndex] < fromViewLineNumber
      ) {
        if (events.length < MAX_EVENT_COUNT) {
          const oldMinimapLineNumber = oldIndex + 1 + oldDeltaLineCount;
          if (
            lastEvent && lastEvent.type === "deleted" &&
      ...
      if (
        oldIndex < oldLength && oldMinimapLines[oldIndex] <= toViewLineNumber
      ) {
        ...
        if (events.length < MAX_EVENT_COUNT) {
          const oldMinimapLineNumber = oldIndex + 1 + oldDeltaLineCount;
          if (
            lastEvent && lastEvent.type === "inserted" && lastEvent._i === i - 1
          ) {
  ...
  modelLineRangeToMinimapLineRange(fromLineNumber, toLineNumber) {
    ...
    if (fromLineIndex === toLineIndex) {
      ...
      if (
        sampledLineNumber < fromLineNumber || sampledLineNumber > toLineNumber
      ) {
  ...
  decorationLineRangeToMinimapLineRange(startLineNumber, endLineNumber) {
    ...
    if (
      startLineNumber !== endLineNumber && minimapLineEnd === minimapLineStart
    ) {
...
export class Minimap extends ViewPart {
  ...
  onLinesChanged(e) {
    if (this._samplingState) {
      const minimapLineRange = this._samplingState
        .modelLineRangeToMinimapLineRange(e.fromLineNumber, e.toLineNumber);
      if (minimapLineRange) {
  ...
  onLinesDeleted(e) {
    if (this._samplingState) {
      const [changeStartIndex, changeEndIndex] = this._samplingState
        .onLinesDeleted(e);
    ...
    if (this._minimapSelections === null) {
      if (this._samplingState) {
        ...
        for (const selection of this._selections) {
          const [minimapLineStart, minimapLineEnd] = this._samplingState
            .decorationLineRangeToMinimapLineRange(
  ...
  getMinimapDecorationsInViewport(startLineNumber, endLineNumber) {
    ...
    if (this._samplingState) {
      ...
      for (const decoration of decorations) {
        ...
        const minimapStartLineNumber = this._samplingState
...
class InnerMinimap extends Disposable {
      ...
      (e) => {
        ...
        const internalOffsetY =
          (this._model.options.canvasInnerHeight /
            this._model.options.canvasOuterHeight) * e.browserEvent.offsetY;
  ...
  _startSliderDragging(
    ...
    const handleMouseMove = (posy, posx) => {
      ...
      if (
        platform.isWindows && mouseOrthogonalDelta > MOUSE_DRAG_RESET_DISTANCE
      ) {
  ...
  scrollDueToTouchEvent(touch) {
    ...
    const scrollTop = this._lastRenderData.renderedLayout
      .getDesiredScrollTopFromTouchLocation(touch.pageY - startY);
  ...
  render(renderingCtx) {
    ...
    const scrollLeftChars = renderingCtx.scrollLeft /
  ...
  renderDecorations(layout) {
    if (this._renderDecorations) {
      ...
      for (let i = 0; i < decorations.length; i++) {
        ...
        const decorationColor = decoration.options.minimap.getColor(
          this._theme,
        );
        for (
  ...
  renderDecorationOnLine(
    ...
    const { startColumn, endColumn, startLineNumber, endLineNumber } =
      decorationRange;
    const x = startLineNumber === lineNumber
      ? lineIndexToXOffset[startColumn - 1]
      : MINIMAP_GUTTER_WIDTH;
    const endColumnForLine = endLineNumber > lineNumber
    ...
    for (
      ...
      if (copySourceEnd === sourceStart && copyDestEnd === destStart) {
        ...
        if (copySourceStart !== -1) {
          ...
          if (
            dirtyPixel1 === -1 && copySourceStart === 0 &&
            copySourceStart === copyDestStart
          ) {
            dirtyPixel1 = copySourceEnd;
          }
          if (
            dirtyPixel2 === -1 && copySourceEnd === maxDestPixel &&
            copySourceStart === copyDestStart
          ) {
  ...
  static _renderLine(
    ...
    for (
      ...
      for (; charIndex < tokenEndIndex; charIndex++) {
        ...
        if (charCode === 9 /* Tab */) {
          const insertSpacesCount = tabSize -
            (charIndex + tabsCharDelta) % tabSize;
