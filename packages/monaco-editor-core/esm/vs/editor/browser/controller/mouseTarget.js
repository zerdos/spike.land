...
export class MouseTarget {
  ...
  static toString(target) {
    return this._typeToString(target.type) + ": " + target.position + " - " +
      target.range + " - " + target.detail;
  }
  toString() {
...
export class HitTestContext {
  constructor(context, viewHelper, lastRenderData) {
    ...
    this.typicalHalfwidthCharacterWidth =
      options.get(38 /* fontInfo */).typicalHalfwidthCharacterWidth;
  ...
  static getZoneAtCoord(context, mouseVerticalOffset) {
    ...
    if (viewZoneWhitespace) {
      let viewZoneMiddle = viewZoneWhitespace.verticalOffset +
...
export class MouseTargetFactory {
  ...
  static _createMouseTarget(ctx, request, domHitTestExecuted) {
    ...
    result = result ||
      MouseTargetFactory._hitTestViewZone(ctx, resolvedRequest);
    result = result || MouseTargetFactory._hitTestMargin(ctx, resolvedRequest);
    result = result ||
      MouseTargetFactory._hitTestViewCursor(ctx, resolvedRequest);
    result = result ||
      MouseTargetFactory._hitTestTextArea(ctx, resolvedRequest);
    result = result ||
  ...
  getMouseColumn(editorPos, pos) {
    ...
    const mouseContentHorizontalOffset =
      this._context.viewLayout.getCurrentScrollLeft() + pos.x - editorPos.x -
  ...
  static _doHitTestWithCaretRangeFromPoint(ctx, request) {
    ...
    const lineCenteredVerticalOffset = lineVerticalOffset +
      Math.floor(ctx.lineHeight / 2);
  ...
  static _doHitTestWithCaretPositionFromPoint(ctx, coords) {
    const hitResult = document.caretPositionFromPoint(
      coords.clientX,
      coords.clientY,
    );
  ...
  static _doHitTest(ctx, request) {
    ...
    if (typeof document.caretRangeFromPoint === "function") {
      ...
      result = this._doHitTestWithMoveToPoint(
        ctx,
        request.pos.toClientCoordinates(),
      );
    } else {
...
export function shadowCaretRangeFromPoint(shadowRoot, x, y) {
  ...
  if (el !== null) {
    ...
    while (
      el && el.firstChild &&
      el.firstChild.nodeType !== el.firstChild.TEXT_NODE && el.lastChild &&
