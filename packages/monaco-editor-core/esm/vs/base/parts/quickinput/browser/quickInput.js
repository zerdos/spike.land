...
import { ItemActivation, NO_KEY_MODS } from "../common/quickInput";
...
import { renderCodicons } from "../../../browser/codicons";
const $ = dom.$;
const backButtonIcon = registerCodicon(
  "quick-input-back",
  Codicon.arrowLeft,
  localize(
    "backButtonIcon",
...
class QuickInput extends Disposable {
  ...
  update() {
    ...
    if (this.buttonsUpdated) {
      ...
      const leftButtons = this.buttons.filter((button) =>
        button === backButton
      );
      ...
      const rightButtons = this.buttons.filter((button) =>
        button !== backButton
      );
...
class QuickPick extends QuickInput {
  constructor() {
    ...
    this._itemActivation = this.ui.isScreenReaderOptimized()
      ? ItemActivation
        .NONE /* https://github.com/microsoft/vscode/issues/57501 */
        ...
        this.ui.list.onDidChangeSelection(({ items: selectedItems, event }) => {
          ...
          if (selectedItems.length) {
            this.onDidAcceptEmitter.fire(
              {
                inBackground: event instanceof MouseEvent &&
      ...
      (e) => {
        ...
        const wasTriggerKeyPressed = quickNavKeys.some((k) => {
          ...
          if (firstPart.shiftKey && keyCode === 4 /* Shift */) {
            if (
              keyboardEvent.ctrlKey || keyboardEvent.altKey ||
  ...
  update() {
    ...
    if (this._lastValidationMessage !== validationMessage) {
      ...
      this.showMessageDecoration(
        this.validationMessage ? Severity.Error : Severity.Ignore,
      );
    }
...
export class QuickInputController extends Disposable {
  ...
  getUI() {
    ...
    this._register(list.onLeave(() => {
      ...
      setTimeout(() => {
        inputBox.setFocus();
        if (
          this.controller instanceof QuickPick && this.controller.canSelectMany
        ) {
          list.clearFocus();
        }
      }, 0);
          ...
          case 2 /* Tab */:
            if (!event.altKey && !event.ctrlKey && !event.metaKey) {
              ...
              if (event.shiftKey && event.target === stops[0]) {
                ...
                !event.shiftKey && event.target === stops[stops.length - 1]
              ) {
                dom.EventHelper.stop(e, true);
                stops[0].focus();
              }
  ...
  pick(picks, options = {}, token = CancellationToken.None) {
    return new Promise((doResolve, reject) => {
      ...
      input.matchOnLabel = (options.matchOnLabel === undefined) ||
        options.matchOnLabel; // default to true
      input.autoFocusOnList = (options.autoFocusOnList === undefined) ||
      ...
      Promise.all([picks, options.activeItem])
        .then(([items, _activeItem]) => {
          ...
          input.items = items;
          if (input.canSelectMany) {
            input.selectedItems = items.filter((item) =>
              item.type !== "separator" && item.picked
            );
          }
          if (activeItem) {
            input.activeItems = [activeItem];
  ...
  hide() {
    ...
    if (controller) {
      ...
      if (!focusChanged) {
        if (
          this.previousFocusElement && this.previousFocusElement.offsetParent
        ) {
