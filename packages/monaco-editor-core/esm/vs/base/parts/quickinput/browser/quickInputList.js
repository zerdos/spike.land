    ...
    if (buttons && buttons.length) {
      data.actionBar.push(
        buttons.map((button, index) => {
          let cssClasses = button.iconClass ||
...
export class QuickInputList {
  ...
  get onDidChangeSelection() {
    return Event.map(
      ...
      (e) => ({ items: e.elements.map((e) => e.item), event: e.browserEvent }),
    );
  }
  ...
  setElements(inputElements) {
    ...
    this.elements = inputElements.reduce((result, item, index) => {
      ...
      if (item.type !== "separator") {
        ...
        const saneDescription = item.description &&
          item.description.replace(/\r?\n/g, " ");
        const saneDetail = item.detail && item.detail.replace(/\r?\n/g, " ");
        const saneAriaLabel = item.ariaLabel ||
        ...
        result.push(
          new ListElement({
            ...
            saneDescription,
            saneDetail,
            labelHighlights: (_a = item.highlights) === null || _a === void 0
              ? void 0
              : _a.label,
            descriptionHighlights:
              (_b = item.highlights) === null || _b === void 0
                ? void 0
                : _b.description,
            detailHighlights: (_c = item.highlights) === null || _c === void 0
              ? void 0
              : _c.detail,
            checked: false,
            separator: previous && previous.type === "separator"
              ? previous
              : undefined,
            fireButtonTriggered,
          }),
  ...
  focus(what) {
    ...
    if (
      what === QuickInputListFocus.Previous && this.list.getFocus()[0] === 0
    ) {
  ...
  filter(query) {
    if (
      !(this.sortByLabel || this.matchOnLabel || this.matchOnDescription ||
        this.matchOnDetail)
    ) {
      this.list.layout();
      return false;
    ...
    if (
      ...
      this.elements.forEach((element) => {
        ...
        element.separator = previous && previous.type === "separator"
          ? previous
          : undefined;
      });
      ...
      this.elements.forEach((element) => {
        const labelHighlights = this.matchOnLabel
          ? withNullAsUndefined(
            matchesFuzzyCodiconAware(query, parseCodicons(element.saneLabel)),
          )
          : undefined;
        const descriptionHighlights = this.matchOnDescription
          ? withNullAsUndefined(
            matchesFuzzyCodiconAware(
              query,
              parseCodicons(element.saneDescription || ""),
            ),
          )
          : undefined;
        const detailHighlights = this.matchOnDetail
          ? withNullAsUndefined(
            matchesFuzzyCodiconAware(
              query,
              parseCodicons(element.saneDetail || ""),
            ),
          )
          : undefined;
