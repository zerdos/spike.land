...
import { binarySearch, range } from "../../../common/arrays";
...
class TraitRenderer {
  ...
  renderElement(element, index, templateData) {
    const renderedElementIndex = this.renderedElements.findIndex((el) =>
  ...
  disposeTemplate(templateData) {
    const index = this.renderedElements.findIndex((el) =>
      el.templateData === templateData
    );
    if (index < 0) {
      return;
...
class Trait {
  ...
  splice(start, deleteCount, elements) {
    ...
    const indexes = [
      ...
      ...elements.map((hasTrait, i) => hasTrait ? i + start : -1).filter((i) =>
        i !== -1
      ),
...
class TraitSpliceable {
  ...
  splice(start, deleteCount, elements) {
    ...
    const pastElementsWithTrait = this.trait.get().map((i) =>
      this.identityProvider.getId(this.view.element(i)).toString()
    );
    const elementsWithTrait = elements.map((e) =>
      pastElementsWithTrait.indexOf(this.identityProvider.getId(e).toString()) >
        -1
    );
...
class KeyboardController {
  constructor(list, view, options) {
    ...
    onKeyDown.filter((e) => e.keyCode === 3 /* Enter */).on(
      this.onEnter,
      this,
      this.disposables,
    );
    onKeyDown.filter((e) => e.keyCode === 16 /* UpArrow */).on(
      this.onUpArrow,
      this,
      this.disposables,
    );
    onKeyDown.filter((e) => e.keyCode === 18 /* DownArrow */).on(
      this.onDownArrow,
      this,
      this.disposables,
    );
    onKeyDown.filter((e) => e.keyCode === 11 /* PageUp */).on(
      this.onPageUpArrow,
      this,
      this.disposables,
    );
    onKeyDown.filter((e) => e.keyCode === 12 /* PageDown */).on(
      this.onPageDownArrow,
      this,
      this.disposables,
    );
    onKeyDown.filter((e) => e.keyCode === 9 /* Escape */).on(
      this.onEscape,
      this,
      this.disposables,
    );
...
export const DefaultKeyboardNavigationDelegate = new class {
  mightProducePrintableCharacter(event) {
    ...
    return (event.keyCode >= 31 /* KEY_A */ &&
      event.keyCode <= 56 /* KEY_Z */) ||
  ...
  onInput(word) {
    ...
    for (let i = 0; i < this.list.length; i++) {
      ...
      const label = this.keyboardNavigationLabelProvider
        .getKeyboardNavigationLabel(this.view.element(index));
...
class DOMFocusController {
  constructor(list, view) {
    ...
    const onKeyDown = Event.chain(domEvent(view.domNode, "keydown"))
      ...
      .map((e) => new StandardKeyboardEvent(e));
    onKeyDown.filter((e) =>
      e.keyCode === 2 /* Tab */ && !e.ctrlKey && !e.metaKey && !e.shiftKey &&
      !e.altKey
    )
...
export class MouseController {
  constructor(list) {
    ...
    this.mouseSupport = typeof list.options.mouseSupport === "undefined" ||
  ...
  isSelectionChangeEvent(event) {
    return this.isSelectionSingleChangeEvent(event) ||
...
export class List {
  constructor(
    ...
    renderers = renderers.map((r) =>
      new PipelineRenderer(r.templateId, [...baseRenderers, r])
    );
    ...
    if (
      typeof _options.keyboardSupport !== "boolean" || _options.keyboardSupport
    ) {
  ...
  get onContextMenu() {
    const fromKeydown = Event.chain(domEvent(this.view.domNode, "keydown"))
      .map((e) => new StandardKeyboardEvent(e))
      .filter((e) =>
        this.didJustPressContextMenuKey = e.keyCode === 58 /* ContextMenu */ ||
    ...
    const fromKeyup = Event.chain(domEvent(this.view.domNode, "keyup"))
      ...
      .filter(() =>
        this.getFocus().length > 0 && !!this.view.domElement(this.getFocus()[0])
      )
      .map((browserEvent) => {
  ...
  focusNextPage(browserEvent, filter) {
    ...
    if (currentlyFocusedElement !== lastPageElement) {
      ...
      this.view.setScrollTop(
        previousScrollTop + this.view.renderHeight -
