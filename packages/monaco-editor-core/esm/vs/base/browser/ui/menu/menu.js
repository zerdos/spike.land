...
import {
  ...
  registerCodicon,
  stripCodicons,
} from "../../../common/codicons";
import {
  ActionViewItem,
  BaseActionViewItem,
} from "../actionbar/actionViewItems";
      ...
      this._register(
        addDisposableListener(menuElement, EventType.KEY_DOWN, (e) => {
          ...
          if (event.equals(14 /* Home */) || event.equals(11 /* PageUp */)) {
            ...
            event.equals(13 /* End */) || event.equals(12 /* PageDown */)
          ) {
            this.focusedItem = 0;
            this.focusPrevious();
            EventHelper.stop(e, true);
...
class BaseMenuActionViewItem extends BaseActionViewItem {
  constructor(ctx, action, options = {}) {
    ...
    if (this.options.label && options.enableMnemonics) {
      ...
      if (label) {
        ...
        if (matches) {
          this.mnemonic = (!!matches[1] ? matches[1] : matches[3])
            .toLocaleLowerCase();
        }
      }
    }
  ...
  updateTooltip() {
    ...
    if (this.getAction().tooltip) {
      ...
      !this.options.label && this.getAction().label && this.options.icon
    ) {
  ...
  applyStyle() {
    ...
    const bgColor = isSelected && this.menuStyle.selectionBackgroundColor
      ...
      : undefined;
    const border = isSelected && this.menuStyle.selectionBorderColor
...
class SubmenuMenuActionViewItem extends BaseMenuActionViewItem {
    ...
    this._register(
      addDisposableListener(this.element, EventType.KEY_DOWN, (e) => {
        ...
        if (getActiveElement() === this.item) {
          if (
            event.equals(17 /* RightArrow */) || event.equals(3 /* Enter */)
          ) {
            EventHelper.stop(e, true);
          }
        }
  ...
  calculateSubmenuMenuLayout(
    ...
    ret.left = layout(
      ...
      submenu.width,
      {
        position: expandDirection === Direction.Right
          ? 0 /* Before */
          : 1, /* After */
        offset: entry.left,
        size: entry.width,
    ...
    ret.top = layout(
      ...
      submenu.height,
      { position: 0, /* Before */ offset: entry.top, size: 0 },
    );
