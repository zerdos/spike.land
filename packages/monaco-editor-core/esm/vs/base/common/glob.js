      ...
      for (const char of segment) {
        ...
        switch (char) {
          ...
          case "}":
            ...
            const braceRegExp = `(?:${
              choices.map((c) => parseRegExp(c)).join("|")
            })`;
            regEx += braceRegExp;
            inBraces = false;
            braceVal = "";
...
function trivia2(base, originalPattern) {
  ...
  const parsedPattern = function (path, basename) {
    ...
    return path === base || path.endsWith(slashBase) ||
...
function trivia3(pattern, options) {
  ...
  const withBasenames = parsedPatterns.find((pattern) =>
    !!pattern.allBasenames
  );
  if (withBasenames) {
...
function trivia4and5(path, pattern, matchPathEnds) {
  const nativePath = paths.sep !== paths.posix.sep
    ? path.replace(ALL_FORWARD_SLASHES, paths.sep)
    : path;
  const nativePathEnd = paths.sep + nativePath;
  const parsedPattern = matchPathEnds
    ? function (path, basename) {
      return typeof path === "string" &&
          (path === nativePath || path.endsWith(nativePathEnd))
        ? pattern
        : null;
    }
    : function (path, basename) {
...
function parsedExpression(expression, options) {
  ...
  if (
    !parsedPatterns.some((parsedPattern) => !!parsedPattern.requiresSiblings)
  ) {
    if (n === 1) {
      return parsedPatterns[0];
    }
...
function aggregateBasenameMatches(parsedPatterns, result) {
  const basenamePatterns = parsedPatterns.filter((parsedPattern) =>
    !!parsedPattern.basenames
  );
  ...
  const aggregatedPatterns = parsedPatterns.filter((parsedPattern) =>
    !parsedPattern.basenames
  );
