...
import { basename, isEqual, relativePath } from "./resources";
/**
...
export function getPathLabel(resource, userHomeProvider, rootProvider) {
  ...
  if (rootProvider) {
    ...
    if (baseResource) {
      ...
      if (hasMultipleRoots) {
        const rootName = baseResource.name
          ? baseResource.name
          : basename(baseResource.uri);
...
export function tildify(path, userHome) {
  ...
  let normalizedUserHome = normalizedUserHomeCached.original === userHome
  ...
  if (
    isLinux
      ? path.startsWith(normalizedUserHome)
      : startsWithIgnoreCase(path, normalizedUserHome)
  ) {
