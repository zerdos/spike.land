...
import { _util, IInstantiationService, optional } from "./instantiation";
...
export class InstantiationService {
  ...
  _createInstance(ctor, args = [], _trace) {
    ...
    let serviceDependencies = _util.getServiceDependencies(ctor).sort((a, b) =>
      a.index - b.index
    );
    let serviceArgs = [];
    ...
    if (args.length !== firstServiceArgPos) {
      console.warn(
        `[createInstance] First service dependency of ${ctor.name} at position ${firstServiceArgPos +
          1} conflicts with ${args.length} static arguments`,
      );
  ...
  _getOrCreateServiceInstance(id, _trace) {
    ...
    if (thing instanceof SyncDescriptor) {
      return this._createAndCacheServiceInstance(
        id,
        thing,
  ...
  _createServiceInstance(
    ...
    if (!_supportsDelayedInstantiation) {
      ...
      const idle = new IdleValue(() =>
        this._createInstance(ctor, args, _trace)
      );
...
class Trace {
  ...
  static traceCreation(ctor) {
    return !_enableTracing
      ? Trace._None
      : new Trace(0, /* Creation */ ctor.name);
  }
  branch(id, first) {
  ...
  stop() {
    ...
    let lines = [
      ...
      `${printChild(1, this)}`,
      `DONE, took ${dur.toFixed(2)}ms (grand total ${
        Trace._totals.toFixed(2)
      }ms)`,
    ];
