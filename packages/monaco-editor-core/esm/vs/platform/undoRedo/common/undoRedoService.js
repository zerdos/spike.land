    ...
    if (isValid) {
      ...
      if (!this.invalidatedResources.has(strResource)) {
        this.invalidatedResources.set(
          strResource,
          new ResourceReasonPair(resourceLabel, 0 /* ExternalRemoval */),
        );
      }
    }
  }
...
let UndoRedoService = class UndoRedoService {
  ...
  _findClosestUndoElementWithSource(sourceId) {
    ...
    for (const [strResource, editStack] of this._editStacks) {
      ...
      if (candidate.sourceId === sourceId) {
        if (
          !matchedElement || candidate.sourceOrder > matchedElement.sourceOrder
        ) {
  ...
  _invokeResourcePrepare(element, callback) {
    if (
      element.actual.type !== 1 /* Workspace */ ||
  ...
  _isPartOfUndoGroup(element) {
    ...
    for (const [, editStack] of this._editStacks) {
      ...
      if (pastElement === element) {
        ...
        if (
          secondPastElement && secondPastElement.groupId === element.groupId
        ) {
  ...
  _confirmAndExecuteWorkspaceUndo(strResource, element, editStackSnapshot) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      return this._safeInvokeWithLocks(
        ...
        editStackSnapshot,
        cleanup,
        () => this._continueUndoInGroup(element.groupId),
      );
    });
  }
  ...
  _findClosestUndoElementInGroup(groupId) {
    ...
    for (const [strResource, editStack] of this._editStacks) {
      ...
      if (candidate.groupId === groupId) {
        if (
          !matchedElement || candidate.groupOrder > matchedElement.groupOrder
        ) {
          matchedElement = candidate;
          matchedStrResource = strResource;
        }
  ...
  undo(resourceOrSource) {
    if (resourceOrSource instanceof UndoRedoSource) {
      ...
      return matchedStrResource
        ? this._undo(matchedStrResource, resourceOrSource.id)
        : undefined;
    }
  ...
  _undo(strResource, sourceId = 0) {
    ...
    if (element.groupId) {
      ...
      const [matchedElement, matchedStrResource] = this
        ._findClosestUndoElementInGroup(element.groupId);
  ...
  _redo(strResource) {
    ...
    if (element.groupId) {
      ...
      const [matchedElement, matchedStrResource] = this
        ._findClosestRedoElementInGroup(element.groupId);
      if (element !== matchedElement && matchedStrResource) {
