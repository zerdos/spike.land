...
let MenuEntryActionViewItem = class MenuEntryActionViewItem
    ...
    if (this._action.alt) {
      this._register(this._altKey.event((value) => {
        alternativeKeyDown = value.altKey ||
          ((isWindows || isLinux) && value.shiftKey);
        updateAltState();
      }));
    }
    this._register(
    ...
    if (ThemeIcon.isThemeIcon(icon)) {
      ...
      if ((_b = icon.dark) === null || _b === void 0 ? void 0 : _b.scheme) {
        ...
        if (ICON_PATH_TO_CSS_RULES.has(iconPathMapKey)) {
          ...
          createCSSRule(
            `.icon.${iconClass}`,
            `background-image: ${asCSSUrl(icon.light || icon.dark)}`,
          );
          createCSSRule(
...
let SubmenuEntryActionViewItem = class SubmenuEntryActionViewItem
  ...
  constructor(action, _notificationService, _contextMenuService) {
    ...
    if (action.item.icon) {
      if (ThemeIcon.isThemeIcon(action.item.icon)) {
        ...
        (_a = action.item.icon.dark) === null || _a === void 0
          ? void 0
          : _a.scheme
      ) {
        ...
        if (ICON_PATH_TO_CSS_RULES.has(iconPathMapKey)) {
          ...
          createCSSRule(
            `.icon.${className}`,
            `background-image: ${
              asCSSUrl(action.item.icon.light || action.item.icon.dark)
            }`,
          );
          createCSSRule(
    ...
    super(
      ...
      action.actions,
      _contextMenuService,
      { classNames: classNames, menuAsChild: true },
    );
  }
};
