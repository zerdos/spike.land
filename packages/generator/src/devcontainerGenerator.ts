...
export class DevcontainerGenerator {
    ...
    private _templateInputs = ['base', 'android', 'git', 'debianBackports', 'amplify', 'google-chrome','chromium', 'gitUbuntu', 'node', 'cypress', 'dotnet', 'docker', 'kubernetes', 'dotnet3', 'xfce', 'noVNC', 'xpra', 'zsh', 'vscode', 'suffix'];
    ...
    public updateGit(forceFromSoure=false) {
        if ((getDistro(this.base) === "ubuntu") && !forceFromSoure) this._gitVersion='ubuntu';
  ...
  //      if ((this._noVNC===true || this._xfce) && this._xpra===true) throw new Error("You can't have VNC and Xpra on the same image");
    ...
    public async generate() {
        ...
        if (this._gitVersion) {
            if (this._gitVersion === "ubuntu") { this._dockerfile += dockerTemplates['gitUbuntu'] }
            else {
        ...
        if (this._dotnet) {
            if (this._dotnet === "2")
                this._dockerfile += dockerTemplates['dotnet'].replace('{DOTNET_SDK_VERSION}', softwareVersions.dotnet)
                    .replace('{dotnet_sha512}', softwareVersions.sha.dotnet_sha512["2.1.805"]);
            else {
                this._dockerfile += dockerTemplates['dotnet3'].replace('{DOTNET_SDK_VERSION}', softwareVersions.dotnet3)
                    .replace('{dotnet_sha512}', softwareVersions.sha.dotnet_sha512["3.1.201"]);
            }
