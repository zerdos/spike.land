...
const RCA = fetch("https://testing.spike.land/live/rca/index.tsx").then(res=>res.text());
...
const RCA = fetch("https://testing.spike.land/live/rca/index.tsx").then(res=>res.text());
  ...
  await expect( page.locator('[data-test-id=z-body]')).toContainText("Hello", { timeout: 10000 });
~
