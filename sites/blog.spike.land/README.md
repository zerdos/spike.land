...
4. **`gatsby-browser.js`**: This file is where Gatsby expects to find any usage
...
5. **`gatsby-config.js`**: This is the main configuration file for a Gatsby
...
9. **`package-lock.json`** (See `package.json` below, first). This is an
...
10. **`package.json`**: A manifest file for Node.js projects, which includes
...
11. **`README.md`**: A text file containing useful reference information about
    your project.
...
- **For most developers, we recommend starting with our
  [in-depth tutorial for creating a site with
  Gatsby](https://www.gatsbyjs.org/tutorial/).** It starts with zero assumptions
  about your level of ability and walks through every step of the process.
