...
var esbuild_default = "./esbuild-HZXT6DI4.wasm";
...
async function transform2(code, retry = 4) {
  try {
    let result = await esbuild.transform(`/** @jsx jsX */
    import {jsx as jsX} from "@emotion/react";
    ` + code, {
      loader: "tsx",
      target: "esnext"
    });
    const transpiled = result.code.replaceAll(regex1, ' from "/live').replaceAll(regex2, ' from "/live');
    return transpiled;
  } catch (e) {
    if (retry > 0) {
      await wait(100);
