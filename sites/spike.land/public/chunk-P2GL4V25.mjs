...
var CodeSession = class {
  ...
  constructor(room, user, originStr) {
    session = this;
    this.room = room;
  ...
  applyPatch = async ({
    ...
    if (!Object.keys(hashStore).map((x) => Number(x)).includes(Number(oldHash)) && codeSpace) {
      console.log(Object.keys(hashStore));
      const resp = await fetch(`/live/${codeSpace}/mST`);
      if (resp.ok) {
...
import { createRoot } from "/react.mjs";
...
var initShims = async (assets) => import("./es-module-shims-I6DFSQG2.mjs").then(() => importShim.addImportMap({
  "imports": {
...
var appRoot = createRoot(document.getElementById("root") || (() => {
